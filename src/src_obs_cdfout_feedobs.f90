...
!-------------------------------------------------------------------------------
!ROA this doesn't work with PGI
!!$  USE mo_netcdf_param
    ...
    ELSE
      ...
      IF (iveri_ens_member >= 1)                                               &
        WRITE( yveri_descript,'("ensemble forecast member ",I3)' )             &
               iveri_ens_member
      iveri_run_type      =   VT_ANALYSIS
      IF ((iveri_run_class == RC_ASS) .OR. (hverend < 3.01_wp))            &
...
! ------------------------
  DO irep = 1, n_rep
    ...
    nc_body_buf(irep)% obs       (:) = REAL(rmdi)
    nc_body_buf(irep)% bcor      (:) = REAL(rmdi)
    nc_body_buf(irep)% e_o       (:) = REAL(rmdi)
    nc_body_buf(irep)% level     (:) = REAL(rmdi)
    nc_body_buf(irep)% plevel    (:) = REAL(rmdi)
    nc_body_buf(irep)% accuracy  (:) = REAL(rmdi)
    ...
    nc_body_buf(irep)% veri_data (:) = REAL(rmdi)
      ...
      DO iobs = 1, ilenr_b1
        ...
        IF ((lwrobs) .AND. (rodrbufa(iposr_obs) > rmdich)) THEN
          ...
          IF (kfmt == 1) THEN
            ...
            nc_body_buf(irep)% level     (kk) = REAL(rodrbufa(iposr_bdy + ncbp))
...
!                                                      , nvapoc-1 )
              IF (      (rodrbufa(iposr_bdy + ncbz) > rmdich)                  &
                ...
                nc_body_buf(irep)% level     (kk) = REAL(rodrbufa(iposr_bdy + ncbz))
                nc_body_buf(irep)% plevel    (kk) = REAL(rodrbufa(iposr_bdy + ncbp))
              ENDIF
...
!           nc_body_buf(irep)% level     (kk) = rodrbufa(iposr_bdy + ncbz)
          ...
          ENDIF
          nc_body_buf(irep)% obs       (kk) = REAL(rodrbufa(iposr_obs))
          nc_body_buf(irep)% bcor      (kk) = c0
          nc_body_buf(irep)% e_o       (kk) = REAL(rmdi)
          nc_body_buf(irep)% accuracy  (kk) = REAL(rmdi)
          ...
          IF ((iobs == ncbu) .OR. (iobs == ncbv)) THEN
            nc_body_buf(irep)% e_o     (kk) = REAL(rodrbufa(iposr_bdy + ncbuer))
            IF (kfmt == 1)                                                     &
              nc_body_buf(irep)% accuracy(kk) = REAL(rodrbufa(iposr_bdy + ncbuac))
            nvfxbp = nvfubp
            nvrx   = nvru
          ...
          ELSEIF (iobs == ncbt) THEN
            nc_body_buf(irep)% e_o     (kk) = REAL(rodrbufa(iposr_bdy + ncbter))
            nvfxbp = nvftbp
            nvrx   = nvrt
              ! humidity
          ELSEIF (iobs == ncbrh) THEN
            nc_body_buf(irep)% e_o     (kk) = REAL(rodrbufa(iposr_bdy + ncbqer))
            nc_body_buf(irep)% bcor    (kk) = REAL(rodrbufa(iposr_bdy + ncbdrh))
            nvfxbp = nvfqbp
            nvrx   = nvrq
            nvrx2  = nvrqbc
              ! pressure / height
          ELSEIF ((iobs == ncbp) .OR. (iobs == ncbz)) THEN
            IF ((kfmt == 2) .OR. (kfmt == 3))                                  &
              nc_body_buf(irep)% level (kk) = REAL(rodrbufa(iposr_bdy + ncbz))
            nc_body_buf(irep)% e_o     (kk) = REAL(rodrbufa(iposr_bdy + ncbzer))
            nvfxbp = nvfzbp
            nvrx   = nvrz
            nvrx2  = nvrzbc
          ENDIF
          ...
          ELSEIF (kfmt == 3) THEN
            ...
            IF (iobs == ncbiwa) THEN
              IF (rodrbufa(iposr_bdy + ncbiwc) > rmdich)                       &
                nc_body_buf(irep)% bcor  (kk) = REAL(rodrbufa(iposr_bdy + ncbiwc))
          ...
          IF (      (nc_body_buf(irep)% varno     (kk) == VN_HEIGHT)           &
            ...
            nc_body_buf(irep)% plevel (kk) = REAL(rmdi)
            kk = kk - 1
        ...
        IF ((kobv /= imdi) .AND. (ixobs_bis(iobs) >= 0)) THEN
          ...
          IF (iobs == ncbwwe) THEN
            ...
            IF (kobv /= nibits( nvw0oc )) THEN
              kk = kk + 1
              laddobs = .TRUE.
              nc_body_buf(irep)% obs       (kk) = REAL(kobv)
              nc_body_buf(irep)% level     (kk) = REAL(c0)
...
!   ENDIF
    ...
    DO icl = 1, MIN( ilstidc , 10 )
      report(irep)% header% statid(icl:icl) = CHAR( iodrbufa(iposi+mxchi(kfmt) &
                                                                  +icl) )        
    ENDDO
    report(irep)% header% lat          = REAL(rodrbufa( iposr + nchlat ))
    report(irep)% header% lon          = REAL(rodrbufa( iposr + nchlon ))
    report(irep)% header% sun_zenit    = REAL(rodrbufa( iposr + nchsol ))
