...
!------------------------------------------------------------------------------
  ...
  IF ( l_diff_cold_pools ) THEN
    ...
    T_threshold = 10.0_wp   ! threshold temperature in K
    ...
    CALL targeted_diffusion_cold_pools( Tp(:,:,:), T_threshold, dk )
...
SUBROUTINE targeted_diffusion_cold_pools( Tp, T_threshold, dk )
  ...
  INTEGER (KIND=iintegers), INTENT(IN)  :: dk  

  INTEGER (KIND=iintegers), ALLOCATABLE :: i_liste(:)
  INTEGER (KIND=iintegers), ALLOCATABLE :: j_liste(:)
  INTEGER (KIND=iintegers), ALLOCATABLE :: k_liste(:)

  INTEGER (KIND=iintegers) :: nmbr_points
  INTEGER (KIND=iintegers) :: i,j,k,l
  ...
  INTEGER (KIND=iintegers) :: ibegin, istop
  INTEGER (KIND=iintegers) :: jbegin, jstop

  INTEGER (KIND=iintegers) ::  izerror
  CHARACTER (LEN=80)       ::  yzerrmsg


  !WRITE(*,*) "Subr. [targeted_diffusion_cold_pools] ..."

  ALLOCATE( i_liste(ie*je*dk) )
  ALLOCATE( j_liste(ie*je*dk) )
  ALLOCATE( k_liste(ie*je*dk) )
  ALLOCATE( T_new  (ie*je*dk) )
  ...
  DO k= ke-dk, ke
    DO j=jbegin, jstop
      DO i=ibegin, istop
        ...
        IF ( Tp(i,j,k) < Tp_environ - T_threshold ) THEN
  ...
  IF (nmbr_points > 0) THEN
    WRITE(*,'(A,I6,A)') "  WARNING: targeted diffusion at ", nmbr_points, " gridpoints"
    DO l=1, MIN( nmbr_points, 10)
      i = i_liste(l)
      j = j_liste(l)
      k = k_liste(l)
      WRITE(*,'(3I5,F13.6,A,4F13.6)')  i_global(i), j_global(j), k,    &
         &  Tp(i,j,k), " -- ", Tp(i-1,j,k), Tp(i+1,j,k), Tp(i,j-1,k), Tp(i,j+1,k)
    END DO
  END IF
