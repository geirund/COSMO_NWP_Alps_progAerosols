...
!<FG: 2M-scheme
    iradpar_cloud,& ! type of parameterization of radiative transfer
                    ! parameters (ext., sing. alb., asym.) for grid- 
                    ! and subgrid scale clouds (cloud water, ice water)
...
!cloud forcing>
    lcrf,         & ! if .true., computes clear-sky fluxes in order to 
                    ! obtain the cloud radiative forcing (CRF)
!cloud forcing<
    ...
    !(WL;2010)b
    lneutral,     & ! Flag for transfer coefficients for neutral PBL
    ...
    lINprog,      & ! include prognostic equation for IN following Solomon & Feingold 2015 (ACP)
    lCCNprog,     & ! include prognostic aerosol number concentration for CCN size distribution
    ...
    !OCH<<
    !WL2011b 
    lbud,         & ! if .TRUE., budget arrays are allocated and calculated,
    lbud_avg,     & ! if .True., average tendencies are computed in lmorg.f90
    lbud_h_theta, & ! currently these flags switch also the budget tool for moisture variables
    !WL2011e 
    lnc_const,    & !constant NCLOUD in src_twomom_sb.f90
    lni_const_imm   !constant NICE in src_twomom_sb.f90 only produced by immersion freezing
  ...
  REAL (KIND=wp)     ::           &
    ...
    Tdiff_IN,     & ! temperature interval for initialisation
    Tmax_IN,      & ! warmest bin temperature
    Na_IN,        & ! number of aerosols (DeMott 2015) in 1/cm3
    CCN_ship_N,   & ! number of aerosols for CCN in 1/m3
    CCN_1_N,      & ! number of ccn1 aerosol size distri
    CCN_2_N,      & ! number of ccn2 aerosol size distri
    CCN_3_N,      & ! number of ccn3 aerosol size distri
    Na_IN_pert,   & ! number perturbation for ship Na_IN
    tperturb        ! time [h] after which ship perturbation is applied
...
! -------------------------------
  ...
  !LS, b
    lrelax,       & ! whether relaxation towards profile is performed in the idealized runs
    lrelax_soil,  & ! whether relaxation for soil is performed in the idealized runs
  !LS, e
  !SBoeing, b
    llsf
  !SBoeing, e
...
! -----------------------------------
  ...
  INTEGER   (KIND=iintegers),PARAMETER       ::          npatmax=5
  ...
  INTEGER   (KIND=iintegers)       ::           &
    ...
    ninc_rn(npatmax),     & ! time step increment for drawing a new set of 
                            ! random numbers
    hinc_rn(npatmax),     & ! time step increment for drawing a new set of 
                            ! random numbers in hours
    nstep_rn(npatmax),    & ! actual random number time step
    npattern_rn,          & ! number of patterns with different scale/correl.
    nseed_rn(npatmax),    & ! external specified seed for random number generation
    itype_vtaper_rn,      & ! type of vertical tapering near surface and/or in stratosphere
    ...
    ie_rn(npatmax),       & ! number of hor. coarse grid points in zonal dir. 
    je_rn(npatmax)          ! number of hor. coarse grid points in merid. dir.
