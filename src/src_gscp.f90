...
USE stoch_physics,            ONLY : apply_tqx_tend_adj
...
DO  k = 1, ke
  ...
  IF ( ncc > 0 ) THEN  ! Calculation of the conversion rates
    DO ij = 1, ncc
      ...
      END IF
      zztau     = 1.5_wp*( EXP(0.66_wp*LOG(zmsmin/zmi)) - 1._wp)
      zsdau     = zsvidep/zztau
...
!FR old    
!   ccsdep = 0.26_wp*gamma_fct((zv1s+5.0_wp)/2.E0_wp)*SQRT(0.5_wp/zeta)
    ccsdep = 0.26_wp*gamma_fct((zv1s+5.0_wp)/2.E0_wp)*SQRT(1.0_wp/zeta)
...
!   ccdvtp = 2.11E-5_wp * t0_melt**(-1.94_wp) * 101325.0_wp
    ...
    zcevxp = (mu_rain+2._wp)/(mu_rain+4._wp)
    zcev   = 2.0_wp*pi*zdv/zhw*zn0r*zar**(-zcevxp) * gamma_fct(mu_rain+2.0_wp)
    zbevxp = (2._wp*mu_rain+5.5_wp)/(2._wp*mu_rain+8._wp)-zcevxp

!FR old 
!   zbev   = 0.26_wp * SQRT(0.5*zrho0*130.0_wp/zeta)*zar**(-zbevxp)
...
#ifdef TEND
            countref(i,j,k) = countref(i,j,k) + 1._wp
            IF(ldum) counter(i,j,k) = counter(i,j,k) + 1._wp
#endif
          ENDIF
        ENDIF
...
#ifdef TEND
      ttend_hyd(i,j,k) = ttend_hyd(i,j,k) + ztt      ! Sum T tendencies of microphysic
      qvten_hyd(i,j,k) = qvten_hyd(i,j,k) + zqvt     ! Sum QV tendency of microphysic
#endif
