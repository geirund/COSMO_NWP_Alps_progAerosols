  ...
  ELSE
    ...
    ELSE
      IF (i1 == 0) THEN
        ifirsthour = 1_iintegers
      ELSE
!FUO  The following condition seems weird (rounding error).
!FUO  Could write IF(MOD(i1,3600_int_dp) == 0_int_dp) instead.
        IF (MOD(REAL(i1, wp),3600.0_wp) == 0.0_wp) THEN
          ifirsthour = INT (i1 / 3600_int_dp, iintegers)
        ELSE
!US       ifirsthour = INT (REAL(i1,wp) / 3600.0_wp, iintegers) + 1
