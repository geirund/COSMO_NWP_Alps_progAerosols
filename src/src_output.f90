...
USE data_io,        ONLY : &
  ...
  llockfiles           ! indicates whether to use lock files or not
  ...
  IF (now%lwrite_const .EQV. .TRUE.) THEN
    ...
    IF (now%yform_write /= 'ncdf' .OR. nc_asyn_io < 1) THEN
      ...
      CALL open_file (nuedat, yname, ymode_write, now%yform_write, icomm_cart,   &
                      my_cart_id, num_compute, lasync_io, llockfiles, idbg_level,&
                      yzerrmsg, izerror)
      IF (izerror /= 0) THEN
...
#ifdef MESSY
  ivar_id(:) = 0._wp  ! um_ak_20080312
  zbrn(:,:,:) = 0._wp ! um_gg_20121024
...
!------------------------------------------------------------------------------
  ...
  IF (outblock%yform_write /= 'ncdf' .OR. nc_asyn_io < 1) THEN
    ...
    CALL open_file (nuedat, yname, ymode_write, TRIM(outblock%yform_write),      &
                    icomm_cart, my_cart_id, num_compute, lasync_io, llockfiles,  &
                    idbg_level, yerrmsg, ierrstat)
    IF (ierrstat /= 0) THEN
      WRITE(*,*) 'nuedat'
      WRITE(*,*) nuedat
      WRITE(*,*) 'yname'
      WRITE(*,*) yname
      WRITE(*,*) 'TRIM(outblock%yform_write)'
      WRITE(*,*) TRIM(outblock%yform_write)
      WRITE(*,*) 'icomm_cart'
      WRITE(*,*) icomm_cart
      WRITE(*,*) 'my_cart_id'
...
#ifndef MESSY
    !WL2011b
  ! reset also for lbdclim=False
  !IF (lbdclim .AND. (.NOT. lzrestart)) THEN
  IF (.NOT. lzrestart) THEN
  !WL2011b
    ...
    reset_loop: DO n = 1, numlist
      ...
      !WL2011b
      ! Resetting of 3D variables
      CASE(3)
        IF ( (var(i1,i2,i3)%ntri == 3) .OR. (var(i1,i2,i3)%ntri == 4) ) THEN
          IF (ASSOCIATED(var(i1,i2,i3)%p3)) THEN
             var(i1,i2,i3)%p3(:,:,:) = 0.0_wp
          ENDIF
        ENDIF
      CASE(4)
      ! Resetting of 4D variables
        IF ( (var(i1,i2,i3)%ntri == 3) .OR. (var(i1,i2,i3)%ntri == 4) ) THEN
          IF (ASSOCIATED(var(i1,i2,i3)%p4)) THEN
             var(i1,i2,i3)%p4(:,:,:,:) = 0.0_wp
          ENDIF
        ENDIF
      !WL2011e
...
!------------------------------------------------------------------------------
  ...
  IF (outblock%yform_write /= 'ncdf' .OR. nc_asyn_io < 1) THEN
    CALL close_file (nuedat, TRIM(outblock%yform_write), icomm_cart, my_cart_id, &
                     num_compute, lasync_io, llockfiles, idbg_level, yerrmsg,    &
                     ierrstat)
...
REAL    (KIND=wp)               :: ds_api(ie_tot*je_tot)
  ...
  IF (.NOT. lflush) THEN
    ...
    IF ((.NOT. lrestart) .AND. (var(i1,i2,i3)%ntri == 3)) THEN
      ...
      ELSE
        ...
        !WL2011b
        !Also for lbdclim=F, we are interested in the average between output steps.
        !Who wants to see averaged quantities affected by spin-up?
        !ELSE
    ...
    IF ( loutput ) THEN
      ...
      IF (outblock%yform_write /= 'ncdf') THEN
        ...
        IF (outblock%lanalysis) THEN
          ...
          IF (ABS(NINT(z2) - z2) > 1E-5_wp) THEN
      ...
      IF (.NOT. lrestart) THEN
        ...
        DO j = outblock%j_out_start, outblock%j_out_end
          DO i = outblock%i_out_start, outblock%i_out_end
            ...
            ds_api(ij_out) = REAL(ds_grib(ij_ful),wp)
          ENDDO
        ENDDO
...
!print *, 'message size:   ', ibyte_size_out, lfa
   ...
   IF (l_COSMO_now .AND. L_BM_ORIG_OUTPUT)  THEN
    ...
    IF (izerror /= 0) THEN
      ...
      WRITE(*,*) 'izerror'
      WRITE(*,*) izerror
      WRITE(*,*) ' src_output: problems ', TRIM(var(my_i1, my_i2, my_i3)%name), my_k, ' problems'
...
! Output from spline
REAL(KIND=wp)                  :: fpfls(outblock%kepin),  &
                                  pfls (outblock%kepin)
  ...
  DO j=jstartpar,jendpar
    ...
    IF (TRIM(yzname) == 'T') THEN
      ...
      DO i=istartpar,iendpar
        ...
        ENDIF
        IF(hsurf(i,j) > 2500_wp) THEN
          zt0s   = MIN(zt0(i,j),298.0_wp)
          zalpha(i,j) = r_d * (zt0s - ztstar(i,j))/(hsurf(i,j)*g)
          IF(zt0s < ztstar(i,j)) zalpha(i,j) = 0.0_wp
        ELSEIF((hsurf(i,j) <= 2500_wp)                        &
          .AND.(hsurf(i,j) >= 2000_wp)) THEN
          zt0s   = 0.002_wp * (2500_wp-hsurf(i,j) * zt0(i,j)  &
                   +(hsurf(i,j)-2000_wp)*MIN(zt0(i,j),298.0_wp))
          zalpha(i,j) = r_d * (zt0s - ztstar(i,j))/(hsurf(i,j)*g)
          IF(zt0s < ztstar(i,j)) zalpha(i,j) = 0.0_wp
        ENDIF
      ENDDO
...
!!$    END IF
  ...
  IF (yzname(1:izlen) == 'DBZ') THEN
    WHERE (results(istartpar:iendpar,jstartpar:jendpar,:) >= 1e-9_wp)
...
! Output from spline
REAL(KIND=wp)                  :: fzfls(outblock%kezin),  &
                                  zfls (outblock%kezin)
...
! Local Variables
REAL    (KIND=wp)                        :: zp, hsurf_max, wgt, geopot_diff, &
                                            geopot_sm(ie,je)
INTEGER (KIND=iintegers)                 :: i, j, ii, jj, n, nmean
