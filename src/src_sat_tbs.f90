...
USE data_satellites , ONLY :  &
    ...
    utmn, uqmn, ppres_d, o3_ref, const_aheat, r_sat, jplev

!------------------------------------------------------------------------------

USE rttov7_dp_wrapper, ONLY:    &
    RTTOV7_supsat_vec_wrapper , &
    RTTOV7_spline_vec_wrapper , &
    RTTOV7_RTTOVCLD_wrapper
...
INTEGER (KIND=iintegers) ::                  &
  i, j, k, kl, n, nzp, isens, ic, izmaxerror, izmaxtwarn, izmaxqwarn,  &
  iatm, jlev, ichan, izerr, igatherbuf(3), klu
...
!------------------------------------------------------------------------------
  ...
  ! initialize input arrays of RTTOV7 wrapper
  ! subroutines to zero to avoid FPEs in SP
  presfin (:,:)   = 0.0_wp
  tfin    (:,:)   = 0.0_wp
  wvvmfin (:,:)   = 0.0_wp
  as      (:,:,:) = 0.0_wp
  ...
  !----------------------------------------------------------------------------
    ...
    wvs = 0.0_wp ! initialize return array to zero to avoid FPE in SP
    CALL RTTOV7_supsat_vec_wrapper (t_2m(:,j), wvs, psurfmod, idim, idims, idime)
    ...
    wsup = 0.0_wp ! initialize return array to zero to avoid FPE in SP
    CALL RTTOV7_supsat_vec_wrapper (tempsup, wsup, psup, n, 1, n)
    ...
    aryint(:,:) = 0.0_wp ! initialize return array to zero to avoid FPE in SP
    CALL RTTOV7_spline_vec_wrapper(                  &
            presfin(:,:), tfin(:,:)  , ppres_d(:,1), &
            aryint (:,:), as  (:,:,:), index  (:)  , &
            nlevels+ke  , nlevels    , idim        , &
            idims       , idime      , 1             )
    tint(idims:idime,:)      = aryint(idims:idime,:)

    aryint(:,:) = 0.0_wp ! initialize return array to zero to avoid FPE in SP
    CALL RTTOV7_spline_vec_wrapper(                   &
            presfin(:,:), wvvmfin(:,:), ppres_d(:,1), &
            aryint (:,:), as   (:,:,:), index  (:)  , &
            nlevels+ke  , nlevels     , idim        , &
            idims       , idime       , 1             )
