...
!SCLM ---------------------------------------------------------------------------------
    ...
    zx         = SQRT(ctau1/ctau2)
    zomb       = 2._wp*pi/(ctau1*86400._wp)
    DO jb      = 1, 10
      zzdlam   = cala1(jb) - cala0(jb)
      zwmean   = 0.5_wp*(cfcap(jb) + cpwp(jb))
      z4wdpv   = 4._wp*zwmean/cporv(jb)
      ...
      cdz1(jb) = SQRT (2._wp*zzalam/(zrhoc*zomb))/(1._wp+zx)
  ...
  DO   j = jstarts, jends
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN   ! for land-points only
        ...
        xrhoch(i,j)    = ztmch(i,j)*(1._wp/g) + eps_soil
        ...
        zep_snow(i,j) = (1._wp-ztsnow_pm(i,j))*tfv(i,j)*xrhoch(i,j)*zdqsnow
    ...
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        xalam(i,j)   = xalam(i,j) + zdlam(i,j)                                &
                      * (0.25_wp + 0.30_wp*zdlam(i,j) / (1._wp+0.75_wp*zdlam(i,j)))  &
                      * MIN (z4wdpv, 1.0_wp + (z4wdpv-1.0_wp)    &
        ...
        zrww =       ztsnow_pm(i,j)  * MAX(0.01_wp,                       &
                    1._wp - EXP ( MAX (-5.0_wp, -zwin  (i,j)/cf_w) ) )
          ...
          IF (llandmask(i,j)) THEN     ! land-points only
            IF (zep_s(i,j) < 0.0_wp  &           ! potential evaporation and
              ...
              ztlpmwp = (zfcap(i,j) - zpwp(i,j)) &
                       *(0.81_wp + 0.121_wp*ATAN(-86400._wp*zep_s(i,j) - 4.75_wp))
...
! select timelevel and timestep for calculations
  ...
  zx      = SQRT(ctau1/ctau2)
  zomb    = 2._wp*pi/(ctau1*86400._wp)  
  zalfbs  = 1._wp + zx + zx*zx
  zalfms  = zx
  zbetss  =    SQRT(1._wp + zx*zx)*EXP(  zx/(1._wp + zx))
  zbetms  = zx*SQRT(1._wp + zx*zx)*EXP( -zx/(1._wp + zx))
  zdmddb  = zalfbs/zbetms - 1._wp
  zomhqx  = SQRT(0.5_wp*zomb)/(1._wp + zx)
  ...
  DO   j = jstarts, jends
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN
        zgstr = - sigma*(1._wp - Ctalb) * ( (1._wp - fr_snow(i,j))*zts(i,j) &
    ...
    DO i = istarts, iends
      IF(llandmask(i,j))THEN     ! land-points only
        ...
        zverbo(i,j) = xdwidt(i,j) + xesoil(i,j) + xtrang(i,j,1) + xtrang(i,j,2)&
                      + xtrang(i,j,3) + (1._wp-fr_snow(i,j))*(xrr(i,j) + xrs(i,j))
        ...
        zwimax = cwimax*(1._wp + plcov(i,j)*5._wp)
        ...
        IF (xrr(i,j)-eps_soil > 0.0_wp) THEN
          ...
          zalf = zts_pm(i,j)*zalf + (1._wp - zts_pm(i,j))
        ...
        zvers = zvers + (1._wp - zalf)*xrr(i,j)
  ...
  DO   j = jstarts, jends
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        zrnet_s = (1._wp - fr_snow(i,j))*(sobs(i,j)+zthsoi(i,j)) 
        zshfl_s = (1._wp - fr_snow(i,j))*cp_d*xrhoch(i,j)*(xth_low(i,j) - zts(i,j))
        zlhfl_s = (zts_pm(i,j)*lh_v + (1._wp-zts_pm(i,j))*lh_s)*zverbo(i,j)
        ...
        ELSEIF (zwsnow(i,j) == 0.0_wp .AND. (1._wp-ztsnow_pm(i,j))*xrr(i,j) > 0.0_wp)THEN
          zsprs   = lh_f*xrr(i,j)
        ...
        zfor_s = zrnet_s + zshfl_s + zlhfl_s + zsprs                          &
                         + fr_snow(i,j) * (1._wp-ztsnow_pm(i,j)) * zgsb
        ...
        IF (ztsnow(i,j) < t0_melt .AND. zwsneu > eps_soil) THEN
          ztsnown(i,j) = ztsnow(i,j) + zdt*2._wp*(zfor_snow - zgsb)/xrocs(i,j)  &
                                    - ( ztsn(i,j) - zts(i,j) )
          ! implicit formulation  
          ztsnow_im    = - xrhoch(i,j) * (cp_d + xdqvtsnow(i,j) * lh_s)       &
                                       - zalas/xdzs(i,j)
          zfak  = MAX(eps_soil,1._wp - zdt*zimpli*ztsnow_im/xrocs(i,j))
          ...
          IF (ztsnew > t0_melt) THEN
            ...
            ELSE    ! melting
              ...
              zdtsdt(i,j) = zdtsdt(i,j) - zeisb * MIN (zdtsmx,               &
                                    2._wp* (zensm-zenm_snow) * z1d2dt/xrocg(i,j))
              ztsnew      = zts(i,j) + zdt*zdtsdt(i,j)
              ztsnownew   = 2._wp*(t0_melt - eps_soil) - ztsnew
              ...
              ztsnownew      = (1._wp - zfak ) * ztsnownew                       &
