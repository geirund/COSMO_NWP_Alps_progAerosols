...
!              -----------------------------------------------------------
    nvrdbp        ! bit pos. for flag: 'redundant report'           nhschr
...
SUBROUTINE obs_cdf_proc_init ( lobpfrs , ndimev2 , ndimev3 )
...
!   Check whether (report ID's are identical, both reports are ships, or one
!                  is a surface report from a TEMP and the other is a SYNOP),
!     and whether both reports are aircrafts, or sat winds
...
! --------------------------------------------------------------------------
    ...
    IF (     (osgbdy(nodract,nbstn ) <  rmdich)                                &
        .AND.(osgbdy(nodrrej,nbstn ) >  rmdich) .AND. (lrepla))                &
      osgbdy (nodract,nbstn ) = osgbdy (nodrrej,nbstn )
    IF (     (osgbdy(nodract,nbstx ) <  rmdich)                                &
        .AND.(osgbdy(nodrrej,nbstx ) >  rmdich) .AND. (lrepla))                &
...
!   IF (     (osgbdy(nodract,nbscfw) ==   imdi)                                &
!       .AND.(osgbdy(nodrrej,nbscfw) /=   imdi) .AND. (lrepla))                &
!     mosgbd (nodract,nbscfw) = mosgbd (nodrrej,nbscfw)
...
!     mosghd (nodrrej,nhqofl)  = IBSET( mosghd(nodrrej,nhqofl) , nf3rbp )
    ELSE
...
!             =====
    ...
    ELSEIF ((npassiv < 2) .AND. (npassv2 < 2)) THEN
      neventr (neredx,icma,1) = neventr(neredx,icma,1) + 1
    ENDIF
...
!     momlhd (nmlrj ,nhqofl)  = IBSET( momlhd(nmlrj,nhqofl) , nf3rbp )
    ELSE
...
!             =====
    IF ((npassiv < 2) .AND. (npassv2 < 2)) THEN
      cma(icma)%cnt_ac = cma(icma)%cnt_ac - 1
      IF (iactio == 1)  neventr (neredn,icma,1) = neventr(neredn,icma,1) + 1
      IF (iactio == 2)  neventr (neslps,icma,1) = neventr(neslps,icma,1) + 1
    ELSE
      cma(icma)%cnt_ps = cma(icma)%cnt_ps - 1
    ENDIF
    IF ((lverpas) .AND. (.NOT. lsub) .AND. (iactio == 1)) THEN
...
!     mogphd (nodrrej,nhqofl)  = IBSET( mogphd(nodrrej,nhqofl) , nf3rbp )
    ELSE
! this report will be discarded in 'obs_cdf_del_old_rep'
...
SUBROUTINE obs_cdf_mult_qualicheck ( ntotmlo, ntotml, maxmlv, i1evnt, i2evnt   &
                                   , rdocp )
...
! --------------------
  ...
  INTEGER (KIND=iintegers), INTENT(IN) :: &
    i1evnt  (ntotml+1) ,& ! first  index \ for data event
    i2evnt  (ntotml+1)    ! second index / counters
      ...
      IF (krej(klev) /= 0) THEN
        ...
        IF (.NOT. BTEST( momlbd(iob,ilev,nbtflg), nvftbp+nvfbps(4) ))          &
          momlbd (iob,ilev,nbtflg) = IBSET( momlbd(iob,ilev,nbtflg)            &
                                          , nvftbp +nvfbps(4) )
        IF (lactrep) THEN
          neventd (netlps,i1evnt(iob),i2evnt(iob)) =                           &
          neventd (netlps,i1evnt(iob),i2evnt(iob)) + 1
          ilen = 7 + istrej
    ...
    DO klev = 1 , nclev
      IF (krej(klev) /= 0) THEN
        ...
        IF ((lactrep) .AND. (krej(klev) == 1))                                 &
          neventd (nefshr,i1evnt(iob),i2evnt(iob)) =                           &
          neventd (nefshr,i1evnt(iob),i2evnt(iob)) + 1
        IF ((lactrep) .AND. (krej(klev) >= 2))                                 &
          neventd (nedshr,i1evnt(iob),i2evnt(iob)) =                           &
          neventd (nedshr,i1evnt(iob),i2evnt(iob)) + 1
        IF (lactrep) THEN
          ilen = 7 + istrej
          IF (nacout+ilen <= nmxoln) THEN
            outbuf(nacout+1) = ilen
