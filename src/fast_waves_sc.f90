...
SUBROUTINE fast_waves_strong_conserv ( n_small_steps, dts, xkd,             &
  ...
  DO n_step = 1, n_small_steps 
    ...
    IF ( l_no_div_damping_in_1st_step .AND. (n_step == 1) ) THEN
      switch_div_damp = 0.0_wp
    ELSE 
      switch_div_damp = 1.0_wp
    END IF
    ...
    IF ( ldyn_bbc ) THEN
      ...
      CALL dynamic_bottom_BC_for_p( pp_minus_div, 0.0_wp )
    END IF
    ...
    IF ( n_step == 1 ) THEN
      ...
      DO k=1, ke
        DO j=jstart, jend
          DO i=istart, iend
            w_k_avg_old(i,j,k)  = 0.5_wp * ( w(i,j,k+1) + w(i,j,k) ) 
          END DO
        END DO
      END DO
    END IF
    ...
    ELSE
      ...
      ! w(:,:,1)    = 0.0_wp
      w( istart:iend, jstart:jend, 1) = 0.0_wp
...
CONTAINS
  ...
  SUBROUTINE dynamic_bottom_BC_for_p( pp_mod, switch_div_damp )
    ...
    REAL(KIND=wp),     INTENT(in) :: switch_div_damp 
...
SUBROUTINE init_div_damping_coeff( xkd, dts, alpha_div_h, alpha_div_v_to_h )
  ...
  REAL (KIND=wp)      :: alpha_div_h_min, alpha_div_h_max
  REAL (KIND=wp)      :: alpha_div_h_avg
  REAL (KIND=wp)      :: c_sound = 330.0_wp
  REAL (KIND=wp)      :: work(ie,je) 

  REAL (KIND=wp)      :: dlon_arg, dlat_arg
  REAL (KIND=wp)      :: delta_x_inv, delta_x_inv_0, delta_y_inv
  REAL (KIND=wp)      :: ax, ay, h1

  REAL (KIND=wp)      :: dz_dlam, dz_dphi
  REAL (KIND=wp)      :: wght0, wght1, wght2, wght_fac
  ...
  wght_fac = 0.25_wp
  wght2 = wght_fac**2
  wght1 = wght_fac * ( 1.0_wp - 2.0_wp * wght_fac )
  wght0 = ( 1.0_wp - 2.0_wp * wght_fac )**2
  ...
  alpha_div_h_max = -1.0e30_wp
  alpha_div_h_min =  1.0e30_wp

  alpha_div_h_avg = 0.0_wp
    ...
    DO k=vcoord%kflat, ke
      ...
      DO j=1, je-1
        ...
        DO i=1, ie-1
          ...
          ax = (delta_x_inv *(2.0_wp + dlon_arg *ABS(dz_dlam) ) )**2
          ay = (delta_y_inv *(2.0_wp + dlat_arg *ABS(dz_dphi) ) )**2

          !ax = (delta_x_inv *(2.0_wp + dlon_arg *ABS(dz_dlam(i,j,k))))**2
          !ay = (delta_y_inv *(2.0_wp + dlat_arg *ABS(dz_dphi(i,j,k))))**2

          !work(i,j) = 5.0_wp * 2.0_wp / ( dts * ( ax + ay ) )
          work(i,j) = divdamp_slope * 2.0_wp / ( dts * ( ax + ay ) )
