...
zbp  = (1._wp+betasw)*0.5_wp
zbm  = (1._wp-betasw)*0.5_wp
zbp2 = zbp**2
...
DO    k = 1, ke
  ...
  DO  j = jstart, jend
    DO  i = istart, iend
      zgu       = 1._wp - zgfakt(i,j,k  )*dp0(i,j,k-1)
      zgo       = 1._wp + zgfakt(i,j,k-1)*dp0(i,j,k)
      zaw       = zcw1(i,j,k)*zgo*zbp2
      zcw       = zcw1(i,j,k)*zgu*zbp2
      za(i,j,k) =    - zaw*( zcp1(i,j,k-1)-rho0(i,j,k-1)*zgdtsh )
      zb(i,j,k) = 1._wp + zcw*( zcp1(i,j,k  )-rho0(i,j,k  )*zgdtsh )    &
