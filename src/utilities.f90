...
!
!     - fsleep:
!       Fortran binding to C-function sleep() which allows to pause a program
!       for a specified number of seconds
!
!     - check_field_NaN
!       function to check for the occurrence of not a number values inside
!       a field of arbitrary rank
!
!     - check_cold_pools
!       function to check for the occurrence of cold pools
!
...
!------------------------------------------------------------------------------
  DATA fw / -0.00390625_dp, 0.03125_dp, -0.109375_dp,     &
             0.21875_dp,    0.7265625_dp /
...
!------------------------------------------------------------------------------
  DATA fw / -0.00390625_sp, 0.03125_sp, -0.109375_sp,     &
             0.21875_sp,    0.7265625_sp /
...
! inhour is the same in (full) hours
  ...
  ELSE
     inhour = INT(FLOOR(REAL(insec,wp)/3600_wp),iintegers)
  ENDIF
...
! Epsilons
REAL(KIND=wp),PARAMETER               :: &
    EPS_DIV = repsilon    ! avoid division by zero: a/MAX(b,eps_div)
...
!                    ALPHA_VEC(i) = ALPH(ONEMZT_VEC(i))
                     ...
                     COEF(i,4,L-1) = COEF(i,4,L) -6.0_wp*D*                 &
                                       (1.0_wp-ALPHA_VEC(i))/MAX(DEL*ZT2_VEC(i),EPS_DIV)
...
! REAL    (KIND=wp)       , INTENT(IN)    ::  idata (nn)  ! array to be sorted
...
SUBROUTINE check_field_NaNs_4d_double (field, fieldname, found, my_id)
  ...
  INTEGER (KIND=iintegers)            :: NaNs          ! counter
  INTEGER (KIND=iintegers)            :: i, j, k, l    ! loop indexes
  INTEGER (KIND=iintegers)            :: ii, jj, kk, ll! loop limits
  INTEGER (KIND=iintegers)            :: i1, j1, k1, l1! memorize last occurrence
...
SUBROUTINE check_field_NaNs_3d_double (field, fieldname, found, my_id)
  ...
  INTEGER (KIND=iintegers)            :: NaNs          ! counter
  INTEGER (KIND=iintegers)            :: i, j, k       ! loop indexes
  INTEGER (KIND=iintegers)            :: ii, jj, kk    ! loop limits
  INTEGER (KIND=iintegers)            :: i1, j1, k1    ! memorize last occurrence
...
SUBROUTINE check_field_NaNs_2d_double (field, fieldname, found, my_id)
  ...
  INTEGER (KIND=iintegers)            :: NaNs          ! counter
  INTEGER (KIND=iintegers)            :: i, j          ! loop indexes
  INTEGER (KIND=iintegers)            :: ii, jj        ! loop limits
  INTEGER (KIND=iintegers)            :: i1, j1        ! memorize last occurrence
...
SUBROUTINE check_field_NaNs_1d_double (field, fieldname, found, my_id)
  ...
  INTEGER (KIND=iintegers)            :: NaNs          ! loop indexes
  INTEGER (KIND=iintegers)            :: i             ! loop indexes
  INTEGER (KIND=iintegers)            :: ii            ! loop limits
  INTEGER (KIND=iintegers)            :: i1            ! memorize last occurrence
...
!-------------------------------------------------------------------------------
!   check for occurences of cold pools
!       t         : temperature field
!       hhl       : height of layers
!       my_id     : PE number
!-------------------------------------------------------------------------------
SUBROUTINE check_cold_pools(t, hhl, nstep, my_id, nboundlines)

  IMPLICIT NONE

  REAL      (KIND=wp),        INTENT(in) :: t(:,:,:)   ! temperature field
  REAL      (KIND=wp),        INTENT(IN) :: hhl(:,:,:) ! heigh of layers
  INTEGER   (KIND=iintegers), INTENT(IN) :: nstep      ! current timestep
  INTEGER   (KIND=iintegers), INTENT(IN) :: my_id      ! MPI rank of calling PE
  INTEGER   (KIND=iintegers), INTENT(IN) :: nboundlines! number of boundary lines to ignore

  INTEGER (KIND=iintegers)            :: nfound        ! counter
  INTEGER (KIND=iintegers)            :: i, j, k       ! loop indexes
  INTEGER (KIND=iintegers)            :: ii, jj, kk    ! loop limits
  INTEGER (KIND=iintegers)            :: i1, j1, k1    ! memorize worst occurrence
  REAL    (KIND=wp)                   :: t_std         ! standard atmosphere temperature
  REAL    (KIND=wp)                   :: t_thresh      ! temperature threshold above which to issue warning
  REAL    (KIND=wp)                   :: t_diff        ! difference between model and standard atmosphere
  REAL    (KIND=wp)                   :: t_worst       ! largest difference
  REAL    (KIND=wp)                   :: t_diff_worst  ! largest difference
  REAL    (KIND=wp)                   :: alt, alt_worst
  ...
  ! initialize the worst difference
  t_diff_worst = 0.0_wp

  ! we only search the lower half of the atmospheric levels
  DO k = kk/2, kk
    DO j = 1+nboundlines, jj-nboundlines
      DO i = 1+nboundlines, ii-nboundlines

        ! compute difference to standard atmosphere
        alt = 0.0005_wp * (hhl(i,j,k) + hhl(i,j,k+1)) ! height of T-level in km
        t_std = StdAtmosphere(alt)                    ! compute standard atmosphere
        t_diff = ABS(t(i,j,k) - t_std)                ! absolute difference

        ! check if threshold is exceeded
        IF (t_diff > t_thresh) THEN

          nfound = nfound+1_iintegers

          ! save worst difference
          IF (t_diff > t_diff_worst) THEN
            t_diff_worst = t_diff
            alt_worst = 1000.0_wp*alt
            t_worst = t(i,j,k)
            i1 = i
            j1 = j
            k1 = k
  ...
  IF (nfound > 0) THEN
    WRITE(0,'(A,I7,A,A,I5,A,3(I4,A),F6.1,A,F8.1,A)')               &
         'WARNING:', nfound, ' gridpoints failed coldpool check ',   &
         'on PE ', my_id, ' e.g. (i,j,k) = (', i_global(i1), ',',    &
         j_global(j1), ',', k1, ') has a temperature of ', t_worst,  &
         'K at ', alt_worst, 'm'
  ENDIF

CONTAINS

  PURE FUNCTION StdAtmosphere(alt)
  !   -------------------------------------------------------------------------
  ...
  !     A R G U M E N T S                                                     |
  !============================================================================
    REAL(KIND=wp),INTENT(IN):: alt                    ! geometric altitude, km.
    REAL(KIND=wp)           :: StdAtmosphere    ! temperature of std atmosphere
  !============================================================================
  !     L O C A L   C O N S T A N T S                                         |
  !============================================================================
    REAL(KIND=wp),PARAMETER:: REARTH = 6369.0_wp     ! radius of the Earth (km)
    REAL(KIND=wp),PARAMETER:: GMR = 34.163195_wp         ! hydrostatic constant
    INTEGER,PARAMETER:: NTAB=8       ! number of entries in the defining tables
  !============================================================================
  !     L O C A L   V A R I A B L E S                                         |
  !============================================================================
    INTEGER:: i,j,k                                                  ! counters
    REAL(KIND=wp):: h                              ! geopotential altitude (km)
    REAL(KIND=wp):: tgrad, tbase  ! temperature gradient and base temp of layer
    REAL(KIND=wp):: tlocal                                  ! local temperature
    REAL(KIND=wp):: deltah                    ! height above base of this layer
  !============================================================================
  !     L O C A L   A R R A Y S   ( 1 9 7 6   S T D.  A T M O S P H E R E )   |
  !============================================================================
    REAL(KIND=wp),DIMENSION(NTAB),PARAMETER:: htab= &
              (/0.0_wp, 11.0_wp, 20.0_wp, 32.0_wp, 47.0_wp, 51.0_wp, 71.0_wp, &
                                                                    84.852_wp/)
    REAL(KIND=wp),DIMENSION(NTAB),PARAMETER:: ttab= &
          (/288.15_wp, 216.65_wp, 216.65_wp, 228.65_wp, 270.65_wp, 270.65_wp, &
                                                        214.65_wp, 186.946_wp/)
    REAL(KIND=wp),DIMENSION(NTAB),PARAMETER:: ptab= &
                   (/1.0_wp, 2.233611E-1_wp, 5.403295E-2_wp, 8.5666784E-3_wp, &
             1.0945601E-3_wp, 6.6063531E-4_wp, 3.9046834E-5_wp, 3.68501E-6_wp/)
    REAL(KIND=wp),DIMENSION(NTAB),PARAMETER:: gtab= &
  ...
  !----------------------------------------------------------------------------
    ...
    StdAtmosphere=tlocal
  
  END FUNCTION StdAtmosphere

END SUBROUTINE check_cold_pools
