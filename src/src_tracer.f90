...
INTEGER, PARAMETER :: NMAX_META = 12
...
!------------------------------------------------------------------------------
  ...
  meta_list(1)%cosmo_str = 'DAMP_FORCED'
  meta_list(1)%cosmo_idx = -1
  meta_list(1)%messy_idx = I_DAMP

  meta_list(2)%cosmo_str = 'SP_ADV_LF'
  meta_list(2)%cosmo_idx = -1
  meta_list(2)%messy_idx = I_ADVECT

  meta_list(3)%cosmo_str = T_ADV_KEY
  meta_list(3)%cosmo_idx = T_ADV_ID
  meta_list(3)%messy_idx = I_ADVECT

  meta_list(4)%cosmo_str = T_DIFF_KEY
  meta_list(4)%cosmo_idx = T_DIFF_ID
  meta_list(4)%messy_idx = I_HDIFF

  meta_list(5)%cosmo_str = T_TURB_KEY
  meta_list(5)%cosmo_idx = T_TURB_ID
  meta_list(5)%messy_idx = I_VDIFF

  meta_list(6)%cosmo_str = T_CONV_KEY
  meta_list(6)%cosmo_idx = T_CONV_ID
  meta_list(6)%messy_idx = I_CONVECT

  meta_list(7)%cosmo_str = T_INI_KEY
  meta_list(7)%cosmo_idx = T_INI_ID
  meta_list(7)%messy_idx = I_INITIAL

  meta_list(8)%cosmo_str = T_LBC_KEY
  meta_list(8)%cosmo_idx = T_LBC_ID
  meta_list(8)%messy_idx = I_LBC

  meta_list(9)%cosmo_str = T_RELAX_KEY
  meta_list(9)%cosmo_idx = T_RELAX_ID
  meta_list(9)%messy_idx = I_RELAX

  meta_list(10)%cosmo_str = T_DAMP_KEY
  meta_list(10)%cosmo_idx = T_DAMP_ID
  meta_list(10)%messy_idx = I_DAMP

  meta_list(11)%cosmo_str = 'BD_0GRAD_FORCED'
  meta_list(11)%cosmo_idx = -1
  meta_list(11)%messy_idx = -1

  meta_list(12)%cosmo_str = 'BD_SET_FORCED'
  meta_list(12)%cosmo_idx = -1
  meta_list(12)%messy_idx = -1
#endif
...
SUBROUTINE trcr_meta_get_int_byindname(ierr,itrcr,ymeta,ydata)
  ...
  IF (ierr == 0_iintegers) THEN
    ...
    IF (imeta > 0 .AND. imeta <= MAX_CASK_I) THEN
       ...
       ELSE IF (imeta == T_DAMP_ID) THEN
          IF (ti_gp(itrcr)%tp%meta%cask_i(imeta) >= 1) THEN
             IF (TRIM(ymeta) == 'DAMP_FORCED') THEN
                ydata = ti_gp(itrcr)%tp%meta%cask_i(imeta)
             ELSE
                ydata = T_DAMP_ON
             END IF
