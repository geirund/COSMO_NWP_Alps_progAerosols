...
!  Enabled lprog_tke=.true. also for M. Raschendorfers turbulence scheme. For this,
!   added an exchange of tke(:,:,:,ntlev) and tketens.
!
! Code Description:
! Language: Fortran 90.
...
!WL2011b                               
       tt_mic, qvt_mic,qcit_mic,                                             &
       qrsgt_mic,tt_adv,tt_rad,tt_turb,qvt_turb,qcit_turb,qvt_adv,           &
       qcit_adv,qrsgt_adv,tt_hd,qvt_hd,qcit_hd, tt_tot,  crlat, hhl,         &
       qvt_tot, qcit_tot, qrsgt_tot,tt_zadv,tt_dpdt, tt_con,tt_hadv,         &
       qvt_zadv, qcit_zadv, qrsgt_zadv, tt_rlx,qvt_rlx,qcit_rlx,             &
       qrsgt_rlx, qvt_conv, qct_conv, qit_conv, qrt_conv, qst_conv,          &     
       ! and averaged budget terms
       att_mic, aqvt_mic, aqcit_mic, aqrsgt_mic,att_adv,att_rad,att_turb,    &
       aqvt_turb,aqcit_turb,aqvt_adv,                                        &
       aqcit_adv,aqrsgt_adv,att_hd,aqvt_hd,aqcit_hd, att_tot,                &
       aqvt_tot, aqcit_tot, aqrsgt_tot, att_rlx,                             &
       att_zadv, aqvt_zadv, aqcit_zadv, aqrsgt_zadv, att_con, aqvt_con,      &
       aqcit_con, aqvt_rlx, aqcit_rlx, aqrsgt_rlx, aqrsgt_con, att_ssowl,    &
       tt_ssowl, att_hadv, att_dpdt,                                         &
!WL2011e                               
       tt_relax, qvt_relax, wso_relax, att_relax, aqvt_relax, awso_relax,    &
!SBoeing, b
       tt_lsf, qvt_lsf, qcit_lsf, att_lsf, aqvt_lsf, aqcit_lsf
!SBoeing, e
...
USE data_runcontrol,     ONLY:                                               &
       ...
       l2dim, nincsn, itype_aerosol, l_2mom, itype_turb, rperturb,           &
       itype_pert,                                                           &
!WL2011b
       lbud, lbud_avg, lbud_h_theta , itheta_adv,                            &
!WL2011e
! LS, b 
       lrelax, lrelax_soil,                                                  &
! LS, e 
...
USE utilities,           ONLY:   get_utc_date, check_field_NaNs, check_cold_pools
...
USE m_perturb,             ONLY: perturb_prognostic_fields
...
!------------------------------------------------------------------------------
  ...
  IF (my_cart_id == 0) THEN
    ...
    CALL info_define ('cosmo')             ! Pre-define the program name
...
!------------------------------------------------------------------------------
  ...
  IF (.NOT. l2tls) THEN
    ...
    ELSE
      ...
      zforecasttime = zforecasttime - nzdiv * 3600_wp
    ENDIF
  ELSE
    IF (ntstep == 0) THEN
      zforecasttime = 0.0_wp
...
! AF Put l_cosmo_art to false if you don't want to calculate the ART part
    IF (l_cosmo_art_nl) THEN
      l_cosmo_art=(l_cosmo_art_nl .AND. ((ntstep*dt/3600._wp) >= artstart ))
    ENDIF

    ! Initialization of COSMO_ART
    IF (l_cosmo_art .AND. (ntstep*dt/3600._wp) == artstart) THEN
    ...
    IF (lbud) THEN
      ...
      IF ( izerror /= 0_iintegers .AND. izerror /= T_ERR_NOTFOUND )THEN
        ...
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yroutine)
      ENDIF
      CALL trcr_get(izerror, "QR", ptr_tlev=nnew, ptr=qr_new)
      IF ( izerror /= 0_iintegers .AND. izerror /= T_ERR_NOTFOUND )THEN
        yzerrmsg = trcr_errorstr(izerror)
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yroutine)
      ENDIF
      ...
      IF ( izerror /= 0_iintegers .AND. izerror /= T_ERR_NOTFOUND )THEN
        ...
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yroutine)
      ENDIF
      CALL trcr_get(izerror, "QS", ptr_tlev=nnew, ptr=qs_new)
      IF ( izerror /= 0_iintegers .AND. izerror /= T_ERR_NOTFOUND )THEN
        yzerrmsg = trcr_errorstr(izerror)
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yroutine)
      ENDIF
      ...
      IF ( izerror /= 0_iintegers .AND. izerror /= T_ERR_NOTFOUND )THEN
        ...
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yroutine)
      ENDIF
      CALL trcr_get(izerror, "QG", ptr_tlev=nnew, ptr=qg_new)
      IF ( izerror /= 0_iintegers .AND. izerror /= T_ERR_NOTFOUND )THEN
        yzerrmsg = trcr_errorstr(izerror)
        CALL model_abort(my_cart_id, izerror, yzerrmsg, yroutine)
      ENDIF
    ...
    nexch_tag = MOD (ntstep, 24*3600*10/INT(10*dt))

    CALL initialize_loop (ntstep, nbd1, nbd2, nold, nnow, nnew)

    ! add relative random perturbations to prognostic fields (only if itype_pert > 0)
    CALL perturb_prognostic_fields(ntstep,itype_pert,rperturb,nnow,izdebug)
...
!WL2011b
    IF (lbud) THEN
      qrsgt_mic (:,:,:) = 0.0_wp
      IF (ASSOCIATED(qr_now)) qrsgt_mic (:,:,:) = qrsgt_mic (:,:,:) + (qr_new(:,:,:) - qr_now(:,:,:))/dt 
      IF (ASSOCIATED(qs_now)) qrsgt_mic (:,:,:) = qrsgt_mic (:,:,:) + (qs_new(:,:,:) - qs_now(:,:,:))/dt 
      IF (ASSOCIATED(qg_now)) qrsgt_mic (:,:,:) = qrsgt_mic (:,:,:) + (qg_new(:,:,:) - qg_now(:,:,:))/dt 
    ENDIF
!WL2011e
...
#ifdef NUDGING
    IF (luseobs) THEN
    CALL organize_assimilation ('nudge', izerror, yzerrmsg)
      IF (izerror /= 0_iintegers) THEN
      CALL model_abort (my_cart_id, 100+izerror, yzerrmsg,               &
...
#ifdef NUDGING
    IF (luseobs) THEN
    CALL organize_assimilation ('lhn', izerror, yzerrmsg)
      IF (izerror /= 0_iintegers) THEN
        CALL model_abort (my_cart_id, 100+izerror, yzerrmsg,               &
...
!WL2011b
    !store current temperature to latent heating before microphysics and satad
    !-----------------------------------------------------------------------------
    IF (lbud) THEN      
      tt_mic(:,:,:)    = (t(:,:,:,nnew) - t(:,:,:,nnow)) / dt -  tt_mic(:,:,:)
      qvt_mic(:,:,:)   = (qv_new(:,:,:) - qv_now(:,:,:))/dt -  qvt_mic (:,:,:)
      qcit_mic (:,:,:) = (qc_new(:,:,:) - qc_now(:,:,:))/dt -  qcit_mic(:,:,:)
      IF(ASSOCIATED(qi_now))  qcit_mic (:,:,:) =  qcit_mic (:,:,:) + (qi_new(:,:,:) - qi_now(:,:,:))/dt
      qrsgt_mic (:,:,:) = - qrsgt_mic(:,:,:) 
      IF (ASSOCIATED(qr_now)) qrsgt_mic(:,:,:) =  qrsgt_mic(:,:,:) + (qr_new(:,:,:) - qr_now(:,:,:))/dt
      IF (ASSOCIATED(qs_now)) qrsgt_mic(:,:,:) =  qrsgt_mic(:,:,:) + (qs_new(:,:,:) - qs_now(:,:,:))/dt
      IF (ASSOCIATED(qg_now)) qrsgt_mic(:,:,:) =  qrsgt_mic(:,:,:) + (qg_new(:,:,:) - qg_now(:,:,:))/dt
...
!WL2011b
    IF (lbud) THEN      
      ...
      ! put complete local moisture tendencies to *_tot fields
      qvt_tot(:,:,:) = (qv_new(:,:,:)-qv_now(:,:,:))/dt
      qcit_tot(:,:,:) = (qc_new(:,:,:)-qc_now(:,:,:))/dt
      IF(ASSOCIATED(qi_now))  qcit_tot(:,:,:) = qcit_tot(:,:,:) + (qi_new(:,:,:)-qi_now(:,:,:))/dt
      qrsgt_tot(:,:,:) = 0.0_wp
      IF (ASSOCIATED(qr_now)) qrsgt_tot(:,:,:)= qrsgt_tot(:,:,:)+ (qr_new(:,:,:)-qr_now(:,:,:))/dt
      IF (ASSOCIATED(qs_now)) qrsgt_tot(:,:,:)= qrsgt_tot(:,:,:)+ (qs_new(:,:,:)-qs_now(:,:,:))/dt
      IF (ASSOCIATED(qg_now)) qrsgt_tot(:,:,:)= qrsgt_tot(:,:,:)+ (qg_new(:,:,:)-qg_now(:,:,:))/dt
   
 
      DO k = 1, ke
        DO j = jstart, jend
          DO i = istart, iend
            IF (lbud_h_theta) THEN
              !calculate pot. temp tendencies out of temp. tendencies
              zpi = ( 1.0E-5_wp * ( p0(i,j,k) + pp(i,j,k,nnow) ) )**rdocp
              ztheta_k = t(i,j,k,nnow) / zpi
              tt_mic(i,j,k)  = tt_mic(i,j,k) * 1.0_wp/zpi
              tt_turb(i,j,k) = tt_turb(i,j,k) * 1.0_wp/zpi
              tt_rad(i,j,k)  = tt_rad(i,j,k) * 1.0_wp/zpi
              tt_hd(i,j,k)   = tt_hd(i,j,k) * 1.0_wp/zpi
              tt_con(i,j,k)  = tt_con(i,j,k) * 1.0_wp/zpi
              tt_rlx(i,j,k)  = tt_rlx(i,j,k) * 1.0_wp/zpi
              tt_ssowl(i,j,k)= tt_ssowl(i,j,k) * 1.0_wp/zpi
              IF (lrelax) THEN
                tt_relax(i,j,k)  = tt_relax(i,j,k) * 1.0_wp/zpi
              ENDIF
              IF (llsf) THEN
                tt_lsf(i,j,k)  = tt_lsf(i,j,k) * 1.0_wp/zpi
              ENDIF
            ELSE
              ztheta_k = t(i,j,k,nnow) 
            ENDIF
            IF (lbud_h_theta) THEN
              ! calc of local pot. temp. tendency
              tt_tot(i,j,k)= (t(i,j,k,nnew) / (( 1.0E-5_wp * ( p0(i,j,k) + pp(i,j,k,nnew) ) )**rdocp)  - ztheta_k)/dt
              IF (itheta_adv==0) THEN
                ! since temperature advection is computed pot. temperatue is diagnosed (still exactly) as residuum
                tt_adv(i,j,k) = tt_tot(i,j,k)-tt_mic(i,j,k)-tt_turb(i,j,k)-tt_rad(i,j,k)-tt_hd(i,j,k)-tt_con(i,j,k)-tt_rlx(i,j,k)-tt_ssowl(i,j,k)
                IF (lrelax) THEN
                  tt_adv(i,j,k) = tt_adv(i,j,k) - tt_relax(i,j,k)
                ENDIF
                IF (llsf) THEN
                  tt_adv(i,j,k) = tt_adv(i,j,k) - tt_lsf(i,j,k)
                ENDIF
              END IF
            ELSE
              ! calc of local temp. tendency
              tt_tot(i,j,k)= (t(i,j,k,nnew) - t(i,j,k,nnow))/dt
            ENDIF

            ! With the next line (comment now) the budget equation is tested for its closure
            ! tt_dpdt is zero for lbud_h_theta=True 
            !tt_hd(i,j,k)= tt_tot(i,j,k) - tt_adv(i,j,k) - tt_mic(i,j,k)-tt_turb(i,j,k)-tt_rad(i,j,k)&
            !                -tt_dpdt(i,j,k)-tt_rlx(i,j,k) - tt_ssowl(i,j,k) - tt_con(i,j,k)

            ! some preparations
            IF (k < ke .AND. k > 1) THEN
              IF (lbud_h_theta) THEN
                ztheta_kp1 = t(i,j,k+1,nnow) / (( 1.0E-5_wp * ( p0(i,j,k+1) + pp(i,j,k+1,nnow) ) )**rdocp)
                ztheta_km1 = t(i,j,k-1,nnow) / (( 1.0E-5_wp * ( p0(i,j,k-1) + pp(i,j,k-1,nnow) ) )**rdocp)
              ELSE
                ztheta_kp1 = t(i,j,k+1,nnow)
                ztheta_km1 = t(i,j,k-1,nnow)
              END IF
              hhl_mkp1  = 0.5_wp*(hhl(i,j,k+1)+hhl(i,j,k+2))
              hhl_mkm1  = 0.5_wp*(hhl(i,j,k-1)+hhl(i,j,k))
            ELSEIF (k==ke) THEN
              IF (lbud_h_theta) THEN
                ztheta_km1 = t(i,j,k-1,nnow) / (( 1.0E-5_wp * ( p0(i,j,k-1) + pp(i,j,k-1,nnow) ) )**rdocp)
              ELSE
            ...
            ELSEIF (k==1) THEN
              ...
              hhl_mkp1  = 0.5_wp*(hhl(i,j,k+1)+hhl(i,j,k+2))
            END IF
            hhl_mk    = 0.5_wp*(hhl(i,j,k)+hhl(i,j,k+1))


            ! Calculate 2nd-order estimate of vertical advection of temperature and moisture variables

            IF (k==ke) THEN     ! one-sided at bottom
              tt_zadv(i,j,k) = 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * (ztheta_k-ztheta_km1)  / &
                               (hhl_mk-hhl_mkm1)
              qvt_zadv(i,j,k)= 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * (qv_now(i,j,k)-qv_now(i,j,k-1))  / &
                               (hhl_mk-hhl_mkm1)
              qcit_zadv(i,j,k)= 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * (qc_now(i,j,k)-qc_now(i,j,k-1))  / &
                               (hhl_mk-hhl_mkm1) 
              IF (ASSOCIATED(qi_now))  qcit_zadv(i,j,k) =  qcit_zadv(i,j,k) + 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qi_now(i,j,k)-qi_now(i,j,k-1)) / (hhl_mk-hhl_mkm1)
              qrsgt_zadv(i,j,k)=0.0_wp
              IF (ASSOCIATED(qr_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+ 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qr_now(i,j,k)-qr_now(i,j,k-1)) / (hhl_mk-hhl_mkm1)
              IF (ASSOCIATED(qs_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+ 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qs_now(i,j,k)-qs_now(i,j,k-1)) / (hhl_mk-hhl_mkm1)
              IF (ASSOCIATED(qg_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+ 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qg_now(i,j,k)-qg_now(i,j,k-1)) / (hhl_mk-hhl_mkm1)
            ELSEIF (k==1) THEN   !one-sided at top
              tt_zadv(i,j,k) = 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow) ) * (ztheta_kp1-ztheta_k) / &
                               (hhl_mkp1 - hhl_mk)
              qvt_zadv(i,j,k) = 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow) ) *(qv_now(i,j,k+1)-qv_now(i,j,k))  / &
                               (hhl_mkp1 - hhl_mk)
              qcit_zadv(i,j,k)= 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * (qc_now(i,j,k+1)-qc_now(i,j,k))  / &
                               (hhl_mkp1-hhl_mk) 
              IF (ASSOCIATED(qi_now))  qcit_zadv(i,j,k) =  qcit_zadv(i,j,k) + 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qi_now(i,j,k+1)-qi_now(i,j,k))  / (hhl_mkp1-hhl_mk)
              qrsgt_zadv(i,j,k)=0.0_wp
              IF (ASSOCIATED(qr_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+ 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qr_now(i,j,k+1)-qr_now(i,j,k))  / (hhl_mkp1-hhl_mk)
              IF (ASSOCIATED(qs_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+ 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qs_now(i,j,k+1)-qs_now(i,j,k))  / (hhl_mkp1-hhl_mk)
              IF (ASSOCIATED(qg_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+ 0.5_wp* (w(i,j,k,nnow) + w(i,j,k+1,nnow)) * &
                                                          (qg_now(i,j,k+1)-qg_now(i,j,k))  / (hhl_mkp1-hhl_mk)
            ELSE  ! centered differences elsewhere
              tt_zadv(i,j,k) = 0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(ztheta_kp1+ztheta_k)- &
                               0.5_wp*(ztheta_km1+ztheta_k))/ (hhl(i,j,k+1)-hhl(i,j,k))
              qvt_zadv(i,j,k) = 0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(qv_now(i,j,k+1)+qv_now(i,j,k))- &
                                0.5_wp*(qv_now(i,j,k-1)+qv_now(i,j,k)))/ (hhl(i,j,k+1)-hhl(i,j,k))
              qcit_zadv(i,j,k)= 0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(qc_now(i,j,k+1)+qc_now(i,j,k))- &
                                0.5_wp*(qc_now(i,j,k-1)+qc_now(i,j,k)))/ (hhl(i,j,k+1)-hhl(i,j,k))
              IF (ASSOCIATED(qi_now))  qcit_zadv(i,j,k) =  qcit_zadv(i,j,k) +                                             &
                                0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(qi_now(i,j,k+1)+qi_now(i,j,k))- &
                                0.5_wp*(qi_now(i,j,k-1)+qi_now(i,j,k)))/ (hhl(i,j,k+1)-hhl(i,j,k))
              qrsgt_zadv(i,j,k)= 0.0_wp
              IF (ASSOCIATED(qr_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+                                             &
                                0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(qr_now(i,j,k+1)+qr_now(i,j,k))- &
                                0.5_wp*(qr_now(i,j,k-1)+qr_now(i,j,k)))/ (hhl(i,j,k+1)-hhl(i,j,k))
              IF (ASSOCIATED(qs_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+                                             &
                                0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(qs_now(i,j,k+1)+qs_now(i,j,k))- &
                                0.5_wp*(qs_now(i,j,k-1)+qs_now(i,j,k)))/ (hhl(i,j,k+1)-hhl(i,j,k))
              IF (ASSOCIATED(qg_now))  qrsgt_zadv(i,j,k)=  qrsgt_zadv(i,j,k)+                                             &
                                0.5_wp*(w(i,j,k,nnow)+w(i,j,k+1,nnow)) * (0.5_wp*(qg_now(i,j,k+1)+qg_now(i,j,k))- &
                                0.5_wp*(qg_now(i,j,k-1)+qg_now(i,j,k)))/ (hhl(i,j,k+1)-hhl(i,j,k))
            END IF
  
            !put calculated vertical advection on right hand side of budget equation
            tt_zadv(i,j,k)   = - tt_zadv(i,j,k)
            qvt_zadv(i,j,k)  = - qvt_zadv(i,j,k)
            qcit_zadv(i,j,k) = - qcit_zadv(i,j,k)
            qrsgt_zadv(i,j,k)= - qrsgt_zadv(i,j,k)

            IF (itheta_adv==0.AND.lbud_h_theta) THEN
              !Calculate horizontal advection term as a residual
              tt_hadv(i,j,k) = tt_adv(i,j,k) - tt_zadv(i,j,k)
            ENDIF

            ! For lbud_h_theta=.false., convert pot. temp advection tendencies
            ! to scaled temperature tendencies
            IF ((.NOT.lbud_h_theta) .AND. (itheta_adv==1.OR.itheta_adv==2)) THEN
               zpi = ( 1.0E-5_wp * ( p0(i,j,k) + pp(i,j,k,nnow) ) )**rdocp
               tt_hadv(i,j,k) = tt_hadv(i,j,k)*zpi
               tt_adv(i,j,k) = tt_adv(i,j,k)*zpi
               tt_zadv(i,j,k) = tt_zadv(i,j,k)*zpi
            ENDIF
    
            ! Calculate sums of budget terms,
            ! if average values are needed; averaging is done at other location
            IF (lbud_avg) THEN
               att_tot(i,j,k)    = att_tot(i,j,k)    +  tt_tot(i,j,k)
               att_dpdt(i,j,k)   = att_dpdt(i,j,k)   +  tt_dpdt(i,j,k)
               att_hd(i,j,k)     = att_hd(i,j,k)     +  tt_hd(i,j,k)
               att_rad(i,j,k)    = att_rad(i,j,k)    +  tt_rad(i,j,k)
               att_turb(i,j,k)   = att_turb(i,j,k)   +  tt_turb(i,j,k)
               att_con(i,j,k)    = att_con(i,j,k)    +  tt_con(i,j,k)
               att_mic(i,j,k)    = att_mic(i,j,k)    +  tt_mic(i,j,k)
               att_adv(i,j,k)    = att_adv(i,j,k)    +  tt_adv(i,j,k)
               att_rlx(i,j,k)    = att_rlx(i,j,k)    +  tt_rlx(i,j,k)
               att_zadv(i,j,k)   = att_zadv(i,j,k)   +  tt_zadv(i,j,k)
               att_hadv(i,j,k)   = att_hadv(i,j,k)   +  tt_hadv(i,j,k)
               att_ssowl(i,j,k)  = att_ssowl(i,j,k)  +  tt_ssowl(i,j,k)
               aqvt_tot(i,j,k)   = aqvt_tot(i,j,k)   +  qvt_tot(i,j,k)
               aqvt_con(i,j,k)   = aqvt_con(i,j,k)   +  qvt_conv(i,j,k)
               aqvt_adv(i,j,k)   = aqvt_adv(i,j,k)   +  qvt_adv(i,j,k)
               aqvt_rlx(i,j,k)   = aqvt_rlx(i,j,k)   +  qvt_rlx(i,j,k)
               aqvt_mic(i,j,k)   = aqvt_mic(i,j,k)   +  qvt_mic(i,j,k)
               aqvt_turb(i,j,k)  = aqvt_turb(i,j,k)  +  qvt_turb(i,j,k)
               aqvt_hd(i,j,k)    = aqvt_hd(i,j,k)    +  qvt_hd(i,j,k)
               aqvt_zadv(i,j,k)  = aqvt_zadv(i,j,k)  +  qvt_zadv(i,j,k)
               aqcit_tot(i,j,k)  = aqcit_tot(i,j,k)  +  qcit_tot(i,j,k)
               aqcit_con(i,j,k)  = aqcit_con(i,j,k)  +  qct_conv(i,j,k) + qit_conv(i,j,k)
               aqcit_adv(i,j,k)  = aqcit_adv(i,j,k)  +  qcit_adv(i,j,k)
               aqcit_rlx(i,j,k)  = aqcit_rlx(i,j,k)  +  qcit_rlx(i,j,k)
               aqcit_mic(i,j,k)  = aqcit_mic(i,j,k)  +  qcit_mic(i,j,k)
               aqcit_turb(i,j,k) = aqcit_turb(i,j,k) +  qcit_turb(i,j,k)
               aqcit_zadv(i,j,k) = aqcit_zadv(i,j,k) +  qcit_zadv(i,j,k)
               aqrsgt_tot(i,j,k)  = aqrsgt_tot(i,j,k)  +  qrsgt_tot(i,j,k)
               aqrsgt_adv(i,j,k)  = aqrsgt_adv(i,j,k)  +  qrsgt_adv(i,j,k)
               aqrsgt_rlx(i,j,k)  = aqrsgt_rlx(i,j,k)  +  qrsgt_rlx(i,j,k)
               aqrsgt_mic(i,j,k)  = aqrsgt_mic(i,j,k)  +  qrsgt_mic(i,j,k)
               aqrsgt_zadv(i,j,k) = aqrsgt_zadv(i,j,k) +  qrsgt_zadv(i,j,k)
               aqrsgt_con(i,j,k)  = aqrsgt_con(i,j,k)  +  qrt_conv(i,j,k) + qst_conv(i,j,k)
            ENDIF
      ...
      IF (lbud_avg) THEN
        DO j = jstart, jend
          DO i = istart, iend
            IF (lrelax) THEN
              DO k = 1, ke
                att_relax(i,j,k) = att_relax(i,j,k)  + tt_relax(i,j,k)
                aqvt_relax(i,j,k)= aqvt_relax(i,j,k) + qvt_relax(i,j,k)
              ENDDO
            ENDIF
            IF (lrelax_soil) THEN
              DO kso = 1, ke_soil
                awso_relax(i,j,kso)= awso_relax(i,j,kso) + wso_relax(i,j,kso)
              ENDDO
            ENDIF
            IF (llsf) THEN
              DO k = 1, ke
                att_lsf(i,j,k) = att_lsf(i,j,k)  + tt_lsf(i,j,k)
                aqvt_lsf(i,j,k)= aqvt_lsf(i,j,k) + qvt_lsf(i,j,k)
                aqcit_lsf(i,j,k)= aqcit_lsf(i,j,k) + qcit_lsf(i,j,k)
              ENDDO
            ENDIF
          ENDDO
        ENDDO
...
!-------------------------------------------------------------------------------
!  Section 6: Check for cold pools
!-------------------------------------------------------------------------------o

  call check_cold_pools(t(:,:,:,nnow), hhl(:,:,:), ntstep, my_cart_id, nboundlines)
