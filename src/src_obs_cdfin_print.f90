  ...
  IF (num_compute > 1) THEN
    ...
    DO ima = 1 , n_cma
      DO jevt = nfrsev , nlasev
        inevent ((ima-1)*nevc+jevt-nfrsev+1)  =  nevent(jevt,ima,1)
      ENDDO
    ENDDO
...
!   ==================
    ...
    IF (my_cart_id == 0) THEN
      DO ima = 1 , n_cma
        DO jevt = nfrsev , nlasev
          nevent (jevt,ima,1)  =  inevent((ima-1)*nevc+jevt-nfrsev+1)
        ENDDO
      ENDDO
    ENDIF
...
! observation types
      ...
      ELSE
        WRITE( nustat, y1900   )  cma(ima)%name                                &
                               , (nevent(jevt,ima,1), jevt=nfrsev,nlasev)
      ENDIF
    ENDDO
...
! -------------
  ...
  REAL (KIND=wp)           :: &
    ...
    zpp    , zzz     ,& ! observed pressure and height
...
! -------------------------
  ...
  INTEGER (KIND=iintegers) :: &
    ialllen (num_compute) ,& ! number of buffer elements from each node
    iallpos (num_compute)    ! displacement of first elements from each node
...
! -------------
  ...
  REAL (KIND=wp)           :: &
    ...
    zobtim           ,& ! observation time in forecast hour units
    zuu  , zvv       ,& ! observed wind components
    ztt  , zrh       ,& ! observed temperature and rel.humidity
    zpp  , zzz          ! observed pressure and height
