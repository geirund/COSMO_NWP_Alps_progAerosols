...
! Global Parameters
  INTEGER (KIND=iintegers), PARAMETER     ::                       &
    khmax = 299         ! max. number of vertical coordinate parameters
                        ! according to grib.
...
REAL (KIND=wp)               ::    &
  rundefined = -9999.00_wp ! a default value that indicates that a 
...
IF (.NOT. ASSOCIATED(vcoord%vert_coord)) THEN
   ALLOCATE(vcoord%vert_coord(khmax))
   vcoord%vert_coord = 0._wp
ENDIF

IF (.NOT. ASSOCIATED(vcoord%sigm_coord)) THEN
   ALLOCATE(vcoord%sigm_coord(khmax))
   vcoord%sigm_coord = 0._wp
ENDIF
...
REAL (KIND=wp)             :: zgdrt, ztdbe, zbetf, zpxx, zp0sl, zt0sl, zdt0lp, zn
...
!   ENDDO
  ...
  CASE ( 2, 3, 4 )
    ...
    ELSEIF ( ANY( vc_type%ivctype == (/3,4/) ) ) THEN
  ...
  IF ( ANY( hhl(:,:,1:ke) - hhl(:,:,2:ke+1) <= 0.0_wp ) ) THEN
    WRITE(*,*) "ERROR: negative dz encountered in reference_atmosphere!"
    STOP
  END IF
...
INTEGER (KIND=iintegers)   :: k
REAL (KIND=wp)             :: zpxx, zt00, zp0sl, zt0sl, zdelta_t, zh_scal, zn
REAL (KIND=wp)             :: zak(ke+1), zbk(ke+1), zbk2(ke+1), zhfl(ie,je)
...
INTEGER (KIND=iintegers)   :: k
REAL (KIND=wp)             :: zgdrt, ztdbe, zbetf, zpxx, zth00, zp00, zc_p,   &
                              zp0sl, zt0sl, zthsl, zbvref, zg2_o_N2cp, zn
REAL (KIND=wp)             :: zak(ke+1), zbk(ke+1), zbk2(ke+1), zhfl(ie,je)
