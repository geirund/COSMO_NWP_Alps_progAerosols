...
!AP>> add debug streams 1:5 for use as needed
    debug1(:,:,:), &
    debug2(:,:,:), &
    debug3(:,:,:), &
    debug4(:,:,:), &
...
! fields of the turbulent scheme defined on half-levels:
   ...
   tketens(:,:,:),  & ! non-advective tke-tendency (half-levels)      ( m/s )
   tket_adv(:,:,:), & ! pure advective tke-tendency (half-levels)     ( m/s )
...
!  4b. WL 2011 tendencies for heat and moisture budget diagnosis
! -----------------------------------------------
  REAL  (KIND=wp), TARGET, ALLOCATABLE ::           &
                       !  heating tendencies due to:
     ...
     tt_ssowl(:,:,:), & !  sso, might pot. temp. tend.               ( K/s )
     ...
     tt_hadv(:,:,:),  &!  estimate for vertical advection            ( K/s )
                       !  vapor tendencies:
     qvt_tot(:,:,:),  &!                                             ( 1/s )
     qvt_mic(:,:,:),  &!                                             ( 1/s ) 
     qvt_adv(:,:,:),  &!                                             ( 1/s )
...
!LS, b
! 4b. tendency fields due to relaxation                               (unit )
    ttens_tot(:),   & ! field of temperature tendency due to relaxation ( K/s )
    qvtens_tot(:),  & ! field of qv tendency due to relaxation ( 1/s )
    utens_tot(:),   & ! field of u tendency due to relaxation ( m/s2 )
    vtens_tot(:),   & ! field of v tendency due to relaxation ( m/s2 )
!LS, e
...
! SBoeing, b
    ...
    qvtens_lsf(:)       ! qt-tendency lsf          ( 1/s )
! SBoeing, e

!  4c. WL 2011 averaged tendencies for heat and moisture budget diagnosis
! -----------------------------------------------
  REAL  (KIND=wp), TARGET, ALLOCATABLE ::           &
                       !  heating tendencies due to:    
     ...
     att_ssowl(:,:,:), & !  sso, might pot. temp. tend.               ( K/s )
     ...
     att_hadv(:,:,:),  &!  estimate for vertical advection            ( K/s )
                       !  vapor tendencies:
     aqvt_tot(:,:,:),  &!                                             ( 1/s )
     aqvt_mic(:,:,:),  &!                                             ( 1/s )
     aqvt_adv(:,:,:),  &!                                             ( 1/s )
    ...
    !SBoeing, b
    tt_lsf(:,:,:),& !  temperature tendency due to large scale forcing  ( K/s )
    qvt_lsf(:,:,:),& !  humidity tendency due to large scale forcing     ( 1/s )
    qcit_lsf(:,:,:),& !  humidity tendency due to large scale forcing     ( 1/s )
    ut_lsf(:,:,:), & !  u tendency due to large scale forcing            ( m/s2 )
    vt_lsf(:,:,:), & !  v tendency due to large scale forcing            ( m/s2 )
    att_lsf(:,:,:),& !  avg temperature tendency due to large scale forcing  ( K/s )
    aqvt_lsf(:,:,:),&!  avg humidity tendency due to large scale forcing ( 1/s )
    aqcit_lsf(:,:,:),&!  avg humidity tendency due to large scale forcing ( 1/s )
    aut_lsf(:,:,:),& !  avg u tendency due to large scale forcing        ( m/s2 )
    avt_lsf(:,:,:) !  avg v tendency due to large scale forcing        ( m/s2 )
    !Sboeing, e
...
!  4c. micorphysical process rates
  REAL  (KIND=wp), TARGET, ALLOCATABLE ::           &
    ...
    cmp_melt_qi_qr(:,:,:)   , & ! riming on ice particles
    cmp_melt_qs_qr(:,:,:)   , & ! riming on snow particles
...
!   fields from the radiation scheme
    ...
    sotrc  (:,:,:), & ! solar transmissivity at top and bottom
