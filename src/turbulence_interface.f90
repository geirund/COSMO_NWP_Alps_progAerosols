...
!WL2011b 
    tt_turb, ttens, qvt_turb, qcit_turb
!WL20211e
...
USE src_turbulence, ONLY: &
    parturs, partura, horizontal_diffcoeffs, prankolmo_rk, smag_horizontal

!(WL;2010)b
USE src_slow_tendencies_rk,   ONLY:  calc_deformation
!(WL;2010)e
!-------------------------------------------------------------------------------
...
!WL2011b calculation of turbulent heat tendency; before - after principle
    IF (lbud) THEN
      tt_turb(:,:,:) =  ttens(:,:,:)
      qvt_turb(:,:,:)=  qv_tens(:,:,:)
      qcit_turb(:,:,:)= qc_tens(:,:,:)
...
!US    CALL turbdiff(nvar,zdt,dt,lstfnct)
    ...
    ! Horizontal coefficients will always be computed using Smagorinsky closure
    !IF ( l3dturb ) CALL horizontal_diffcoeffs
    IF ( l3dturb ) THEN
        CALL calc_deformation
        CALL smag_horizontal
     END IF
    !(WL;2010)b
!WL2011b calculation of turbulent heat tendency; before - after principle
    IF (lbud) THEN
...
!WL2011e
  ...
  !(WL;2010)b
  CASE( 11 ) ! use a Smagorinsky-Lilly LES scheme
    IF (izdebug > 5) THEN
    PRINT *, '      TURBULENCE with itype_turb  = ', &
                                    itype_turb
    ENDIF
