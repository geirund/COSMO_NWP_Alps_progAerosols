...
!  - bugfix in lhn_sumrad: change 12._wp by real(ndata), which is the number
...
!      KIND=wp and KIND=irealgrib. In the former version a observation point was added
!      if "ds_rad(ind) > 0.0_irealgrib".
!      In the new version the observation is already converted to wp (SR lhn_obs_read), therefore the 
!      condition is changed to "datafield_all(i,j,n) >= 0.0_wp"
!    - The former normalisation term "ndata" is now space dependend, which better accounted for data outage of
!      single radar stations (ie. "datafield_all(i,j,n) < 0.0_wp").
...
! first for obs_read_dx
  ...
  REAL  (KIND=wp)                  ::           &
    ...
    radmax=200._wp        ,& ! maximal radius of data coverage around radar ( km )
    sec_per_hr=3600._wp   ,& ! seconds per hour
...
!-------------------------------------------------------------------------------
   ...
   DO     k=1,ke
     DO   j=jstart,jend
       DO i=istart,iend
          if (lhn_bright .AND. NINT(brightband(i,j)) > 0_iintegers) tinc_lhn(i,j,k) = 0.0_wp
...
!-------------------------------------------------------------------------------
  ...
  IF (lhn_dt_obs <= 10.0) THEN
     zlhigh_freq_data = .TRUE.
  ELSE
     zlhigh_freq_data = .FALSE.
  ENDIF

  tnow  = ntstep*dt/60._wp
...
!                 spqual_tot(:,:) = missval
           ...
           min_rad=min_rad+NINT(lhn_dt_obs,iintegers)
           IF (min_rad >= 60) THEN

             CALL close_file( nulhnrad, yform_read, 0, 0, 1, .FALSE., .FALSE., idbg_level, yerrmsg, ierr)
             IF (ierr /= 0) THEN
               WRITE(nulhn,*) 'Something wrong by closing radar file ('//yform_read//')'
             ENDIF
           ...
           IF (field_num > 1) &
               obs_time(field_num)=obs_time(field_num-1)+INT(lhn_dt_obs,iintegers)
        ENDIF ! my_cart_id == 0
        ...
        IF (num_compute > 1) THEN
           IF (my_cart_id == 0) THEN
              realbuf(1) = REAL(obs_time(field_num),wp)
           ENDIF

           CALL distribute_values (realbuf,1,0,imp_reals,icomm_cart,ierr)

           IF (my_cart_id /= 0) THEN
              obs_time(field_num) = NINT(realbuf(1),iintegers)
           ENDIF
        ENDIF
...
! determine current model time as minutes since start of forecast
  ...
  IF (.NOT.lfirst .AND. (tnow >= next_obs_time)) THEN
     ...
     IF (my_cart_id == 0) THEN
        ...
        IF (zlhigh_freq_data) THEN
           obs_time_s=(INT(tnow+3.0_wp*lhn_dt_obs)) 
        ELSE
  ...
  IF (lhn_obs_read_status) THEN
     ...
     DO i=1,iread
        IF (((tnow-obs_time(i)) >= 0_iintegers)&
           ...
           next_time_1=center_time+1_iintegers*NINT(lhn_dt_obs)
           IF (iread > 2) THEN
            next_time_2=center_time+2_iintegers*NINT(lhn_dt_obs)
            next_time_3=center_time+3_iintegers*NINT(lhn_dt_obs)
            prev_time_1=center_time-1_iintegers*NINT(lhn_dt_obs)
            prev_time_2=center_time-2_iintegers*NINT(lhn_dt_obs)
           ENDIF
           EXIT
        ENDIF
     ENDDO
...
!  - for yform_read='apix': the preprocessing job prepares a filename with '.grib'  )
  ...
  IF (.NOT. lzexist1) THEN
    ...
    IF (.NOT. lzexist2) THEN
      PRINT *,  ' *** ERROR LHN: no radar input file available:  ', TRIM(radar_in)//yobs_date(3:10)//'.grib_'
      WRITE(nulhn, *) ' *** ERROR LHN: no radar input file available:  ', TRIM(radar_in)//yobs_date(3:10)//'.grib_'
...
!      CALL model_abort(my_cart_id,6010,yerrmsg,yroutine)
  ...
  ! open file in case it exists (otherwise nulhnrad retains its default value
  ! of zero and be ignored by the lhn_obs_read() method )
  IF (lzexist1 .OR. lzexist2) THEN
...
! if this part is reached, the file ypath exists and can be opened (whether .grib1 or .grib)
    ...
    CALL open_file( nulhnrad, ypath, 'r  ', yform_read, 0, 0, 1, .FALSE., .FALSE., idbg_level, yerrmsg, ierr )
    IF (ierr /= 0) THEN
...
!US    this is only a very crude first guess: this should be refined soon
         ...
         IF     (TRIM(yzshortname) == 'TOT_PREC') THEN
           ivar(nread) = 61
           itab(nread) =  2
         ELSEIF (TRIM(yzshortname) == 'PREC_GSP') THEN
           ivar(nread) = 62
           itab(nread) =  2
         ELSEIF (TRIM(yzshortname) == 'PREC_CON') THEN
           ivar(nread) = 63
           itab(nread) =  2
         ELSEIF (TRIM(yzshortname) == 'HHL') THEN
           ivar(nread) =  8
           itab(nread) =  2
         ELSE
...
!-------------------------------------------------------------------------------
  ...
  IF (nsum > 0_iintegers ) datasum=datasum/real(nsum,wp)
...
! open GRIB file
  ...
  CALL open_file( nulhnblackdx, path, 'r  ', yform_read, 0, 0, 1, .FALSE., .FALSE., idbg_level, yerrmsg, ierr )
...
!US      this is only a very crude first guess: this should be refined soon
         CALL grib_get (igribid, 'shortName',            yzshortname, ierr)
         IF     (TRIM(yzshortname) == 'PREC_CON') THEN
           iee = 63
         ENDIF
       ENDIF
  ...
  CALL close_file( nulhnblackdx, yform_read, 0, 0, 1, .FALSE., .FALSE., idbg_level, yerrmsg, ierr)
  IF (ierr /= 0) THEN
...
! open GRIB file
  ...
  CALL open_file( nulhnh, path, 'r  ', yform_read, 0, 0, 1, .FALSE., .FALSE., idbg_level, yerrmsg, ierr )
...
! (input product definition section)
  ...
  CALL close_file( nulhnh, yform_read, 0, 0, 1, .FALSE., .FALSE., idbg_level, yerrmsg, ierr)
  IF (ierr /= 0) THEN
...
!-------------------------------------------------------------------------------
  yroutine = 'detect_bright_band'
     ...
     CALL calhzero( hzero_mod(:,:), t(:,:,:,nnow_nold), hhl, vcoord%vert_coord, &
                    ie, je, ke, t0_melt)
     minheight(:,:) = 9999.9_wp
     maxheight(:,:) = -9999.9_wp
     ...
     DO i=istart,iend
        DO j=istart,jend
           ...
           IF(NINT(blacklist(i,j)) /= 1_iintegers) THEN
              ...
              ENDDO
              height_interval(i,j)=(maxheight(i,j)-minheight(i,j))/1000._wp
              DO nh=1,nradar
...
! for check only, delete later
  ...
  REAL (KIND=wp)                             ::       &
    eps=0.2_wp/3600._wp ,& ! limit used for profile filterering (flags/
...
!!sk20060818          treat_diag(i,j)=6_wp
!!sk20060818       ENDIF
...
! exchange the profiles
     ALLOCATE ( tt_lh_prof(ke,n_fetch) , STAT=izlocstat ) ; tt_lh_prof = 0._wp
     IF (izlocstat /= 0) THEN
        yerrmsg =' ERROR  *** Allocation of arrays for profile request failed!'
...
! test if difference in grid point height is not too large
            IF ( ABS(topo - topo_tot(i,j)) > 200._wp ) THEN
               ltopo = .TRUE.
               CYCLE
            ENDIF
...
! test if precipitation and tt_lheat_int is high enough
            IF ( prmod(i,j) > (1.0_wp - pr_eps) * prclim_tot(i,j) .AND. &
              ...
              IF ( prana >= (fac_lhn_down*prmod(i,j)) .AND. &
                 ...
                 IF ( ratio > 1._wp ) ratio = 1._wp/ratio
...
!    eliminate oscillations around zero between adjacent levels
     ...
     DO k=klow-1,kup+1,-1
        IF ( (prof(k-1)*prof(k)) <= 0._wp .AND.   &
             (prof(k)*prof(k+1)) <= 0._wp         ) THEN
           lflag(k) = .TRUE.
           nelimosc = nelimosc + 1
        ENDIF
     ENDDO
     WHERE ( lflag ) proffilt = 0._wp
