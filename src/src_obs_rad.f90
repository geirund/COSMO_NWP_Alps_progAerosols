...
!-------------------------------------------------------------------------------
    ...
    DO irec = 1,radv%n_rec
      ...
      IF (istat /= 0) THEN
        ifcaststep(irec) = -1
      ELSE  
        ifcaststep(irec) = int(real(idiffm,wp) * 60.0_wp / dt)
      ENDIF
    ENDDO
...
!-------------------------------------------------------------------------------
      ...
      DO iinstr = 1, rs%n_instr
        ...
        emiss(1:ncalcs)   = 0.0_wp
        emiss_k(1:ncalcs) = 0.0_wp
        ...
        IF (istat == 0) THEN
          DO iobs=ipstart,ipend
            SELECT CASE(fdbk_height_method)
              CASE (1)
                obs_rad(iradv)%plevel(icstart:icend,iobs) = 100.0 * &
              ...
              CASE (2)
                ...
                ELSE
                  obs_rad(iradv)%plevel(icstart:icend,iobs) = 100.0_wp * &
            ...
            WHERE (MAXLOC(ABS(temp_k(:,1:rs%n_ch_i(iinstr),iobs)),1) == 1)
              obs_rad(iradv)%plevel(icstart:icend,iobs) = 0.000001_wp
            END WHERE
          END DO
        ELSE
...
!      report_body(:,:)%check   = r%check(:,:)
      ...
      ELSEWHERE (r%h_fg(isurfsens,:) > 1000.0)
  ...
  !-----------------------------------------------------------------------------
      CALL write_report(fdbk(iradv), reports, inobs, inobs*rs%n_chan,    &
                    ...
                    imdi, REAL(rmdich,wp), istat,ymsg)
...
!-------------------------------------------------------------------------------
  ...
  IF ( extrp_type /= extrp_const) THEN
    ...
    avg_p_1 = avg_p_1 / REAL(SIZE(iob_loc),wp)
    avg_p_2 = avg_p_2 / REAL(SIZE(iob_loc),wp)
  ...
  DO k = 1,n_add
    DO iprof = 1,nprof
      ...
      ELSEIF (extrp_type == extrp_clim) THEN
        temp(k,iprof) = temp(n_add + 1, iprof) + (n_add - k + 1) * &
                        (t_top - temp(n_add+1, iprof)) / REAL(n_add,wp)
        
        humidity(k,iprof) = humidity(n_add + 1, iprof) + (n_add - k + 1) * &
                            (q_top - humidity(n_add+1, iprof)) / REAL(n_add,wp)
      ELSE                      
...
!CDIR COLLAPSE  
    ...
    DO k=1, ke
      DO iprof = 1,nprof
        ...
        IF (qsh > 1.0E-7_wp) THEN
          w = 1.0_wp
        ELSE
          w = clc_sgs(i,j,k)
        ENDIF

        IF (clw_con(i,j,k) > 0._wp) THEN
          w_con = clc_con(i,j,k) * (1.0_wp - w)
        ELSE
          w_con = 0._wp
        ENDIF

        clch = MAX(0.0_wp, MIN(1.0_wp,w + w_con))

        IF (clch > 0._wp) THEN
          ! Liquid Wate content
          clwh = qc(i,j,k) * 1000._wp * rho(i,j,k) 

          ! Convective Clouds
          IF (lcon_clw .AND. clc_con(i,j,k) > 0._wp) THEN
            clw_conh = clw_con(i,j,k) * 1000._wp * rho(i,j,k)
          ELSE
            clw_conh = 0._wp 
          ENDIF  

          ! Ice content
          IF(qi(i,j,k) > 1.E-7_wp) THEN
            qih = (qi(i,j,k) + qsh) * 1000._wp * rho(i,j,k)
          ELSE
            qih = (            qsh) * 1000._wp * rho(i,j,k)
          ENDIF  
          ...
          IF (.false.) THEN
#endif          
          ELSEIF ( (clwh + clw_conh) <= 0._wp .AND. qih > 0._wp) THEN
            cloud(6,k+n_add,iprof) = qih
            cfrac(6,k+n_add,iprof) = clch
          ELSEIF ( (clwh + qih) <= 0._wp   .AND. clw_conh > 0._wp) THEN
            cloud(3,k+n_add,iprof) = clw_conh
            cfrac(3,k+n_add,iprof) = clch
          ELSEIF ( (clwh + qih + clw_conh) > 0._wp) THEN
