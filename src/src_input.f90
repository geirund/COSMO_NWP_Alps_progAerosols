...
!------------------------------------------------------------------------------
  ...
  IF (.NOT. lopen_file) THEN
    ...
    CALL open_file (nufile, yname, ymode_read, yzformat_tmp, icomm_cart,    &
                    my_cart_id, num_compute, lasync_io, llockfiles,         &
                    idbg_level, yerrmsg, ierr)
    IF (ierr /= 0) THEN
...
!       ENDIF
  ...
  !----------------------------------------------------------------------------
          ...
          IF (izexed == 1) THEN
            ...
            search_refatm1: DO n = 1, imax_refatmtype
              IF     (refatm%irefatm == 1) THEN
                IF ( (refatm%irefatm == refatm_defaults(n)%irefatm)            .AND.   &
                     (ABS(refatm%p0sl    - refatm_defaults(n)%p0sl   ) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%t0sl    - refatm_defaults(n)%t0sl   ) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%dt0lp   - refatm_defaults(n)%dt0lp  ) < 1E-3_wp) ) THEN
                  refatm%irefatm_id = n
              ...
              ELSEIF (refatm%irefatm == 2) THEN
                IF ( (refatm%irefatm == refatm_defaults(n)%irefatm)            .AND.   &
                     (ABS(refatm%p0sl    - refatm_defaults(n)%p0sl   ) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%t0sl    - refatm_defaults(n)%t0sl   ) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%delta_t - refatm_defaults(n)%delta_t) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%h_scal  - refatm_defaults(n)%h_scal ) < 1E-3_wp) ) THEN
                  refatm%irefatm_id = n
              ...
              ELSEIF (refatm%irefatm == 3) THEN
                IF ( (refatm%irefatm == refatm_defaults(n)%irefatm)            .AND.   &
                     (ABS(refatm%p0sl    - refatm_defaults(n)%p0sl   ) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%t0sl    - refatm_defaults(n)%t0sl   ) < 1E-3_wp)    .AND.   &
                     (ABS(refatm%bvref   - refatm_defaults(n)%bvref  ) < 1E-3_wp) ) THEN
                  refatm%irefatm_id = n
                  refatm%dt0lp      = rundefined
                  refatm%delta_t    = rundefined
                  refatm%h_scal     = rundefined
            ...
            search_vcoord1: DO n = 1, imax_vcoordtype
              IF ( (vcoord%nlevels    == vcoord_defaults(n)%nlevels)    .AND.   &
                   ...
                   ((vcoord%vcflat     - vcoord_defaults(n)%vcflat) < 0.001_wp) ) THEN
                ...
                IF     (vcoord%ivctype == 1) THEN
                  ...
                  IF (ALL (ABS(vcoord%sigm_coord(1:ke+1) -                             &
                               vcoord_defaults(n)%sigm_coord(1:ke+1)) < 0.01_wp)) THEN
                ...
                ELSEIF ( ANY( vcoord%ivctype == (/2,3,4/) ) ) THEN
                  ...
                  IF (ALL (ABS(vcoord%vert_coord(1:ke+1) -                             &
                               vcoord_defaults(n)%vert_coord(1:ke+1)) < 0.01_wp)) THEN
                    ! set the ID of vcoord_out to the ID of the default set
                    vcoord%ivcoord_id = vcoord_defaults(n)%ivcoord_id
                    lgot_vparam = .TRUE.
                    EXIT search_vcoord1
    ...
    IF ((lgot_vcoord) .AND. (.NOT. lgot_refatm) .AND. (izexed == 2) ) THEN
  ...
  !----------------------------------------------------------------------------
    ...
    IF ( (yzformat_tmp == 'grb1') .OR. (yzformat_tmp == 'apix') .OR. (yzformat_tmp == 'bina') ) THEN
      IF (ydata == 'boundary') THEN
        ...
        !rsecleft = (zhour - REAL(ihour, wp)) * 3600.0_wp
...
!------------------------------------------------------------------------------
  ...
  CALL close_file (nufile, yzformat_tmp, icomm_cart, my_cart_id,     &
                   num_compute, lasync_io, llockfiles, idbg_level,   &
                   yerrmsg, ierr)
  IF (ierr /= 0) THEN
...
!------------------------------------------------------------------------------
  ...
  IF     (igrbednr == 1) THEN
    ...
    CASE(211)
      ...
      END IF
    CASE(111,112)
    IF ( ((itabtyp==201).AND. ( (iee==197) .OR. (iee==198) .OR. (iee==199).OR.(iee==201))).OR.((itabtyp==250).AND.(iee==36)) ) THEN!LS, zusätzliche Variablen eingefügt
...
REAL     (KIND=wp)        :: rfact, rdepth1, rdepth2
    ...
    ELSE
      zmy(iz_is:iz_ie,iz_js:iz_je) =                                  &
        SQRT( rmy(iz_is:iz_ie,iz_js:iz_je,izrmy) * zdtrddt /          &
              ( 1.0_wp+rmy(iz_is:iz_ie,iz_js:iz_je,izrmy) * zdtrddt ) )
    ENDIF
...
! Get longitude and latitude data
  ...
  zdlat_in   = (latitude(jedim_in) - latitude(1))/ REAL(jedim_in-1,wp)
  IF (longitude(iedim_in) - longitude(1) < 0.0_wp) THEN
    ...
    zdlon_in   = (longitude(iedim_in) - longitude(1) + 360.0_wp) / (iedim_in-1)
  ELSE
...
!US        .AND. .NOT. lyear_360) .OR. (calendar(1:7) == '360_day'  .AND. lyear_360))) THEN
  ...
  istatus = nf90_get_att (ncid, jtimeID, 'units', ydate1)
    ...
    ! check the date for which the product is valid
      ...
      iref_act = iref_act * 60 + isec - isec_ref
