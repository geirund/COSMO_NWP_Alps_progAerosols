...
USE data_parameters,    ONLY:  wp, iintegers, repsilon
...
USE data_fields,        ONLY:                                        &
       ...
       sobcs, sobct, thbcs, thbct, asobc_s, asobc_t, athbc_s, athbc_t, &
...
!------------------------------------------------------------------------------
  ...
  IF (.NOT.llm .AND. itype_synd == 2  &
               .AND. ( (lphys .EQV. .FALSE.) .OR. (itype_tran .NE. 2) )) THEN
  ...
  IF (l2tls .OR. (ntstep > 0)) THEN
    ...
    DO j = jstartpar, jendpar
      DO i = istartpar, iendpar
        ...
        asobc_t(i,j) = asobc_t(i,j) + sobct(i,j)
        athbc_t(i,j) = athbc_t(i,j) + thbct(i,j)
        asobc_s(i,j) = asobc_s(i,j) + sobcs(i,j)
        athbc_s(i,j) = athbc_s(i,j) + thbcs(i,j)
        ...
        !(WL;2010)b
        ! accumulation for 3D fluxes
           DO k=1,ke1
              aeflux (i,j,k) = aeflux (i,j,k) + eflux (i,j,k)
              ahflux (i,j,k) = ahflux (i,j,k) + hflux (i,j,k)
           ENDDO
        !(WL;2010)e
