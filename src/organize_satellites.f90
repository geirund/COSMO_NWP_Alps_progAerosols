...
USE data_parallel,    ONLY :  &
    ...
    imp_reals,       & ! determines the correct REAL type used in the model
                       ! for MPI
    imp_double,      & ! double precision REAL MPI type
...
USE rttov7_dp_wrapper, ONLY: RTTOV7_rttvi_new_wrapper
...
ELSEIF (yaction == 'init') THEN
  ...
  IF (itype_rttov == 7) THEN
    ...
    CALL rttov7_mpi_settings (nproc, my_cart_id, icomm_cart, imp_double, izerrstat)
    ...
    CALL RTTOV7_rttvi_new_wrapper(                                             &
         izerrstat, kppf, kpnsat, kplev, kpch, kpchus, kpnav,                  &
         kpnsav, kpnssv, kpncv, num_sensors, sat_compute(:)%nsatell_table_id,  &
         sat_compute(:)%nsat_id, sat_compute(:)%nsensor_table_id, numchans,    &
         ppres_d, utmn, utmx, uqmn, uqmx, uomn, uomx,                          &
         ivch, kiu1, jpch, jpnsat, jplev )
      ...
      DO  k = 1, ke
        DO i=istartpar,iendpar
           DO j=jstartpar,jendpar
             IF (qc(i,j,k) > 1.0E-7_wp) syclc(i,j,k)=1.0_wp
           ENDDO
         ENDDO
      ENDDO
...
IF (num_sensors > 0 .AND. (SIZE(sat_compute) >= 1)) THEN
  ...
  IF ((sat_long_01 >= -180._wp).and.(sat_long_01 <= 360._wp)) THEN
...
IF (num_sensors > 1 .AND. (SIZE(sat_compute) >= 2)) THEN
  ...
  IF ((sat_long_02 >= -180._wp).and.(sat_long_02 <= 360._wp)) THEN
