...
! AS_2010 <<
  ...
  DOUBLE PRECISION :: qnc_const,Nice0
...
!       & cmp_sub_qi_d,   cmp_sub_qs_d,     cmp_self_qi_d,    &
& cmp_self_qni_d, cmp_rime_qi_qc_d, cmp_rime_qs_qc_d,     &
cmp_rime_qi_qr_d, cmp_rime_qs_qr_d, cmp_melt_qs_qr_d, cmp_melt_qi_qr_d,    &
!       & cmp_self_qni_d, cmp_self_qns_d,   cmp_self_qng_d,   &
!       & cmp_self_qnc_d, cmp_rime_qi_qc_d, cmp_rime_qs_qc_d, &
& cmp_rime_ice_d, cmp_rime_snow_d, cmp_coll_qi_qs_d, cmp_conv_qc_qr_d
!       & cmp_rime_qg_qc_d, cmp_rime_ice_d, cmp_rime_snow_d,  &
!      & cmp_rime_graup_d, cmp_coll_qi_qs_d, cmp_coll_qni_qns_d, &
!       & cmp_coll_qc_qr_d, cmp_coll_qnc_qnr_d,               &
!       & cmp_conv_qi_qg_d, cmp_conv_qni_qng_d, cmp_conv_qs_qg_d, &
...
!OCH <<
  ...
  DOUBLE PRECISION, DIMENSION(:,:,:), ALLOCATABLE :: &
       ...
       & int10, int11, int12, int13, int14, int15, ccn_num1, ccn_num2, ccn_num3
...
!AP>>
!#ifdef COSMOART
  USE wolken_driver, ONLY:calc_i,calc_j,calc_k !AP,cloudact
!#endif
!AP<<
! ub<<      
...
CONTAINS
  ...
  SUBROUTINE sedi_lm_loop_vec (nx, qx, v_n_sedi, v_q_sedi, adz, dt, q_flux, &
    ...
    REAL(KIND=wp), DIMENSION( ims:ime , jms:jme , kms:kme ),    &
         INTENT(inout) :: nx, qx
    REAL(KIND=wp), DIMENSION( ims:ime , jms:jme , kms:kme ),    &
         INTENT(in)    :: adz
    REAL(KIND=wp), INTENT(in) :: dt
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms-1:kme), &
         INTENT(inout) :: q_flux
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms:kme+1), &
         INTENT(inout) :: v_n_sedi,v_q_sedi

    !=======================================================
    ! .. Local variables:

    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms-1:kme) :: n_flux
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms:kme)   :: dz
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme) :: &
...
!CDIR ON_ADB(v_n_sedi,v_q_sedi)
    ...
    n_flux(:,:,kms-1) = 0.0_wp ! 
    q_flux(:,:,kms-1) = 0.0_wp ! upper BC on fluxes
...
!!$          v_nv(i,j) = 0.5_wp * (v_n_sedi(i,j,k+1)+v_n_sedi(i,j,k))
!!$          v_qv(i,j) = 0.5_wp * (v_q_sedi(i,j,k+1)+v_q_sedi(i,j,k))
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          s_nv(i,j) = nx(i,j,k) * dz(i,j,k) * MIN(c_nv(i,j),1.0_wp)
        END DO
      END DO
      DO
        IF (kk <= kms) EXIT
...
!CDIR COLLAPSE
        DO j = jms,jme
          DO i = ims,ime
            IF (c_nv(i,j) > 1.0_wp) THEN
              c_nv(i,j) = (c_nv(i,j) - 1.0_wp) * adz(i,j,kk) * dz(i,j,kk+1)
              s_nv(i,j) = s_nv(i,j) + nx(i,j,kk) * dz(i,j,kk) * MIN(c_nv(i,j),1.0_wp)
            END IF
          END DO
        END DO
      ENDDO
...
!CDIR COLLAPSE
      DO j = jms,jme
        DO i = ims,ime
          s_qv(i,j) = qx(i,j,k) * dz(i,j,k) * MIN(c_qv(i,j),1.0_wp)
        END DO
      END DO
      DO 
        IF (kk <= kms) EXIT
...
!CDIR COLLAPSE
        DO j = jms,jme
          DO i = ims,ime
            IF (c_qv(i,j) > 1.0_wp) THEN
              c_qv(i,j) = (c_qv(i,j) - 1.0_wp) * adz(i,j,kk) * dz(i,j,kk+1)
              s_qv(i,j) = s_qv(i,j) + qx(i,j,kk) * dz(i,j,kk) * MIN(c_qv(i,j),1.0_wp)
            END IF
          END DO
        END DO
      ENDDO
...
!CDIR COLLAPSE
  ...
  SUBROUTINE sedi_lm_loop (nx, qx, v_n_sedi, v_q_sedi, adz, dt, q_flux, &
    ...
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms-1:kme) :: n_flux
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms:kme)   :: dz
    REAL(KIND=wp) :: v_n, v_q, s_n, s_q, c_n, c_q
    REAL(KIND=wp) :: odt
...
!!$ UB: not so good, because this is the downward flanking speed, not
!!$     representative of the flux coming from above:
!!$          v_n = 0.5_wp * (v_n_sedi(i,j,k+1)+v_n_sedi(i,j,k))
!!$          v_q = 0.5_wp * (v_q_sedi(i,j,k+1)+v_q_sedi(i,j,k))
!!$ UB: probably better in the sense of a low-order explicit scheme:
          v_n = v_n_sedi(i,j,k)
          v_q = v_q_sedi(i,j,k)
          ...
          kk = k
          s_n = nx(i,j,kk) * dz(i,j,kk) * MIN(c_n,1.0_wp);
          DO WHILE (c_n > 1.0_wp .AND. kk > kms)
            kk = kk - 1
            c_n = (c_n - 1.0_wp) * adz(i,j,kk) * dz(i ,j,kk+1)
            s_n = s_n + nx(i,j,kk) * dz(i,j,kk) * MIN(c_n,1.0_wp)
          END DO    
          s_n = -s_n * odt;

          kk = k
          s_q = qx(i,j,kk) * dz(i,j,kk) * MIN(c_q,1.0_wp);
          DO WHILE (c_q > 1.0_wp .AND. kk > kms)
            kk = kk - 1
            c_q = (c_q - 1.0_wp) * adz(i,j,kk) * dz(i ,j,kk+1)
            s_q = s_q + qx(i,j,kk) * dz(i,j,kk) * MIN(c_q,1.0_wp)
          END DO    
          s_q = -s_q * odt;
  ...
  SUBROUTINE rain_sedimentation_lm (qr,nr,qc,rainrate,rho,rhocorr,adz,dt, &
    ...
    REAL(KIND=wp)       :: n_r,q_r,x_r,n0,lam,G1,G4,D_m,D_r,mue,v_n,v_q,rhoexp

    REAL(KIND=wp), PARAMETER :: alf = 9.65e+00_wp     ! in SI [m/s]
    REAL(KIND=wp), PARAMETER :: bet = 1.03e+01_wp     ! in SI [m/s]
    REAL(KIND=wp), PARAMETER :: gam = 6.00e+02_wp     ! in SI [1/m]
    REAL(KIND=wp), PARAMETER :: n_0 = 1.00e+07_wp     ! in SI [1/m^4]
    REAL(KIND=wp), PARAMETER :: eps = 1.00e-20_wp  

!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms-1:kme)   :: q_fluss
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms:kme+1)   :: v_n_rain,v_q_rain
      ...
      DO j = jms,jme
        DO i = ims,ime
          ...
          ELSEIF (use_mu_Dm_rain_sedi) THEN
            ...
            D_m = ( 6.0_wp / (rho_w*pi) * x_r )**(1.0_wp/3.0_wp) 
            ...
            ELSEIF (D_m.le.rain_cmu3) then    
              mue = rain_cmu0*TANH((4.0_wp*rain_cmu2*(D_m-rain_cmu3))**2) &
                   & + rain_cmu4
            ELSE
              mue = rain_cmu1*TANH((1.0_wp*rain_cmu2*(D_m-rain_cmu3))**2) &
                   & + rain_cmu4
            ENDIF
            D_r = (D_m**3/((mue+3.0_wp)*(mue+2.0_wp)*(mue+1.0_wp)))**(1.0_wp/3.0_wp)
          ...
          ELSE
            ...
            ELSE
              x_r = q_r / n_r                           !..mittlere Masse in SI
              x_r = MIN(MAX(x_r,rain%x_min),rain%x_max)
              n0  = n_r * (G4/x_r)**(1.0_wp/3.0_wp)             !..n0
              n0  = MIN(MAX(250.0e3_wp,n0),20000.0e3_wp)
              lam = (G4*n0/(q_r+eps))**(0.25_wp)                 !..lambda
            ...
            ! UB_20090316<<
          ENDIF
          v_n = alf - bet / (1.0_wp + gam*D_r)**(mue+1.0_wp)
          v_q = alf - bet / (1.0_wp + gam*D_r)**(mue+4.0_wp)
          ! UB_20100107>>
...
!CDIR COLLAPSE
  ...
  SUBROUTINE graupel_sedimentation_lm (qg,ng,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=wp)     :: n_g,q_g,x_g,lam,n,v_n,v_q,D_g,rhoexp
    REAL(KIND=wp), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=wp), PARAMETER :: eps = 1.00e-20_wp

!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms-1:kme)   :: q_fluss
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms:kme+1)   :: v_n_sedi,v_q_sedi

    !==================================================
    ! .. Preparations:

    IF (firstcall.NE.1) THEN
      alf_n = graupel%a_vel * gfct((graupel%nu+graupel%b_vel+1.0_wp)/graupel%mu)&
           &                / gfct((graupel%nu+1.0_wp)/graupel%mu)
      alf_q = graupel%a_vel * gfct((graupel%nu+graupel%b_vel+2.0_wp)/graupel%mu)&
           &                / gfct((graupel%nu+2.0_wp)/graupel%mu)
      c_lam = gfct((graupel%nu+1.0_wp)/graupel%mu)/gfct((graupel%nu+2.0_wp)/graupel%mu)
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
...
!CDIR COLLAPSE
  ...
  SUBROUTINE hail_sedimentation_lm (qh,nh,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=wp)     :: n_g,q_g,x_g,lam,v_n,v_q,rhoexp,D_g
    REAL(KIND=wp), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=wp), PARAMETER :: eps = 1.00e-20   

    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms-1:kme)   :: q_fluss
    REAL(KIND=wp), DIMENSION(ims:ime,jms:jme,kms:kme+1)   :: v_n_sedi,v_q_sedi,zr

    !==================================================
    ! .. Preparations:

    IF (firstcall.NE.1) THEN
      alf_n = hail%a_vel * gfct((hail%nu+hail%b_vel+1.0_wp)/hail%mu)&
           &                / gfct((hail%nu+1.0_wp)/hail%mu)
      alf_q = hail%a_vel * gfct((hail%nu+hail%b_vel+2.0_wp)/hail%mu)&
           &                / gfct((hail%nu+2.0_wp)/hail%mu)
      c_lam = gfct((hail%nu+1.0_wp)/hail%mu)/gfct((hail%nu+2.0_wp)/hail%mu)
      IF (isdebug) THEN
#ifdef SEDI_VECTORIZED
...
!CDIR COLLAPSE
  ...
  SUBROUTINE snow_sedimentation_lm (qs,ns,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=wp)     :: n_s,q_s,x_s,lam,v_n,v_q,mue,nue,D_m,rhoexp
    REAL(KIND=wp), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=wp), PARAMETER :: eps = 1.00e-20_wp   
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      alf_n = snow%a_vel * gfct((snow%nu+snow%b_vel+1.0_wp)/snow%mu) / &
           gfct((snow%nu+1.0_wp)/snow%mu)
      alf_q = snow%a_vel * gfct((snow%nu+snow%b_vel+2.0_wp)/snow%mu) / &
           gfct((snow%nu+2.0_wp)/snow%mu)
      c_lam = gfct((snow%nu+1.0_wp)/snow%mu)/gfct((snow%nu+2.0_wp)/snow%mu)
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
      ...
      DO j = jms,jme
        DO i = ims,ime
          ...
          IF (use_mu_Dm_snow_sedi) THEN
            ...
            nue = (mue-2.0_wp)/3.0_wp
            alf_n = snow%a_vel * gfct((nue+snow%b_vel+1.0_wp)/snow%mu)/gfct((nue+1.0_wp)/snow%mu)
            alf_q = snow%a_vel * gfct((nue+snow%b_vel+2.0_wp)/snow%mu)/gfct((nue+2.0_wp)/snow%mu)
            c_lam = gfct((nue+1.0)/snow%mu)/gfct((nue+2.0_wp)/snow%mu)
          ENDIF
! UB_20100107>>
...
!CDIR COLLAPSE
  ...
  SUBROUTINE ice_sedimentation_lm (qi,ni,rainrate,rho,rhocorr,adz,dt, &
    ...
    INTEGER(KIND=iintegers)         :: i, j, k, kk
    INTEGER(KIND=iintegers), SAVE   :: firstcall
    REAL(KIND=wp)     :: n_i,q_i,x_i,lam,v_n,v_q,n,D_i,rhoexp
    REAL(KIND=wp), SAVE      :: alf_n,alf_q,c_lam
    REAL(KIND=wp), PARAMETER :: eps = 1.00e-20_wp  
...
!!! Maybe convert the following arrays to ALLOCATABLE to save stack memory!
    ...
    IF (firstcall.NE.1) THEN
      alf_n = ice%a_vel * gfct((ice%nu+ice%b_vel+1.0_wp)/ice%mu) / &
           gfct((ice%nu+1.0_wp)/ice%mu)
      alf_q = ice%a_vel * gfct((ice%nu+ice%b_vel+2.0_wp)/ice%mu) / &
           gfct((ice%nu+2.0_wp)/ice%mu)
      c_lam = gfct((ice%nu+1.0_wp)/ice%mu)/gfct((ice%nu+2.0_wp)/ice%mu)
      IF(isdebug) THEN
#ifdef SEDI_VECTORIZED
...
CONTAINS
  ...
  SUBROUTINE ice_nucleation_vec()
    ...
    USE konstanten,         ONLY: r,cv,cp,wolke_typ 
    USE parallele_umgebung, ONLY: isIO,abortparallel
    USE data_runcontrol, ONLY: lINprog
...
!!$                q(i,j,k)     = q(i,j,k)     - nuc_q(i,j,k)
                ...
                n_ice(i,j,k) = 200.0  !1000.0 for highIN
...
!         &                        cmp_rime_qi_qc_d, cmp_rime_ice_cloud_d, cmp_mult_ice_d, &                          
!         &                        cmp_mult_graup_d, cmp_melt_ice_d, cmp_melt_qi_d, &                                
!         &                        cmp_rime_qg_qr_d, cmp_rime_qng_qnr_d, cmp_rime_qi_qr_d,cmp_rime_ice_rain_d, &     
!         &                        cmp_rime_qs_qc_d, cmp_rime_snow_cloud_d, cmp_mult_ice_snow_d, cmp_rime_qs_qr_d, & 
!         &                        cmp_rime_snow_rain_d, cmp_conv_qs_qg_d, cmp_conv_qns_qng_d, cmp_rime_qr_qs_qg_d,&
...
!                     cmp_rime_qng_qnr_d(i,j,k)   = cmp_rime_qng_qnr_d(i,j,k)+rime_n
!                  END IF
!<<OCH
#ifdef SAVE_CONVERSIONRATES
                  IF (speichere_dqdt) THEN
...
!>>OCH
              IF (lcmprates) THEN
                 cmp_rime_qs_qc_d(i,j,k) = rime_q
!                 cmp_rime_snow_cloud_d(i,j,k)   = rime_n
              END IF
!<<OCH
              ! Eismultiplikation nach Hallet und Mossop
...
! ub>> setzte melt_n so, dass x_h beim Schmelzvorgang erhalten bleibt:
  ...
  SUBROUTINE cloud_freeze ()
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, &
                                  T, p, q, &
         &                        q_cloud, n_cloud, q_ice, n_ice, dt, dqdt, speichere_dqdt, s_i!,&
...
!<< OCH
              ELSE                                      
                ...
                IF (T_c > -30.0) THEN            
                  j_hom = 1.0d6 * 10.**(-7.63-2.996*(T_c+30.0))           !..J in 1/(m3 s) 
                ELSE
                  j_hom = 1.0d6 * 10.**(-243.4-14.75*T_c-0.307*T_c**2-0.00287*T_c**3-0.0000102*T_c**4)
                ENDIF
! >> OCH
!                IF (lcmprates) THEN
...
!<<OCH
  ...
  SUBROUTINE rain_evaporation_uli ()
    ...
    USE gamma_functions_mp_seifert,    ONLY: incgfct_lower
    USE wolken_konstanten,  ONLY: gfct
    USE data_gscp, ONLY: wccn1, wccn2, wccn3
    USE data_runcontrol, ONLY: lCCNprog
    USE data_fields, ONLY: debug4
...
!              eva_n = f_n * eva / x_r
            ...
            IF (cloud_typ > 1) THEN
              ...
              if (lCCNprog) then
                 ccn_num1(i,j,k)=ccn_num1(i,j,k)+wccn1*eva_n
                 ccn_num2(i,j,k)=ccn_num2(i,j,k)+wccn2*eva_n
                 ccn_num3(i,j,k)=ccn_num3(i,j,k)+wccn3*eva_n
                 !debug4(i,j,k)=wccn2*eva_n
              endif
  ...
  SUBROUTINE rain_evaporation ()
    ...
    USE data_gscp, ONLY: wccn1, wccn2, wccn3
    USE data_runcontrol, ONLY: lCCNprog
    USE data_fields, ONLY: debug2, debug3, debug5
...
CONTAINS 
  ...
  SUBROUTINE clouds ()
    ...
    USE globale_variablen, ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, dt, &
         ...
         &                       w,s_i, ccn_num1, ccn_num2, ccn_num3,       &
         &                       speichere_dqdt,                &
   ...
   USE data_cosmo_art, ONLY : &
    ...
    USE data_parameters , ONLY :     &
         ...
         & wp                      ! KIND-type parameters for real variables
#endif 
   USE data_runcontrol , ONLY : lINprog, Tdiff_IN,Tmax_IN, lnc_const, lni_const_imm, lCCNprog,lcmprates
    ! .. Local Variables ..
    INTEGER          :: i, j, k, sitat_var, n_dt,dimIN, stat_var
#ifdef COSMOART
    INTEGER          :: ii, jj, kk
#endif 
    DOUBLE PRECISION :: dtrho, e_sat, rdrl, rdrlm1, rlrd, rlrdm1, t_
    DOUBLE PRECISION :: dt_local, dNi
    REAL             :: wmax, qvmax,qcmax,qrmax,qimax,qsmax,qgmax,precmax,ncmax
    DOUBLE PRECISION :: Tnuc_1, Tnuc_2, Tnuc_3,Tnuc_4, Tnuc_5, Tnuc_6,Tnuc_7, Tnuc_8, Tnuc_9, &
                        Tnuc_10, Tnuc_11, Tnuc_12,Tnuc_13, Tnuc_14, Tnuc_15, nuc_IN, nuc_INq
    DOUBLE PRECISION, ALLOCATABLE :: in_trcr(:,:,:,:)
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION (:,:,:) :: ncloud_old, nice_old, ncloud_tmp
    if (lCCNprog) then
      ALLOCATE(ncloud_tmp(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    endif
    if (lnc_const) then
      ALLOCATE(ncloud_old(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    endif
    if (lni_const_imm) then
      ALLOCATE(nice_old(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    endif
...
!CDIR COLLAPSE
    ...
    if (lCCNprog) then
       ccn_num1=ccn_num1+wccn1*(ncloud_tmp - n_cloud)
       ccn_num2=ccn_num2+wccn2*(ncloud_tmp - n_cloud)
       ccn_num3=ccn_num3+wccn3*(ncloud_tmp - n_cloud)
    endif
    ...
    if (lCCNprog) then
       ccn_num1=ccn_num1+wccn1*(ncloud_tmp - n_rain)
       ccn_num2=ccn_num2+wccn2*(ncloud_tmp - n_rain)
       ccn_num3=ccn_num3+wccn3*(ncloud_tmp - n_rain)
    endif
...
! AS_20090609>> new version with hom. nucleation
       if (.NOT. lINprog .AND. .NOT.lni_const_imm) then
...
! AS_20090609<<
    ...
    ENDIF
      ...
      if (.NOT. lINprog .AND. .NOT. lni_const_imm) then
       CALL cloud_freeze ()
      else if (lINprog) then
        dimIN=15
        ALLOCATE(in_trcr(dimIN, 0:dim_ix, 1:dim_iy, 1:dim_iz), STAT=stat_var)
        in_trcr(1,:,:,:)=int1(:,:,:)
        in_trcr(2,:,:,:)=int2(:,:,:)
        in_trcr(3,:,:,:)=int3(:,:,:)
        in_trcr(4,:,:,:)=int4(:,:,:)
        ...
        in_trcr(15,:,:,:)=int15(:,:,:)
        Tnuc_1=Tmax_IN
        Tnuc_2=Tnuc_1-Tdiff_IN
        Tnuc_3=Tnuc_2-Tdiff_IN
        Tnuc_4=Tnuc_3-Tdiff_IN
        Tnuc_5=Tnuc_4-Tdiff_IN
        Tnuc_6=Tnuc_5-Tdiff_IN
        Tnuc_7=Tnuc_6-Tdiff_IN
        Tnuc_8=Tnuc_7-Tdiff_IN
        Tnuc_9=Tnuc_8-Tdiff_IN
        Tnuc_10=Tnuc_9-Tdiff_IN
        Tnuc_11=Tnuc_10-Tdiff_IN
        Tnuc_12=Tnuc_11-Tdiff_IN
        Tnuc_13=Tnuc_12-Tdiff_IN
        Tnuc_14=Tnuc_13-Tdiff_IN
        Tnuc_15=Tnuc_14-Tdiff_IN

        DO k=1, loc_iz
         DO j=1, loc_iy
          DO i=0, loc_ix
            if ((q_cloud(i,j,k)+q_rain(i,j,k)/rho0) >=1e-6_wp ) THEN    !convert [q_cloud]=kg/m3 to kg/kg using air density rho0
             nuc_IN=0.0
             IF (T(i,j,k) .le. Tnuc_1) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(1,i,j,k)
               in_trcr(1,i,j,k)=in_trcr(1,i,j,k)-0.5*in_trcr(1,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_2) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(2,i,j,k)
               in_trcr(2,i,j,k)=in_trcr(2,i,j,k)-0.5*in_trcr(2,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_3) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(3,i,j,k)
               in_trcr(3,i,j,k)=in_trcr(3,i,j,k)-0.5*in_trcr(3,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_4) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(4,i,j,k)
               in_trcr(4,i,j,k)=in_trcr(4,i,j,k)-0.5*in_trcr(4,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_5) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(5,i,j,k)
               in_trcr(5,i,j,k)=in_trcr(5,i,j,k)-0.5*in_trcr(5,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_6) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(6,i,j,k)
               in_trcr(6,i,j,k)=in_trcr(6,i,j,k)-0.5*in_trcr(6,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_7) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(7,i,j,k)
               in_trcr(7,i,j,k)=in_trcr(7,i,j,k)-0.5*in_trcr(7,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_8) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(8,i,j,k)
               in_trcr(8,i,j,k)=in_trcr(8,i,j,k)-0.5*in_trcr(8,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_9) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(9,i,j,k)
               in_trcr(9,i,j,k)=in_trcr(9,i,j,k)-0.5*in_trcr(9,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_10) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(10,i,j,k)
               in_trcr(10,i,j,k)=in_trcr(10,i,j,k)-0.5*in_trcr(10,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_11) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(11,i,j,k)
               in_trcr(11,i,j,k)=in_trcr(11,i,j,k)-0.5*in_trcr(11,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_12) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(12,i,j,k)
               in_trcr(12,i,j,k)=in_trcr(12,i,j,k)-0.5*in_trcr(12,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_13) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(13,i,j,k)
               in_trcr(13,i,j,k)=in_trcr(13,i,j,k)-0.5*in_trcr(13,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_14) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(14,i,j,k)
               in_trcr(14,i,j,k)=in_trcr(14,i,j,k)-0.5*in_trcr(14,i,j,k)
             END IF
             IF (T(i,j,k) .le. Tnuc_15) THEN
               nuc_IN = nuc_IN+0.5*in_trcr(15,i,j,k)
               in_trcr(15,i,j,k)=in_trcr(15,i,j,k)-0.5*in_trcr(15,i,j,k)
             END IF
             nuc_INq = MIN(nuc_IN * ice%x_min, q_cloud(i,j,k))
             n_ice(i,j,k) = n_ice(i,j,k) + nuc_IN
             q_ice(i,j,k) = q_ice(i,j,k) + nuc_INq
             q_cloud(i,j,k) = q_cloud(i,j,k) - nuc_INq
             n_cloud(i,j,k) = n_cloud(i,j,k) - nuc_IN
!              if (lcmprates) then
!                cmp_fr_qni_d(i,j,k)= nuc_IN
!                cmp_fr_qi_d(i,j,k)=nuc_INq
!              endif
            endif
          END DO
         END DO
      ...
      else if (lni_const_imm) then

        DO k=1, loc_iz
         DO j=1, loc_iy
          DO i=0, loc_ix
            if (s_i(i,j,k)>=0.05 .AND. ((q_cloud(i,j,k)+q_rain(i,j,k)/rho0) >=1e-6_wp) ) THEN    !convert [q_cloud]=kg/m3 to kg/kg using air density rho0
                   dNi =  MAX( (Nice0 - n_ice(i,j,k)) ,0.d0)
                   n_ice(i,j,k) = n_ice(i,j,k) + dNi
                   q_ice(i,j,k) = q_ice(i,j,k) + dNi*ice%x_min
                   q    (i,j,k) = q    (i,j,k) - dNi*ice%x_min !vapor
                   n_cloud(i,j,k) = n_cloud(i,j,k) - dNi
            endif
          ENDDO
         ENDDO
        ENDDO
...
! UB_20090227<<
    ...
    !if cloud droplet number is adjusted to remain in bounds, CCN budget has to be adjusted as well to avaid inconsistency
    if (lCCNprog) then 
      ncloud_tmp=n_cloud
    endif
        ...
        DO j = 1, loc_iy
          ...
          kk=calc_k(i)
            ...
            IF (q_cloud(i,j,k) > 1.d-10) THEN   ! maybe use greater value for "real" clouds
             diagnenes(ii,jj,kk,15)= diagnenes(ii,jj,kk,15)+REAL(q_cloud(i,j,k),wp)
             diagnenes(ii,jj,kk,16)= diagnenes(ii,jj,kk,16)+REAL(n_cloud(i,j,k),wp)
             diagnenes(ii,jj,kk,17)= diagnenes(ii,jj,kk,17)+1.0_wp
            ENDIF

            IF (q_rain(i,j,k) > 1.d-15) THEN   
             diagnenes(ii,jj,kk,18)= diagnenes(ii,jj,kk,18)+REAL(q_rain(i,j,k),wp)
             diagnenes(ii,jj,kk,19)= diagnenes(ii,jj,kk,19)+1.0_wp
            ENDIF            

            IF (q_ice(i,j,k) > 1.d-15) THEN   
             diagnenes(ii,jj,kk,20)= diagnenes(ii,jj,kk,20)+REAL(q_ice(i,j,k),wp)
             diagnenes(ii,jj,kk,21)= diagnenes(ii,jj,kk,21)+REAL(n_ice(i,j,k),wp)
             diagnenes(ii,jj,kk,22)= diagnenes(ii,jj,kk,22)+1.0_wp
            ENDIF

            IF (q_snow(i,j,k)+q_graupel(i,j,k)+q_hail(i,j,k) > 1d-15) THEN   
             diagnenes(ii,jj,kk,23)= diagnenes(ii,jj,kk,23)+REAL(q_snow(i,j,k)+q_graupel(i,j,k)+q_hail(i,j,k),wp)
             diagnenes(ii,jj,kk,24)= diagnenes(ii,jj,kk,24)+1.0_wp
            ENDIF
...
!OCH <<
    ...
    USE data_runcontrol,    ONLY: lcmprates,lINprog,lCCNprog
    ...
    IF (firstcall /= 1) THEN
      ...
      END IF
      IF (lINprog) THEN
        ALLOCATE (int1(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int2(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int3(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int4(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int5(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int6(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int7(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int8(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int9(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int10(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int11(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int12(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int13(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int14(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  int15(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  STAT = stat_var)
        IF (stat_var /= 0) THEN
          CALL abortparallel ("Fehler bei Alloziierung der IN Tracer", stat_var)
        END IF
      ENDIF
      IF (lCCNprog) THEN
        ALLOCATE (ccn_num1(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  ccn_num2(0:dim_ix, 1:dim_iy, 1:dim_iz), &
                  ccn_num3(0:dim_ix, 1:dim_iy, 1:dim_iz), &
        STAT = stat_var)
        IF (stat_var /= 0) THEN
          CALL abortparallel ("Fehler bei Alloziierung der IN Tracer", stat_var)
        END IF
...
!OCH<<
  ...
  SUBROUTINE dealloc_wolken ()
    ...
    USE globale_variablen,  ONLY: q_cloud, q_rain, q_ice, q_snow, q_graupel,  &
       ...
       & cmp_dep_qi_d,   cmp_dep_qs_d,     cmp_self_qi_d,&
       & cmp_self_qni_d, cmp_rime_qi_qc_d, cmp_rime_qs_qc_d,     &
       & cmp_rime_qi_qr_d, cmp_rime_qs_qr_d, cmp_melt_qi_qr_d, cmp_melt_qs_qr_d, &
       & cmp_rime_ice_d, cmp_rime_snow_d, cmp_coll_qi_qs_d, cmp_conv_qc_qr_d
!         & cmp_act_qc_d,   cmp_act_qnc_d,    cmp_fr_qi_d,      &
!         & cmp_fr_qni_d,   cmp_nuc_qi_d,     cmp_nuc_qni_d,    &
!         & cmp_dep_qi_d,   cmp_dep_qc_d,     cmp_dep_qs_d,     &
...
#endif
!OCH>>
      DEALLOCATE( cmp_dep_qi_d,   cmp_dep_qs_d,     cmp_self_qi_d,&
       & cmp_self_qni_d, cmp_rime_qi_qc_d, cmp_rime_qs_qc_d,     &
       & cmp_rime_qi_qr_d, cmp_rime_qs_qr_d, cmp_melt_qi_qr_d, cmp_melt_qs_qr_d, &
       & cmp_rime_ice_d, cmp_rime_snow_d, cmp_coll_qi_qs_d, cmp_conv_qc_qr_d)
!     cmp_act_qc_d,   cmp_act_qnc_d,    cmp_fr_qi_d,      &
   ...
   DO k = 1, loc_iz
      DO j = 1, loc_iy
        DO i = 0, loc_ix
          ...
          jj=calc_j(i)
          kk=calc_k(i)
          diagnenes(ii,jj,kk,25)= diagnenes(ii,jj,kk,25)+REAL(rate_q(i,j,k),wp)
          diagnenes(ii,jj,kk,26)= diagnenes(ii,jj,kk,26)+1.0_wp
        END DO
      END DO
    END DO
#endif
...
! << OCH
  ...
  SUBROUTINE vapor_deposition_growth(dt_local,i_local)
    ...
    USE globale_variablen,  ONLY: loc_ix, loc_iy, loc_iz, dim_ix, dim_iy, dim_iz, T, p, q, rho, &
         ...
         &                        deprate_ice, deprate_snow, cmp_dep_qi_d, cmp_dep_qs_d!, &
...
!         &                        cmp_sub_qi_d, cmp_sub_qs_d
    USE konstanten,         ONLY: pi,cv,cp
    USE parallele_umgebung, ONLY: isIO
    USE data_runcontrol,    ONLY: lcmprates,lINprog
    USE data_gscp, ONLY : sumIN_t0,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15
    USE data_modelconfig, ONLY: ie_tot, je_tot, ie,je,ke
    ...
    DOUBLE PRECISION            :: e_sw            !..Wasserpartialdruck bei saettigung
    DOUBLE PRECISION            :: q_d,x_d,e_d,R_f,p_a,rho_a,dep_sum
    INTEGER                     :: i,j,k,n,dimIN,stat_var,izerror
    INTEGER, SAVE               :: firstcall
    DOUBLE PRECISION, SAVE      :: gr_sn_fakt_dep, gr_sn_fakt_rim
...
!If dep_ice, or dep_snow <0, then sublimation of crystals occurs and IN are released again
!Following the Morrison Scheme, the number of released IN is computed as:
!              released IN=dep_ice*n_ice/q_ice + dep_snow*n_snow/q_snow
            IF(lINprog) then
               if (dep_ice(i,j,k) < 0 .OR. dep_snow(i,j,k) < 0 ) then
                if (dep_ice(i,j,k) < 0 .AND. q_ice(i,j,k) > 1d-15) then
                 int1(i,j,k)=int1(i,j,k)-w1*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int2(i,j,k)=int2(i,j,k)-w2*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int3(i,j,k)=int3(i,j,k)-w3*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int4(i,j,k)=int4(i,j,k)-w4*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int5(i,j,k)=int5(i,j,k)-w5*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int6(i,j,k)=int6(i,j,k)-w6*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int7(i,j,k)=int7(i,j,k)-w7*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int8(i,j,k)=int8(i,j,k)-w8*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int9(i,j,k)=int9(i,j,k)-w9*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int10(i,j,k)=int10(i,j,k)-w10*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int11(i,j,k)=int11(i,j,k)-w11*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int12(i,j,k)=int12(i,j,k)-w12*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int13(i,j,k)=int13(i,j,k)-w13*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int14(i,j,k)=int14(i,j,k)-w14*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                 int15(i,j,k)=int15(i,j,k)-w15*dep_ice(i,j,k)*n_ice(i,j,k)/q_ice(i,j,k)
                endif
                if (dep_snow(i,j,k) < 0 .AND. q_snow(i,j,k) > 1d-15) then
                 int1(i,j,k)=int1(i,j,k)-w1*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int2(i,j,k)=int2(i,j,k)-w2*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int3(i,j,k)=int3(i,j,k)-w3*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int4(i,j,k)=int4(i,j,k)-w4*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int5(i,j,k)=int5(i,j,k)-w5*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int6(i,j,k)=int6(i,j,k)-w6*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int7(i,j,k)=int7(i,j,k)-w7*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int8(i,j,k)=int8(i,j,k)-w8*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int9(i,j,k)=int9(i,j,k)-w9*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int10(i,j,k)=int10(i,j,k)-w10*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int11(i,j,k)=int11(i,j,k)-w11*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int12(i,j,k)=int12(i,j,k)-w12*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int13(i,j,k)=int13(i,j,k)-w13*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int14(i,j,k)=int14(i,j,k)-w14*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                 int15(i,j,k)=int15(i,j,k)-w15*dep_snow(i,j,k)*n_snow(i,j,k)/q_snow(i,j,k)
                endif
               endif !dep_ice or dep_snow <0
            endif !lINprog
...
!!$ UB_20120914              f_v  = max(f_v,1.d0) !unnoetig??
  ...
  !GE<<
    ...
    ! Locale Variablen 
    DOUBLE PRECISION            :: T_a             !..Absolute Temperatur
    DOUBLE PRECISION            :: q_d,x_d,e_d,n_c,rho_a,ccn_n1,ccn_n2,ccn_n3
    DOUBLE PRECISION            :: nuc_n, nuc_q, S, dS, dSdz_w, Ntot
    DOUBLE PRECISION            :: wcb,q_c
    DOUBLE PRECISION            :: a_ld,a_dl,e_sw,q_sw
    DOUBLE PRECISION            :: N_ccn,k_ccn,N_max,N_min,S_max
    DOUBLE PRECISION            :: z0_nccn, z1e_nccn
    INTEGER                     :: i,j,k,nuc_typ,stat_var
  !GE>>
    DOUBLE PRECISION, ALLOCATABLE, DIMENSION (:,:,:) :: ncloud_tmpj
  !GE<<

  !AP>>                                                                          
    LOGICAL                     :: lconst_aerosize                             
    lconst_aerosize = .TRUE.
  !AP<<
  !GE>>
    if (lCCNprog) then
     ALLOCATE(ncloud_tmpj(0:dim_ix,1:dim_iy,1:dim_iz), STAT=stat_var)
    endif
  !GE<
    ...
    ELSEIF (nuc_typ == 3) THEN
      !... maritime case
      N_ccn = 300.0d06
      N_max = 50000.0d06 !600.0d06
      N_min = 2.0d06  ! 50.0d06
      S_max = 2.000
      k_ccn = 0.462
    ELSEIF (nuc_typ == 2) THEN
    ...
    DO k = 1, loc_iz
      DO j = 1, loc_iy
        ...
        !AP>>                                                                          
        !Implemented constant aerosol size distribution activatk," on assuming e.g.:      
        !activ_modes(CCN_A)%l_koehler = .TRUE.                                         
        !activ_modes(CCN_A)%number    = 400.0e06_wp      ! is it possible to conver
        !activ_modes(CCN_A)%sigma     = 2.5_wp                                     
        !activ_modes(CCN_A)%solmassfr = 1.0_wp                                     
        !activ_modes(CCN_A)%diameter  = 0.08e-06_wp                                
        !activ_modes(CCN_A)%dissfac_mean = 1.0_wp ! muss bei seasalts gerechnet wer
        !activ_modes(CCN_A)%molweight_mean = 58.4e-03_wp                           
        !activ_modes(CCN_A)%rhosol_mean = 2.2e+03_wp                               
        !activ_modes(CCN_A)%rhoinsol_mean = 2.2e+03_wp                             
           
       IF (lconst_aerosize) then                  
          ! get CCN from activation parameterizations; settings:                         
          ! typeact = 1 (in-cloud activation without considering existing droplets)    
          ! typeact = 2 sgs effects considered for activation                          
          ! typeact = 3 in-cloud & existing cloud and rain drops are considered in acti
          nuc_q = 0.0d0                                                                
          nuc_n = 0.d0                                                                 
          IF (w_cb(i,j,k) > 0.00d0) THEN                                               
            ...
            Ntot = 0.0d0                                                               
            if (lCCNprog) then
              ccn_num2(i,j,k)=500.0e+6_wp   !GE: make background mode passive - constant value enters activation
             ! ccn_num1(i,j,k)=0.0_wp
              ccn_n1=ccn_num1(i,j,k)
              ccn_n2=ccn_num2(i,j,k)
              ccn_n3=ccn_num3(i,j,k)
              CALL call_activation(3,calc_i(i),calc_j(i),calc_k(i),n_c,Ntot,nuc_n,ccn_n1, ccn_n2, ccn_n3)
            else
              CALL call_activation(3,calc_i(i),calc_j(i),calc_k(i),n_c,Ntot,nuc_n)
            endif                                                                        
            nuc_n = nuc_n *dt ! calculate nucleation
            if (lCCNprog) then
              nuc_n=MAX(nuc_n-n_cloud(i,j,k),0.0d0)
              IF(nuc_n > Ntot) THEN
                nuc_n=Ntot 
              ENDIF
            ELSE
             IF(nuc_n+n_cloud(i,j,k) > Ntot) THEN
               nuc_n=Ntot-n_cloud(i,j,k)
             ENDIF
            ENDIF
          ELSE
            nuc_n=0.0
...
!<<OCH
          if (lCCNprog .AND. nuc_n >0) then
           ...
           ccn_num1(i,j,k)=ccn_n1
           ccn_num2(i,j,k)=ccn_n2
           ccn_num3(i,j,k)=ccn_n3
          endif
...
!            that would come from the dynamical core
          if (lCCNprog) then
            ncloud_tmpj(i,j,k)=n_cloud(i,j,k)
          endif
          ...
          if (lCCNprog) then
            ccn_num1(i,j,k)=ccn_num1(i,j,k)+wccn1*(ncloud_tmpj(i,j,k) - n_cloud(i,j,k))
            ccn_num2(i,j,k)=ccn_num2(i,j,k)+wccn2*(ncloud_tmpj(i,j,k) - n_cloud(i,j,k))
            ccn_num3(i,j,k)=ccn_num3(i,j,k)+wccn3*(ncloud_tmpj(i,j,k) - n_cloud(i,j,k))
          endif
...
!<AS20080426
    ...
    if (lCCNprog) then
      ccn_num1=ccn_num1+wccn1*(ncloud_tmpj - n_cloud)
      ccn_num2=ccn_num2+wccn2*(ncloud_tmpj - n_cloud)
      ccn_num3=ccn_num3+wccn3*(ncloud_tmpj - n_cloud)
    endif
...
!CDIR COLLAPSE
  ...
  SUBROUTINE clip_and_limit_wp(qx, nx, parti, ims, ime, jms, jme, kms, kme)

    USE wolken_konstanten, ONLY : particle
    USE data_parameters,   ONLY : iintegers, wp

    IMPLICIT NONE

    INTEGER(kind=iintegers), INTENT(IN) :: ims, ime, jms, jme, kms, kme
    REAL(kind=wp), INTENT(INOUT)    :: qx(ims:ime, jms:jme, kms:kme), &
                                           nx(ims:ime, jms:jme, kms:kme)
    TYPE(particle), INTENT(IN)          :: parti
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE clip_and_limit_wp

  SUBROUTINE limit_only_wp(qx, nx, parti, ims, ime, jms, jme, kms, kme)
...
!CDIR COLLAPSE
  ...
  END SUBROUTINE limit_only_wp

  SUBROUTINE clip_only_wp(qx, ims, ime, jms, jme, kms, kme)
