...
!   p2dp               ! get approx. model layer thickness at specified pressure
!-------------------------------------------------------------------------------
...
SUBROUTINE obs_air_org_mult ( lsvcorl , ntotsgo , ntotmlo , thairh , lcdf      &
...
!   nexcloc          ,& ! number of reports in excess of local array size
    ...
    ntotacd (4)      ,& ! number of reports of a certain aircraft code type
  ...
  INTEGER (KIND=iintegers) ::  &
    ...
    io_tot  ,jo_tot  ,& ! global horizontal coordinates of observation
...
!   irnode           ,& ! node with subdomain containing the observ. location
    ...
    izmplcode, nstat ,& ! error variable for MPI error code / opening files
...
!   ==================
  ...
  ENDDO
  IF (num_compute > 1) THEN
    CALL global_values ( ntotacd, 4, 'SUM', imp_integers, icomm_cart, -1       &
                       , yerr, ierr )
!   ==================
  ENDIF
...
!-------------------------------------------------------------------------------
  ...
  CALL obs_air_list ( 9, ncanda, ntotsg , 0, 0, 0 , lcdf , 1 , ntotnsl )
! =================
...
! ----------------------------------------------
  ...
  IF (nstaid > 0)                                                              &
    ...
    CALL obs_air_list ( 9, 1, ntotsgo , 2, 1, nstaid, lcdf, 1 , ntotasl )
!   =================
...
!               simulates 'obs_pointrs' for aircraft observation type only
                icma  = kcdtyp/100 + MIN( MOD( kcdtyp,10 ) , 3 )
                iev2  = kobtyp
!               noctps (iev2) = noctps(iev2) + 1
              ENDIF
              neventr (neblak,icma,iev2) = neventr(neblak,icma,iev2) + 1
              mosghd (iarlls(irep,1),nhpass) = 2
              mosghd (iarlls(irep,1),nhschr) =                                 &
                          IBSET ( mosghd(iarlls(irep,1), nhschr), nvbkbp )
              mosghd (iarlls(irep,1),nhschr) =                                 &
                          IBSET ( mosghd(iarlls(irep,1), nhschr), FL_BLACKLIST )
            ENDIF
          ENDIF
        ELSE
...
!           simulates 'obs_pointrs' for aircraft observation type only:
            ...
            iev2  = kobtyp
          ENDIF
          neventr (neslml,icma,iev2) = neventr (neslml,icma,iev2) - 1
          EXIT listctl
        ENDIF
      ENDDO listctl
    ENDIF
...
! --------------------------------------------
      ...
      CALL obs_air_list ( ncandls, 1, ncande, 2, ista, ista, lcdf, 3 , ntotssl )
!     =================
...
!             , my_cart_id, ista, yarlls(mzslon(iairls(1,1,3))), ntotssl
      ...
      IF (ntotssl > 0)                                                         &
        ...
        CALL obs_air_make_mult ( ntotssl, thairh, lcdf , ntotaml, nexceair(2) )
!       ======================
...
!       simulates 'obs_pointrs' for aircraft observation type only:
        ...
        iev2  = kobtyp
      ENDIF
      neventr (nenoml,icma,iev2) = neventr (nenoml,icma,iev2) + 1
...
!       simulates 'obs_pointrs' for aircraft observation type only:
        ...
        iev2  = kobtyp
      ENDIF
      ! for simplicity, report status counters are updated only if NetCDF-read
      IF (lcdf) THEN
      ...
      IF ((mosghd(nsga,nhpass) /= 1) .AND. (npassiv == 1))                     &
        neventr (neslps,icma,iev2) = neventr (neslps,icma,iev2) + 1
      IF ((mosghd(nsga,nhpass) == 1) .AND. (npassiv /= 1))                     &
        neventr (neslps,icma,iev2) = neventr (neslps,icma,iev2) - 1
      IF (      (BTEST( mosghd(nsga,nhflag), FL_FLIGHTTRACK ))                 &
          .AND. (npassiv <= 1))                                                &
        neventr (netrac,icma,iev2) = neventr (netrac,icma,iev2) - 1
      IF ((mosghd(nsga,nhpass) <= 1) .AND. (npassiv == 3))                     &
        neventr (netrac,icma,iev2) = neventr (netrac,icma,iev2) + 1
      IF ((mosghd(nsga,nhpass) <= 1) .AND. (npassiv == 4))                     &
        neventr (nethin,icma,iev2) = neventr (nethin,icma,iev2) + 1
    ENDIF
...
SUBROUTINE obs_air_make_mult ( ntotssl , thairh , lcdf , ntotaml , nexcloc )
...
! -------------
  ...
  REAL    (KIND=wp   )     ::  &
    ...
    fidxwp , fidzwp  ,& ! current weighted confidence 
    ...
    DO ivss = 1 , nvss
      ...
      DO WHILE (lchange)
        ...
        DO irep = irepb(ivss) , irepe(ivss)-1
          idif1 = NINT( ife* ABS( zsgbdy(iairls(isortsl(irep  ),1,3),nbsp)     &
                                 -zsgbdy(iairls(isortsl(irepre),1,3),nbsp) )   &
                       -ifb* ABS( zsgbdy(iairls(isortsl(irep  ),1,3),nbsp)     &
                                 -zsgbdy(iairls(isortsl(ireprb),1,3),nbsp) ) )
          idif2 = NINT( ife* ABS( zsgbdy(iairls(isortsl(irep+1),1,3),nbsp)     &
                                 -zsgbdy(iairls(isortsl(irepre),1,3),nbsp) )   &
                       -ifb* ABS( zsgbdy(iairls(isortsl(irep+1),1,3),nbsp)     &
                                 -zsgbdy(iairls(isortsl(ireprb),1,3),nbsp) ) )
...
! if the extrapolation is not well conditioned
            exrefd = NINT(ABS( zsghed(nobm1,nhtime) -zsghed(nobm2,nhtime)) *c60)
...
! ---------------------------------------------------------------------------
    ...
    mzmlhd (ntotaml,nhqofl) = mzsghd (itsl,nhqofl)
...
SUBROUTINE obs_air_list ( ixcand , ncanda , ncande                             &
                        , ixrest , nresta , nreste , lhflag , ixrequ , nrequ )
...
! ---------------------------------------------------------------
      ...
      IF (mxcand == 9) THEN
        ...
        ELSE
          IF (BTEST( mosghd(ncand,nhschr), nvhtbp ))          kcdtyp = -2
          IF (BTEST( mosghd(ncand,nhschr), nvhhbp ))          kcdtyp = -3
          IF (BTEST( mosghd(ncand,nhschr), nvrdbp ))          kcdtyp = -4
          IF (BTEST( mosghd(ncand,nhschr), nvalbp ))          kcdtyp = -4
        ENDIF
