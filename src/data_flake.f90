...
REAL (KIND = wp),     PARAMETER ::         &
  ...
  C_T_max       = 0.8_wp           , & ! Maximum value of the shape factor 
                                           !    C_T (thermocline)
  Phi_T_pr0_1   = 40._wp/3._wp , & ! Constant in the expression for the 
                                           !    T shape-function derivative 
  Phi_T_pr0_2   = 20._wp/3._wp , & ! Constant in the expression for the 
                                           !    T shape-function derivative 
  C_TT_1        = 11._wp/18._wp, & ! Constant in the expression for 
                                           !    C_TT (thermocline)
  C_TT_2        = 7._wp/45._wp , & ! Constant in the expression for 
                                           !    C_TT (thermocline)
  C_B1          = 2._wp/3._wp  , & ! Shape factor (upper layer of 
                                           !    bottom sediments)
  C_B2          = 3._wp/5._wp  , & ! Shape factor (lower layer of 
                                           !    bottom sediments)
  Phi_B1_pr0    = 2._wp            , & ! B1 shape-function derivative 
                                           !
  C_S_lin       = 0.5_wp           , & ! Shape factor (linear temperature 
                                           !    profile in the snow layer)
  Phi_S_pr0_lin = 1._wp            , & ! S shape-function derivative 
                                           !    (linear profile) 
  C_I_lin       = 0.5_wp           , & ! Shape factor (linear temperature 
                                           !    profile in the ice layer)
  Phi_I_pr0_lin = 1._wp            , & ! I shape-function derivative 
                                           !    (linear profile) 
  Phi_I_pr1_lin = 1._wp            , & ! I shape-function derivative 
                                           !    (linear profile) 
  Phi_I_ast_MR  = 2._wp            , & ! Constant in the MR2004 expression 
                                           !    for I shape factor
  C_I_MR        = 1._wp/12._wp , & ! Constant in the MR2004 expression
                                           !    for I shape factor
  H_Ice_max     = 3._wp                ! Maximum ice tickness in the 
...
REAL (KIND = wp),     PARAMETER ::       &
  tpl_grav          = 9.81_wp      , & ! Acceleration due to gravity 
                                           !        [m s^{-2}]
  tpl_T_r           = 277.13_wp    , & ! Temperature of maximum density 
                                           !        of fresh water [K]
  tpl_T_f           = 273.15_wp    , & ! Fresh water freezing point [K]
  tpl_a_T           = 1.6509E-05_wp, & ! Constant in the fresh-water 
                                           !        equation of state [K^{-2}]
  tpl_rho_w_r       = 1.0E+03_wp   , & ! Maximum density of fresh water 
                                           !        [kg m^{-3}]
  ...
  tpl_L_f           = 3.3E+05_wp   , & ! Latent heat of fusion 
                                           !   [J kg^{-1}]
  tpl_c_w           = 4.2E+03_wp   , & ! Specific heat of water 
                                           !   [J kg^{-1} K^{-1}]
  tpl_c_I           = 2.1E+03_wp   , & ! Specific heat of ice 
                                           !   [J kg^{-1} K^{-1}]
  tpl_c_S           = 2.1E+03_wp   , & ! Specific heat of snow 
                                           !   [J kg^{-1} K^{-1}]
