...
!------------------------------------------------------------------------------
  ...
  z1dgdt=1._wp/(g*pdt2)   ! pdt2=1*dt for first model time step
  ...
  IF (lmfdd) THEN     ! downdraft calculations switched on
    ...
    DO i = isc, iec
      IF(llo2(i)) THEN
        ...
        zbuo   = zttest*(1._wp+rvd_m_o*zqtest)- ztenh(i,k)*(1._wp+rvd_m_o*zqenh(i,k))
    ...
    DO i = isc, iec
      IF(locum(i)) THEN ! for convective grid points only
        ...
        pdtlhn_con(i,k) =    g/zrhgdz(i,k)/cp_d  &
                         * (-1._wp) * lh_mp*( zmful(i,k+1) - zmful(i,k) &
    ...
    DO i = isc, iec
      IF (locum(i)) THEN ! for convective grid points only
        ...
        pdtke_con(i,k) = MAX( 0.0_wp, g*r_d/pph(i,k) * &
                         ( zcvfl_s/cp_d + &
                           ztenh(i,k)*zcvfl_q*rvd_m_o/(1.0_wp-rvd_m_o*zqenh(i,k)) ) )
        ! RA
      END IF
    END DO
  END DO
...
!<DM+AS
    ...
    DO i = isc, iec
      ...
      IF(loflag(i)) THEN
        ...
        plu  (i,k) =  zmfulk*( 1._wp/MAX(cmfcmin,pmf_u(i,k)) )
        pqv_u(i,k) =  zmfuqk*( 1._wp/MAX(cmfcmin,pmf_u(i,k)) )
        pt_u (i,k) = (zmfusk*( 1._wp/MAX(cmfcmin,pmf_u(i,k)) ) -      &
                                  pfih(i,k))/cp_d  
    ...
    DO i = isc, iec
      IF( loflag(i)) THEN
        IF( pqv_u(i,k).NE.zqold(i) ) THEN
          ...
          ENDIF
#endif
          zbuo      =  pt_u(i,k)*(1._wp+rvd_m_o*pqv_u(i,k))  &
                      - pth(i,k)*(1._wp+rvd_m_o*pqvh(i,k))
      ...
      DO i = isc, iec
        IF( loflag(i) ) THEN
          ...
          IF( pmf_u(i,k) > 0.0_wp ) THEN
            ptr_u(i,k,iztr) = zmfutr(i,iztr)*(1._wp/pmf_u(i,k))
          ENDIF
        ENDIF
      ENDDO
    ENDDO
      ...
      DO i = isc, iec
        IF( loflag(i) ) THEN
          ...
          zmfuv(i)= zmfuv (i)  + zdmfen(i)*pv(i,k)  &
                               - zdmfde(i)*pv_u(i,k+1)
          IF( pmf_u(i,k) > 0.0_wp ) THEN
            pu_u(i,k) = zmfuu(i)*(1._wp/pmf_u(i,k))
            pv_u(i,k) = zmfuv(i)*(1._wp/pmf_u(i,k))
          ENDIF
        ENDIF
      ENDDO
    ENDIF
...
!DM+AS>
        ...
        qdvdt_m = fr_wat*qdvdt_w + (1._wp-fr_wat)*qdvdt_i
        zcond1  = (pqv(i)-qs_m)/(1._wp+qdvdt_m)
