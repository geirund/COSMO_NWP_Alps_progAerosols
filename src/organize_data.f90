...
!LS, b
    lrelax,       & ! relaxation towards reference profile
!LS, e
!SBoeing, b
    llsf,         & ! large scale forcings for idealized cases
!SBoeing, e
...
USE src_artifdata,      ONLY:  input_artifctl, gen_ini_data, gen_bound_data,  &
                               gen_trcr_data, open_relax_profile
...
!ROA someone should check this
      ! read relaxation profile
      IF (lrelax) THEN
         CALL open_relax_profile(.FALSE.)
      ENDIF
...
ELSEIF (yaction == 'boundary') THEN
  ...
  PREFETCH : IF (ldo_prefetch) THEN
    ...
    OPEN : IF (.NOT. lread_file_open) THEN
      ...
      ! check whether the input file exists (for lockfiles used at MeteoSwiss
      ! there is no guarantee that the files are present at model startup
      ! even if the READY-file mechanisms is disabled)
      IF (lzexist) THEN
        ...
        IF (ytrans_in == '   ') THEN
          IF (my_cart_id == 0) THEN
            IF (izdebug >= 10) PRINT *, '  CHECKING prefetch file '
            INQUIRE (FILE=yname, EXIST=lzexist)
          ENDIF
          IF (num_compute > 1) THEN
            ! Distribute lzexist to all nodes
            CALL distribute_values (lzexist, 1, 0, imp_logical, icomm_cart, ierror)
      ...
      ELSE
        ...
        IF (my_cart_id == 0 .AND. izmpedbg > 1) THEN
          PRINT*,'PREFETCHING skipped: input file does not exist: ', yname(1:LEN_TRIM(yname))
        ENDIF
...
! Variables for default values
  ...
  LOGICAL                    ::       &
    ...
    lbdsst_d,      & ! T_S boundary data are used only over the sea
                     ! (SST is not maintained constant during the integration)
    llockfiles_d     ! indicates whether to use lockfiles or not
...
IF (my_world_id == 0) THEN
  ...
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                            'llockfiles',   llockfiles,   llockfiles_d,' L '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
...
!!$ FOR NOW, SET TO 15 MINUTES INSTEAD OF 5, BUT 5 MINUTES MAY BE COMMENTED IN LATER ON!
  IF ( hincbound /= hincbound_d) THEN
    IF ( ABS(NINT(hincbound) - hincbound) > 1.0E-5_wp) THEN
...
!      IF ( (hincbound /= 0.50_wp) .AND. (hincbound /= 0.25_wp) ) THEN
      IF ( ABS((htest) - NINT(htest)) > 1.0E-5_wp .OR. &
!!$           (htest > 0.5_wp .AND. MOD(60_iintegers,NINT(htest)*5.0_wp) /= 0) ) THEN
  ...
  ELSE  
    ...
    ELSEIF ( (hcomb(1) /= r_huge) .OR. (hgrib(1) /= r_huge) ) THEN
      ...
      DO i = 1, nc_hcomb
        ...
        IF ( ABS(NINT(htest) - htest) > 1.0E-5_wp) THEN
...
! Subroutine / Function arguments
! Scalar arguments with intent(in):
REAL    (KIND=wp)       , INTENT(IN)    :: hour(noutst_max),     hour_comb(ntrip)
