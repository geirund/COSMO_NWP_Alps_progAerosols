...
! Modules used:
  ...
  USE data_parameters, ONLY :   &
       ...
       dp,        & ! KIND-type parameter for real variables
...
! LS, b
       lrelax,       & !relaxation towards profile 
       lrelax_soil,  & !relaxation of soil water towards profile 
! LS, e
! SBoeing, b
       llsf
! SBoeing, ie
...
! -------------
    ...
    qcprof     ,    & ! cloud water from given profile
    qiprof     ,    & ! cloud ice from given profile
...
! -----------------------------------------------------
       ...
       h_b1_lk   ,     & ! thickness of the upper layer
                         ! of bottom sediments                           (  m  )
! LS, b
       w_so_r    ,     & ! reference soil water                       (m H2O)
! LS, e
...
!------------------------------------------------------------------------------
  ...
  REAL (KIND=wp),     PARAMETER         ::   pt00 = 1e5_wp
  ...
  LOGICAL                    ::       &
       ...
       linit_realoro, &
       lraso_theta_ini      ! if .true., theta and not temperature is specified in raso file
  ...
  CHARACTER(len=250) :: rasofile, orofile, z0file, frlandfile, &
       ...
       forefile, fordfile, fluxfile, soilinitfile, relaxfile, lsffile, soilrelaxfile
  ...
  LOGICAL :: llatentflux_fix   ! In case of lsensiflux_fix=.true., turn on also a fixed latent heat flux at the surface 
                               ! (for lsoil=.false.)
  LOGICAL :: lsensiflux_tdep   ! In case of lsensiflux_tdep=.true., turn on also a prescribed time dependent sensible heat flux at the surface (for lsoil=.false.)
  LOGICAL :: llatentflux_tdep  ! In case of llatentflux_tdep=.true., turn on also a prescribed time dependent latent heat flux at the surface (for lsoil=.false.)
...
!!!namelisttag   used by the ed-script ed_addnl.in which automatically adds nlparams
  !SBoeing, b
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: fluxes_time  ! Array for storing time of sensible heat flux
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: sensiflux_arr   ! Array for storing time-dependent sensible heat flux
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: latentflux_arr  ! Array for storing time-dependent latent heat flux
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: lsf_time  ! Array for storing time of large scale forcings
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: thetatlsf_mat   ! Array for storing large scale forcing, temperature 
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: qvlsf_mat  ! Array for storing large scale forcing, vapor 
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: ugeo_mat   ! Array for storing large scale forcing, u 
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: vgeo_mat  ! Array for storing large scale forcing, v 
  REAL(KIND=wp),ALLOCATABLE,DIMENSION(:) :: wsub_mat  ! Array for storing large scale forcing, w subsidence
  INTEGER(KIND=iintegers) :: nfluxes_time  ! Length of array for storing time of sensible heat flux
  INTEGER(KIND=iintegers) :: nlsf_time  ! Length of array for storing time of large scale forcings
  !SBoeing, e
...
CONTAINS
  ...
  SUBROUTINE input_artifctl (nuspecif, ierrstat)
    ...
    LOGICAL                    ::       &
         ...
         linit_realoro_d, lraso_theta_ini_d
    ! i-Shift of the model domain center w.r.t. the center of the input orography data set from file
    ...
    CHARACTER(len=250) :: rasofile_d, orofile_d, z0file_d, frlandfile_d, &
         ...
         forefile_d, fordfile_d, fluxfile_d, soilinitfile_d, relaxfile_d, lsffile_d, soilrelaxfile_d
    ...
    LOGICAL :: llatentflux_fix_d   ! In case of lsensiflux_fix=.true., turn on also a fixed latent heat flux at the surface 
                                   ! (for lsoil=.false.)    
    LOGICAL :: lsensiflux_tdep_d   ! Switch to turn on time dependent sensible and latent heat fluxes at the surface (for lsoil=.false.)
    LOGICAL :: llatentflux_tdep_d   ! In case of lsensiflux_tdep=.true., turn on also a time dependent latent heat flux at the surface (for lsoil=.false.)
...
!!!defaulttag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    NAMELIST /ARTIFCTL/ ivctype, irefatm, p0sl, t0sl, dt0lp, delta_t, h_scal, &
         ...
         rasofile, fluxfile, itype_artifprofiles, itype_anaprof_tqv, itype_anaprof_uv ,&
         ...
         lai_c , rootdp_c , for_e_c , for_d_c , soilinitfile, &
         relaxfile, lsffile, soilrelaxfile, &
         ...
         lsensiflux_fix , llatentflux_fix , lsensiflux_tdep , llatentflux_tdep , sensiflux_c , & 
         latentflux_LzuS , H0_rel_noise , iseed_noise_H0 , &
...
!!!cinitag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    lsensiflux_tdep_d  = .FALSE.
    llatentflux_tdep_d  = .FALSE.
...
!!!adddefaulttag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    fluxfile_d(:) = ' ';   fluxfile_d  = 'noname.dat'
    soilinitfile_d(:) = ' '; soilinitfile = 'noname.dat'
    relaxfile_d(:) = ' '; relaxfile = 'noname.dat'
    lsffile_d(:) = ' '; lsffile = 'noname.dat'
    soilrelaxfile_d(:) = ' '; soilrelaxfile = 'noname.dat'
    lraso_theta_ini_d = .FALSE.
    ...
    rasofile =           rasofile_d
    fluxfile =           fluxfile_d
    soilinitfile =       soilinitfile_d
    relaxfile =          relaxfile_d
    lsffile =            lsffile_d
    soilrelaxfile =      soilrelaxfile_d
    lraso_theta_ini =    lraso_theta_ini_d
    hcond_on =           hcond_on_d
    ...
    lsensiflux_tdep  = lsensiflux_tdep_d
    llatentflux_tdep  = llatentflux_tdep_d
...
!!!setdefaulttag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    IF (my_world_id == 0) THEN
      ...
      IF  (iz_err == 0) THEN
          ...
          CASE (3)
            ...
            DO k=1, nlayers_nconst-1
              ...
              IF (N_nconst(k) < bvref_tconst) THEN
                IF (ABS(N_nconst(k)) > 1e-12_wp) THEN
        ...
        !   turned off:
        IF (lsoil .AND. (lsensiflux_fix .OR. lsensiflux_tdep)) THEN
          WRITE (*,*) 'ERROR: in input_artifctl(): (both lsensiflux_fix=.true. or lsensiflux_tdep=.true.) and lsoil=.true. not allowed!'
          iz_err = 1033
        END IF

        !.. Constant surface fluxes can only be specified if .not. (itype_turb=3 .and. itype_tran=3):
        IF (itype_turb == 3 .AND. itype_tran == 3 .AND.  (lsensiflux_fix .OR. lsensiflux_tdep)) THEN
          WRITE (*,*) 'ERROR: in input_artifctl(): lsensiflux_fix=.true. or lsensiflux_tdep=.true. together with itype_turb=3 .and. itype_tran=3 not working!'
          iz_err = 1033
        END IF
...
!!!charbuftag   used by the ed-script ed_addnl.in which automatically adds nlparams
        ...
        nlbuf = nlbuf +  1;  logbuf  (nlbuf) = lraso_theta_ini
        ...
        nlbuf = nlbuf +  1;  logbuf  (nlbuf) = lsensiflux_tdep
        nlbuf = nlbuf +  1;  logbuf  (nlbuf) = llatentflux_tdep
...
!!!charbufbacktag   used by the ed-script ed_addnl.in which automatically adds nlparams
        ...
        nlbuf = nlbuf +  1;  lraso_theta_ini = logbuf (nlbuf)
        ...
        nlbuf = nlbuf +  1;  lsensiflux_tdep    = logbuf  (nlbuf)
        nlbuf = nlbuf +  1;  llatentflux_tdep    = logbuf  (nlbuf)
...
!!!logbufbacktag   used by the ed-script ed_addnl.in which automatically adds nlparams
      ...
      CALL distribute_values(fluxfile, LEN(fluxfile), 0, imp_character, icomm_world, ierr)
      CALL distribute_values(vcoordvec, nvcoordvec_max, 0, imp_reals, icomm_world, ierr)
      CALL distribute_values(soilinitfile, LEN(soilinitfile), 0, imp_character, icomm_world, ierr)
      CALL distribute_values(relaxfile, LEN(relaxfile), 0, imp_character, icomm_world, ierr)
      CALL distribute_values(lsffile, LEN(lsffile), 0, imp_character, icomm_world, ierr)
      CALL distribute_values(soilrelaxfile, LEN(soilrelaxfile), 0, imp_character, icomm_world, ierr)
      CALL distribute_values(vcoordvec, nvcoordvec_max, 0, imp_reals, icomm_world, ierr)
...
!!!distribbacktag   used by the ed-script ed_addnl.in which automatically adds nlparams
    ...
    IF ( my_world_id == 0 ) THEN
      ...
      WRITE (nuspecif, '(T8,A,T59,A,/,T21,A,/,T21,A)')                       &
           'fluxfile', ' C ', TRIM(fluxfile) , TRIM(fluxfile_d)     
      WRITE (nuspecif, '(T8,A,T59,A,/,T21,A,/,T21,A)')                       &
           'soilinitfile', ' C ', TRIM(soilinitfile) , TRIM(soilinitfile_d)  
      WRITE (nuspecif, '(T8,A,T59,A,/,T21,A,/,T21,A)')                       &
           'relaxfile', ' C ', TRIM(relaxfile) , TRIM(relaxfile_d)
      WRITE (nuspecif, '(T8,A,T59,A,/,T21,A,/,T21,A)')                       &
           'lsffile', ' C ', TRIM(lsffile) , TRIM(lsffile_d)
      WRITE (nuspecif, '(T8,A,T59,A,/,T21,A,/,T21,A)')                       &
           'soilrelaxfile', ' C ', TRIM(soilrelaxfile) , TRIM(soilrelaxfile_d)
      WRITE (nuspecif, '(T8,A,T31,L12,T50,L12,T69,A3)')                      &
           'lraso_theta_ini', lraso_theta_ini, lraso_theta_ini_d    ,' L '
      WRITE (nuspecif, '(T8,A,T21,I12,T40,I12,T59,A)')                       &
      ...
      WRITE (nuspecif, '(T8,A,T21,L12,T40,L12,T59,A)')                       &
           'lsensiflux_tdep', lsensiflux_tdep, lsensiflux_tdep_d     , ' L '
      WRITE (nuspecif, '(T8,A,T21,L12,T40,L12,T59,A)')                       &
           'llatentflux_tdep', llatentflux_tdep, llatentflux_tdep_d     , ' L '
      WRITE (nuspecif, '(T8,A,T21,F12.4,T40,F12.4,T59,A)')                       &
...
!!!nuspecif2tag   used by the ed-script ed_addnl.in which automatically adds nlparams
  ...
  SUBROUTINE gen_ini_data 
    ...
    REAL (KIND=wp)             ::       &
         ...
         zvco_z50(51),      & ! pre-specified vertical coordinates of 50 z-layer LM
         zrhf   (ie,je,ke), & !
         zrhs   (ie,je),    & !
    ...
    zvco_z50 = (/ 22000.0_wp, 21000.0_wp, 20028.5703_wp, 19085.3594_wp, 18170.0_wp, &
                  17282.1406_wp, 16421.4297_wp, 15587.5_wp, 14780.0_wp, 13998.5703_wp, &
                  13242.8594_wp, 12512.5_wp, 11807.1367_wp, 11126.4297_wp, 10470.0_wp, &
                   9837.5_wp, 9228.5703_wp, 8642.8594_wp, 8080.0_wp, 7539.6367_wp, &
                   7021.4297_wp, 6525.0_wp, 6050.0_wp, 5596.0664_wp, 5162.8594_wp, &
                   4750.0_wp, 4357.1367_wp, 3983.9299_wp, 3630.0_wp, 3295.0_wp, &
                   2978.5701_wp, 2680.3601_wp, 2400.0_wp, 2137.1399_wp, 1891.4299_wp, &
                   1662.5_wp, 1450.0_wp, 1253.5698_wp, 1072.8599_wp, 907.5_wp, &
                    757.1399_wp, 621.4299_wp, 500.0_wp, 392.5_wp, 298.5698_wp, &
                    217.86_wp, 150.0_wp, 94.64_wp, 51.43_wp, 20.0_wp, 0.0_wp /)
    ...
    ELSE IF ( vcoord%ivctype == 2 ) THEN ! Height-based hybrid Gal-Chen coordinate
      ...
      CASE('predefined', 'PREDEFINED', 'Predefined')
        ...
        ELSEIF (ke == 40) THEN
          ...
          ENDDO
       ELSEIF ( ke == 50 ) THEN
!ROA: has only the name changed from vcoord_in to vcoord or is there
!     somthing else to do here?
          !vcoord_in%vcflat = 11357.0_wp
          vcoord%vcflat = 11357.0_wp
          DO k = 1, ke1
             !vcoord_in%vert_coord(k) = zvco_z50(k)
...
!US    IF ( ANY(soiltyp < 1.0_wp) .OR. ANY(soiltyp >= 9.0_wp) ) THEN
    ...
    !------------------------------------------------------------------------------
    ! 5. Initialization of the wind fields and thermodynamic variables.
    !
    ...
    CASE (1, 3, 4)
      ...
      ELSE
        ...
        CALL read_raso(rasofile, zml, hhl, t(:,:,:,nnew), zrhf, &
             u(:,:,:,nnew), v(:,:,:,nnew), &
             ps(:,:,nnew), t_s(:,:,nnew), zrhs, &
             integral_averaging, lps_from_file, rasofile_t_is_theta, qc)
    ...
    !***********************************************************************************
    ! read the time dependent surface fluxes
    !***********************************************************************************

    IF (lsensiflux_tdep) THEN
      CALL readfluxes(fluxfile)
    END IF

    IF (llsf) THEN
      CALL readlsf(lsffile)
    END IF
...
! LS, b
    !------------------------------------------------------------------------------
    ! 6.b Read in reference profile
    !------------------------------------------------------------------------------

    IF (lrelax) CALL open_relax_profile(ltheta_ini)
! LS, e
    !------------------------------------------------------------------------------
    ...
    IF (lsoil) THEN
      ...
      CASE (3)
        ...
        !.. Initialize soil temperature and water saturation
        !   as well as snow parameters and interception store
        !   with horizontally homogeneous values from an ASCII-file

        CALL init_tw_soil_snow_ascii_1d()
      ...
      !------------------------------------------------------------------------------
      ! 7.b Store reference profile for soil moisture relaxation
      !------------------------------------------------------------------------------

      IF (lrelax_soil) THEN
        DO k=1,ke_soil+1
          w_so_r(k) = w_so(1,1,k,nnew)
        ENDDO
      ENDIF
    ...
    IF (lmulti_layer) THEN
      ...
      WRITE (*,*)
      WRITE (*,*)  'Initial Profiles of soil variables (cell center) at point (1,1):'
      WRITE (*,'(A3,3A12)') 'k', 'czmls', 't_so', 'w_so'
      DO k = 1,ke_soil+1
        WRITE(*,'(I3,3F12.4)') k, czmls(k), t_so(1,1,k,nnew), w_so(1,1,k,nnew)
      ENDDO
      WRITE (*,*)
      WRITE (*,*) 't_so(1,1,k=0): ', t_so(1,1,0,nnew) 
      WRITE (*,*)
    ENDIF
    ENDIF
  ...
  CONTAINS
    ...
    SUBROUTINE init_tw_soil_snow_c()
      ...
      IF (lmulti_layer) THEN
        ...
        IF (t_soil_c < 100.0_wp) THEN
          DO k = 0, ke_soil+1
            WHERE (llandmask)
              t_so(:,:,k,nnew) = t_s(:,:,nnew) + t_soil_c
            END WHERE
          END DO
        ELSE
          DO k = 0, ke_soil+1
    ...
    SUBROUTINE init_tw_soil_snow_ascii_2d()
      ...
      REAL(KIND=wp)     :: zt_s(ie,je), zt_snow(ie,je), zwf_soil(ie,je)
    ...
    SUBROUTINE init_tw_soil_snow_ascii_1d()

      IMPLICIT NONE

      REAL(kind=wp) :: t_so_in(ke_soil+1), wf_so_in(ke_soil+1)
      INTEGER (KIND=iintegers)     :: ios, funit, num_headerlines

      ! Number of header lines expected in the data file:
      num_headerlines = 4

      IF (my_cart_id == 0 .AND. ldebug_artif .AND. idbg_artif_level > 0) THEN
        WRITE (*,*)  'Subr. init_tw_soil_snow_ascii_1d() ...'
      END IF

      IF (lmulti_layer) THEN
        w_so_ice(:,:,:,nnew) = 0.0_wp

        IF ( my_cart_id == 0 ) THEN
          CALL get_free_unit (funit)
          OPEN(unit=funit, file=soilinitfile, status='old', action='read', iostat=ios)
          IF ( ios /= 0 ) THEN
            WRITE(*,*) "Could not open soil profile :-( (Required by artifdata)"
            STOP
          ENDIF
          ...
          DO k=1,ke_soil+1
            READ(funit, *) t_so_in(k), wf_so_in(k)
          ENDDO
        ...
        IF ( num_compute > 1 ) THEN
          CALL distribute_values (t_so_in, ke_soil+1,0,imp_reals,icomm_cart,izerror)
          CALL distribute_values (wf_so_in, ke_soil+1,0,imp_reals,icomm_cart,izerror)
        ENDIF
            
        ! T in the soil:
        DO k = 1, ke_soil+1
          IF (t_so_in(k) < 100.0_wp) THEN
            t_so(:,:,k,nnew) = t_s(:,:,nnew) + t_so_in(k)
          ELSE
            t_so(:,:,k,nnew) = t_so_in(k)
          ENDIF
        END DO
        t_so(:,:,0,nnew) = t_s(:,:,nnew)

        ! Water content of soil layers:
        DO k=1,ke_soil+1
          DO j=1,je
            DO i=1,ie
              w_so(i,j,k,nnew) = wf_so_in(k) * cporv(INT(soiltyp(i,j),iintegers)) * (czhls(k)-czhls(k-1))
            END DO
          END DO
          w_so_ice(:,:,k,nnew) = 0.0_wp
        ENDDO
      ...
      ! Deep soil temperature:
      IF ( ALLOCATED ( t_cl ) ) THEN
        t_cl(:,:) = t_so_in(ke_soil+1)
      END IF
      ...
      ! set variable for soil water relaxation
      IF (lrelax_soil) THEN
        DO k=1,ke_soil+1
          w_so_r(k) = w_so(1,1,k,nnew)
        ENDDO
    ...
    END SUBROUTINE init_tw_soil_snow_ascii_1d
  ...
  SUBROUTINE gen_bound_data ( naction )
    ...
    ! LS, b
    INTEGER (KIND=iintegers)     :: ios
    ! === Reading of boundary data
    REAL (KIND=wp), dimension(ke)     :: pp_bd_in,t_bd_in,qv_bd_in,u_bd_in,v_bd_in,qc_bd_in
    REAL (KIND=wp), dimension(ke1)    :: w_bd_in
    REAL (KIND=wp), dimension(ke_soil+1):: t_so_in
    !LS, e
...
!      IF (lrelax) THEN 
! LS, b 
! Write out the boundary data for later restart 
!  IF ( my_cart_id == 0 ) THEN
!     open(unit=40,file="bd_fields.dat",status="replace",action="write")
!     DO k=1,ke
!        write(40,'(2f15.9,4f13.9)') pp_bd(4,4,k,1),t_bd(4,4,k,1),&
!        u_bd(4,4,k,1),v_bd(4,4,k,1),qv_bd(4,4,k,1),qc_bd(4,4,k,1)
!     ENDDO
!     close(40,status="keep")
!     IF ( .NOT. lw_freeslip ) THEN
!        open(unit=40,file="bd_field_w.dat",status="replace",action="write")
!        DO k=1,ke1
!           write(40,'(f13.9)') w_bd(4,4,k,1)
!        ENDDO
!        close(40,status="keep")
!     ENDIF
!  ENDIF
...
!!! before convection is triggered.
  ...
  CONTAINS
    ...
    ! Read soil temperature and water saturation from ASCII-file

    SUBROUTINE relax_tw_soil_ascii_1d()
      ...
      IF (my_cart_id == 0 .AND. ldebug_artif .AND. idbg_artif_level > 0) THEN
        WRITE (*,*)  'Subr. relax_tw_soil_ascii_1d() ...'
      END IF
      ...
      IF (lmulti_layer) THEN
        ...
        ! Water content of soil layers:
        DO k=1,ke_soil+1
            w_so_r(k) = wf_so_in(k) * cporv(INT(soiltyp(1,1),iintegers)) * (czhls(k)-czhls(k-1))
        ENDDO
    ...
    END SUBROUTINE relax_tw_soil_ascii_1d
  ...
  SUBROUTINE add_orography_ana(ierrstat)
    ...
    REAL(KIND=wp)           :: &
         ...
         zdi, tmpasym_x, tmpasym_y, zdx_rot, zdy_rot, zhx, zhy, zsideradius
    ...
    DO ii=1, nhill_max
      ...
      CASE ('cos-plateau')
          ...
          !=====================================================================
          ! 
          ! hills with flat tops and cos-shaped slopes
          ! used for the T-REX valley wind model intercomparison
          ! Schmidli et al. 2011, MWR
          ! 

          ! Relevant namelist parameters:
          !
          !   - hill_i(ii)          i-position of mountain center
          !   - hill_j(ii)          j-position of mountain center
          !   - hillheight(ii)      mountain height in m  -> zhmax
          !   - hill_width_x(ii)    x-direction half width of the hill top in m
          !   - hill_width_y(ii)    y-direction half width of the hill top in m
          !   - hillsideradius_y(ii) width of the hill slopes
          ! 
          !=====================================================================
          ...
          IF (lhill_2d(ii)) THEN
            ...
            DO i = 1, ie
              ...
              ! i-position relative to moutain center in GPs
              zdi  = ABS(REAL(i_td-hill_i(ii)+i,wp))

              DO j = 1, je

                zdx   = r_earth * dlon * degrad * crlat(j,1)
                zdax = hill_width_x(ii) / zdx                 ! mountain width X in GPs incl.
                                                            ! grid stretching factor
                zsideradius = hillsideradius_y(ii) / zdy

                IF (zdi <= zdax) THEN
                   zhx = 1.0_wp
                ELSEIF (zdi > zdax .AND. zdi < zdax + zsideradius) THEN
                   zhx = 0.5_wp + 0.5_wp*cos(pi*(zdi-zdax)/zsideradius)
                ELSE
                   zhx = 0.0_wp
                END IF

                hsurf_inc(i,j) = (zhmax+zhillcut) * zhx - zhillcut
            ...
            DO i = 1, ie
              DO j = 1, je
                ...
                IF (zdx_rot >= 0.0_wp) THEN
                  ! we are on the right flank of the mountain - here the asymmetry-parameter takes effect:
                ...
                END IF
                zsideradius = hillsideradius_y(ii)

                zdx_rot = ABS(zdx_rot)
                zdy_rot = ABS(zdy_rot)

                IF (zdy_rot <= zday) THEN
                   zhy = 1.0_wp
                ELSEIF (zdy_rot > zday .AND. zdy_rot < zday + zsideradius) THEN
                   zhy = 0.5_wp + 0.5_wp*cos(pi*(zdy_rot-zday)/zsideradius)
                ELSE
                   zhy = 0.0_wp
                END IF

                IF (zdx_rot <= zdax) THEN
                   zhx = 1.0_wp
                ELSEIF (zdx_rot > zdax .AND. zdx_rot < zdax + zsideradius) THEN
                   zhx = 0.5_wp + 0.5_wp*cos(pi*(zdx_rot-zdax)/zsideradius)
                ELSE
                   zhx = 0.0_wp
                END IF
                ...
                IF (lhill_2d(ii)) THEN
                  hsurf_inc(i,j) = (zhmax+zhillcut) * zhx - zhillcut
                ELSE
                  hsurf_inc(i,j) = (zhmax+zhillcut) * zhy * zhx - zhillcut
                END IF
  ...
  SUBROUTINE read_raso(rasofile, zml, hhl, temperatur, relfeuchte, &
       ...
       integral_averaging, pflag, thetaflag,qc)
    ...
    REAL(KIND=wp),     INTENT(out), DIMENSION(ie,je,ke), OPTIONAL :: qc
    ...
    IF (my_cart_id == 0 .OR. num_compute == 1) THEN
      ...
      !AP>>
      !not needed for my atmos.input file
      !num_headerlines = num_headerlines + 1_iintegers
      !AP<<
...
!APTEST<<
      ...
      ALLOCATE(pin(anz), zin(anz), tempin(anz), taupin(anz), &
           relfin(anz), spezin(anz), qcin(anz), wgin(anz), wrin(anz), uin(anz), vin(anz))
      ...
      DO i=1, anz
        READ(freieunit,*)                            &
             ...
             relfin(i), spezin(i), qcin(i), wgin(i), wrin(i)
      END DO
    ...
    IF (num_compute > 1) THEN
      ...
      CALL distribute_values(qcin, anz, 0, imp_reals, icomm_cart, mpierror)
...
!APTEST
PRINT *, "minval zin=", MINVAL(zin)
PRINT *, "minval hhl=", MINVAL(hhl(:,:,ke+1))
PRINT *, "number datalines=",anz
PRINT *, "number headerlines=",num_headerlines
PRINT *,"zin=",zin
!APTEST
    ...
    ELSE
      ...
      DO j = 1, je
        DO i = 1, ie
          ...
          IF (ierr /= 0) fehler = 1
!APTEST>>
          CALL linear_interpol_vec(zin, qcin, anz, zml_v, &
               qc(i,j,:), ke, ierr)
          IF (ierr /= 0) fehler = 1
!APTEST<<
        END DO
...
!!!=============================================================================
  ...
  !
  !         lsensiflux_tdep:      Master switch, turn on time dependent sensible heat flux H0
  !
  !         llatentflux_tdep:     If lsensiflux_tdep=.true., turn on/off latent heat flux
  !
...
!!! timelevel for leapfrog integration for ntstep = 0 ?
  ...
  SUBROUTINE set_idealized_surffluxes()
    ...
    INTEGER(kind=iintegers) :: i, j, iu, jv, izerror, tindex_smaller, tindex_larger
    ...
    REAL(kind=wp) :: zvbke, ztvb, ztch, tnow, dtp, dtm
    ...
    IF (lsensiflux_fix .or. lsensiflux_tdep) THEN
      ...
      IF(lsensiflux_tdep) THEN
        tnow=dt*ntstep
        write(*,*) 'Tnow,dt,ntstep',tnow,dt,ntstep
        tindex_smaller=1
        tindex_larger=2
        DO WHILE(tnow>fluxes_time(tindex_larger) .and. tindex_larger<nfluxes_time)
          tindex_larger=tindex_larger+1
        END DO
        tindex_smaller=tindex_larger-1
        dtm = (tnow-fluxes_time(tindex_smaller))/(fluxes_time(tindex_larger)-fluxes_time(tindex_smaller))
        dtp = (fluxes_time(tindex_larger)-tnow)/(fluxes_time(tindex_larger)-fluxes_time(tindex_smaller))

        IF(tnow<fluxes_time(nfluxes_time)) THEN
          sensifluxtmp =h0noise*(sensiflux_arr(tindex_smaller)*dtp+sensiflux_arr(tindex_larger)*dtm)
          IF (my_cart_id == 0) THEN
            WRITE (*,*) '        Sensible heat flux without noise', sensiflux_arr(tindex_smaller)*dtp+sensiflux_arr(tindex_larger)*dtm, ' W/m^2'
          END IF
        ELSE
          sensifluxtmp =h0noise*sensiflux_arr(nfluxes_time)
          IF (my_cart_id == 0) THEN
            WRITE (*,*) '        Sensible heat flux without noise', sensiflux_arr(nfluxes_time), ' W/m^2'
          END IF
        ENDIF
      ELSE
      ...
      DO j=1,je
        ...
        DO i=1,ie
          ...
          tch(i,j) = ABS(sensifluxtmp(i,j)) / ( ps(i,j,nnow)/(r_d*t_g(i,j,nnow)*(1.0_wp + rvd_m_o*qv_s(i,j,nnow))) * &
               ...
               cp_d * MAX(ABS(sensifluxtmp(i,j))*zscal_tdiff, 1e-5_wp) )
        END DO
      END DO
      ...
      IF (llatentflux_fix .or. llatentflux_tdep) THEN
        IF(llatentflux_tdep) THEN
          IF(tnow<fluxes_time(nfluxes_time)) THEN
            latentfluxtmp =h0noise*(latentflux_arr(tindex_smaller)*dtp+latentflux_arr(tindex_larger)*dtm)
            IF (my_cart_id == 0) THEN
              WRITE (*,*) '        Latent heat flux without noise', (latentflux_arr(tindex_smaller)*dtp+latentflux_arr(tindex_larger)*dtm), ' W/m^2'
            END IF
          ELSE
            latentfluxtmp =h0noise*latentflux_arr(nfluxes_time)
            IF (my_cart_id == 0) THEN
              WRITE (*,*) '        Latent heat flux without noise', latentflux_arr(nfluxes_time),' W/m^2'
            END IF
          ENDIF
        ELSE
...
!!!=============================================================================
  ...
  SUBROUTINE add_noise_tw(nx)
    ...
    INTEGER(KIND=iintegers)        :: i, j, k, izerror, kzdims(24), k_bot, k_top
    REAL(KIND=wp),     ALLOCATABLE :: tmpnoise(:,:,:), tmpnoise1(:,:,:),windnoise(:,:,:),windnoise1(:,:,:)
    REAL(KIND=wp)                  :: ztnew,fac
    CHARACTER (LEN=250)            :: yzerrmsg
    ...
    IF (ladd_noise_t .AND. ntstep_noise == ntstep) THEN
      ...
      ALLOCATE(tmpnoise1(ie,je,ke))
      ALLOCATE(windnoise(ie,je,ke))
      ALLOCATE(windnoise1(ie,je,ke))
      ...
      CALL gen_bubnoise(fnoise=tmpnoise1(:,:,:), kdim=ke, seed=iseed_noise_t+5)
      CALL gen_bubnoise(fnoise=windnoise(:,:,:), kdim=ke, seed=iseed_noise_w)
      CALL gen_bubnoise(fnoise=windnoise1(:,:,:), kdim=ke, seed=iseed_noise_w+5)
      ...
      ! Version by Linda Schlemmer:
      ! temperature perturbation added only to the lowest atmospheric level
!APTEST>>
!For stratocumulus turbulence is not triggered from surface. Therefore this perturbation is set to 0
!      DO j = 1, je
!        DO i = 1, ie
!          ztnew = tmpnoise(i,j,ke) * dT_noise * 1.0_wp
!          t(i,j,ke,nx) = t(i,j,ke,nx) + ztnew
!        END DO
!      END DO
!APTEST<<
!      CALL gen_bubnoise(fnoise=tmpnoise(:,:,:), kdim=ke, seed=iseed_noise_w)
!APTEST>>
!For stratocumulus use alternative description below
...
!APTEST>> Implement noise formulation for T and w in cloud layer to aid built-up of turbulence in this layer.
!Formulation of noise shape follows MIMICA model (contact person: Annica Ekman)

k_bot=62     !170
k_top=85     !177

DO k = k_bot,k_top
   fac=1.0*(k_top-k)/(1.0*(k_top-k_bot)) !perturbation is largest at cloud base
   IF (my_cart_id == 0) THEN
     PRINT*,"APTEST: fac=",fac," and k=",k
   END IF
   DO j = 1, je
       DO i = 1, ie
         ztnew=fac*dT_noise*max(min(sqrt(-2.*log(max(tmpnoise1(i,j,k),1.e-10)))*sin(2.*pi*tmpnoise(i,j,k))/sqrt(20.),1.),-1.)
         t(i,j,k,nx) = t(i,j,k,nx) + ztnew
         ztnew=fac*dW_noise*max(min(sqrt(-2.*log(max(windnoise1(i,j,k),1.e-10)))*sin(2.*pi*windnoise(i,j,k))/sqrt(20.),1.),-1.)
         w(i,j,k,nx) = w(i,j,k,nx) + ztnew
       END DO
   END DO
END DO
!APTEST<<
      ...
      ! ..clean up memory:
      DEALLOCATE(tmpnoise)
      DEALLOCATE(tmpnoise1)
      DEALLOCATE(windnoise)
      DEALLOCATE(windnoise1)
...
!roa: allocate also on other PEs with (1,1) dim to avoid null pointer error
    ELSE
    ...
    DO k = 1, kdim
      ...
      IF ( num_compute > 1 ) THEN
!APTEST        WRITE (*,*) 'distributing noise...'
...
!APTEST        WRITE (*,*) '... done'
    ...
    ! clean up memory:
!roa: you also then have to deallocate it on all PEs
    DEALLOCATE(bub_glob_tnoisebuffer)
...
!==============================================================================
  SUBROUTINE calc_p_hydrostat_psts(ni,nj,nk,niter, &
    ...
    !.. Local variables:
    REAL(KIND=wp)     :: piterold(ni,nj,nk)
    REAL(KIND=wp)     :: relhum_lim(ni,nj,nk)
    ...
    REAL(KIND=dp)     :: one_m_wgtfac, Tp_k_avg, pp_k_avg, T_hl, p_hl, rho_inv_k_avg, &
                         p0_p_k_avg, T_T0_k_avg, p_inv_k_avg, T0_inv_k_avg, q_x_k_avg, buoy
    REAL(KIND=dp)     :: piterdp(ni,nj,nk),piterolddp(ni,nj,nk)
    INTEGER :: iter2
...
!APTEST>>      qc(:,:,:) = 0.0_wp
    ...
    eps_pp = 1.0e-2_wp  ! required absolute accuracy of the pressure iteration, [Pa]
    ...
    DO iter = 1, niter
      ...
      DO j = 1,nj
        DO i = 1,ni
          ...
          IF (ltheta_ini) THEN
            ! "moist" r_d:
            zrdm      = dble(rd_moist(qv(i,j,nk),qc(i,j,nk)))
            ! COSMO-approximation of cp:
            zcpm      = dble(cp_moist_cosmo(qv(i,j,nk),qc(i,j,nk),0.0_wp))
            t(i,j,nk) = dble(theta(i,j,nk))*(piterdp(i,j,nk)/pt00)**(zrdm/zcpm)
          END IF
...
!APTEST>>            qc(i,j,nk) = MAX ( 0.0_wp ,  MIN(qv(i,j,nk), zmaxqv) - zsqv )
          ELSE
          ...
          piterdp(i,j,nk) = psurf(i,j) * EXP((hsurf(i,j)-zml(i,j,nk))*g/(ztvw*r_d))            
      ...
      DO k = nk,2, -1
        ...
        DO j = 1,nj
          DO i = 1,ni
            ...
            IF (ltheta_ini) THEN
              ! "moist" r_d:
              zrdm       = dble(rd_moist(qv(i,j,k-1),qc(i,j,k-1)))
              ! COSMO-approximation of cp:
              zcpm       = dble(cp_moist_cosmo(qv(i,j,k-1),qc(i,j,k-1),0.0_wp))
              t(i,j,k-1) = dble(theta(i,j,k-1))*(piterdp(i,j,k-1)/pt00)**(zrdm/zcpm)
            END IF
...
!APTEST>>              qc(i,j,k-1) = MAX ( 0.0_wp ,  MIN(qv(i,j,k-1), zmaxqv) - zsqv )
            ELSE
            ...
            IF (itype_fastwaves == 1) THEN
              ...
              zpa   = piterdp(i,j,k) - p0(i,j,k)
              piterdp(i,j,k-1) = p0(i,j,k-1) + ( zpa + zrhogdh*(2._wp-t0(i,j,k-1)/ztvw1 &
                   - t0(i,j,k)/ztvw*(1._wp + zpa/p0(i,j,k)))) &
                   / (1._wp + zrhogdh*t0(i,j,k-1)/(ztvw1*p0(i,j,k-1)))

            ELSE IF (itype_fastwaves == 2) THEN

              one_m_wgtfac = 1.0_dp - dble(wgtfac(i,j,k))

              DO iter2=1,5

              Tp_k_avg = dble(wgtfac(i,j,k)) * ( dble(t(i,j,k))   - dble(t0(i,j,k))  -dble(t(i,j,k-1)) + dble(t0(i,j,k-1)) )  +  &
                   &      ( dble(t(i,j,k-1)) - dble(t0(i,j,k-1)) )

              pp_k_avg = dble(wgtfac(i,j,k)) * ( dble(piterdp(i,j,k))   - dble(p0(i,j,k)) - dble(piterdp(i,j,k-1)) + dble(p0(i,j,k-1)) )  +  &
                   &      ( dble(piterdp(i,j,k-1)) - dble(p0(i,j,k-1)) )

              T_hl = dble(t0hl(i,j,k)) + Tp_k_avg
              p_hl = dble(p0hl(i,j,k)) + pp_k_avg

              zrdm       = dble(rd_moist(qv(i,j,k-1),qc(i,j,k-1)))
              rho_inv_k_avg   = ( dble(zrdm) * dble(T_hl) ) / dble(p_hl)

              p0_p_k_avg   = 1.0_dp / ( 1.0_dp + dble(pp_k_avg) / dble(p0hl(i,j,k)) )
              T_T0_k_avg   = 1.0_dp + dble(Tp_k_avg)/ dble(t0hl(i,j,k))
              p_inv_k_avg  = 1.0_dp / ( dble(p0hl(i,j,k)) + dble(pp_k_avg) )
              T0_inv_k_avg = 1.0_dp / dble(t0hl(i,j,k))

              q_x_k_avg = dble(wgtfac(i,j,k)) * ( dble(rvd_m_o)*dble(qv(i,j,k))   - dble(qc(i,j,k)) -dble(rvd_m_o)*dble(qv(i,j,k-1)) + dble(qc(i,j,k-1))  )  +  &
                           ( dble(rvd_m_o)*dble(qv(i,j,k-1)) - dble(qc(i,j,k-1)) )

              buoy =  dble(g) * ( dble(p0_p_k_avg) * (dble(T0_inv_k_avg) * dble(Tp_k_avg)+dble(T_T0_k_avg) * dble(q_x_k_avg))         &
                            - dble(p_inv_k_avg) * dble(pp_k_avg) )

              zpadp = dble(p0(i,j,k)) - dble(p0(i,j,k-1))

              piterdp(i,j,k-1) = piterdp(i,j,k) - dble(zpadp) + dble(buoy) / ( dble(rho_inv_k_avg) * dble(sqrtg(i,j,k)) )
        ...
        IF ( ANY( ABS(piter(i,j,:) - piterold(i,j,:)) > eps_pp ) ) THEN
          ...
          IF (ltheta_ini) THEN
            WRITE (*,'(a)') 'k     zml(k) theta(k) relhum(k)     t(k) piter(k)      '// &
                 'qv(k)        qc(k) relhum_out(k)'
            DO k=1,nk
              WRITE (*,'(i4,1x,f8.1,1x,f8.1,1x,f8.5,1x,f8.1,2(1x,f18.6),2(1x,es12.5),1x,f8.5)') &
                   k,zml(i,j,k),theta(i,j,k),relhum(i,j,k),t(i,j,k),&
                   piter(i,j,k),piterold(i,j,k),qv(i,j,k),qc(i,j,k), &
...
!=====================================================================================
  ...
  SUBROUTINE calc_p_hydrostat_ana(ni, nj, nk, niter, &
    ...
    INTEGER(KIND=iintegers) ::  i, j, k, iter, iter2
    LOGICAL :: ltheta_ini

    eps_pp = 1.0e-2_wp   ! Pa, required abs. accuracy of the below pressure iteration
...
!!$--------------- inlined code from geo_heading --------------------------------
      ...
      IF (ABS(cangle) < 1e-20_wp) cangle = 1e-20_wp
...
!=================================================================================
  ...
  SUBROUTINE output3d_ascii(zfield, nk, fieldname, fieldcomment, fieldunit, ylevtyp)
    ...
    REAL(KIND=irealgrib) :: d32bit = 0.0_wp
    ...
    IF (num_compute > 1) THEN
      ...
      DO k=1,nk
        ...
        IF (my_cart_id == 0) THEN
          ...
          DO j=1,je_tot
            DO i=1,ie_tot
              IF (ABS(zfield_glob(i,j)) >= 1e-30_wp) THEN
    ...
    ELSE
      IF (my_cart_id == 0) THEN
        DO k=1,nk
          ...
          DO j=1,je
            DO i=1,ie
              IF (ABS(zfield(i,j,k)) >= 1e-30_wp) THEN
...
SUBROUTINE open_relax_profile(lztheta_ini)
...
!============================================================================
  ...
  LOGICAL, INTENT(IN) :: lztheta_ini

  INTEGER           :: i,j,k
  INTEGER (KIND=iintegers)                :: ios, izerror
  REAL (KIND=wp):: zpl(ke) ! total pressure
  REAL(KIND=wp) :: ptr, &  ! pressure at tropopause in Pa
       b,                   &  ! parameter for error fct 
       zmaxqv,              &
       efct(ke)                ! factor to enter error fct

  REAL(KIND=wp), dimension(ie,je,ke) ::&
       zml,                 & ! height of main levels for mass points
       zrhf,                &
       pproft,              &
       tproft,              &
       qvproft,             &
  ...
  REAL(KIND=wp), dimension(ie,je) ::&
       zrhs,                &
       pst,                 &  ! temporary surface pressure
       t_st                    ! temporary surface temperature
  ...
  CHARACTER(len=250)          ::  yzerrmsg
   ...
   yzroutine = "open_relax_profile"
   izerror   = 0_iintegers
   yzerrmsg  = ""

    IF (my_cart_id == 0 .AND. ldebug_artif .AND. idbg_artif_level > 0) THEN
      WRITE (*,*) 'Subr. open_relax_profile() ...'
    END IF



    ! Retrieve the microphysics tracers
    CALL trcr_get( izerror, 'QV', ptr_tlev = nnew, ptr = zqv )
    IF ( izerror /= 0_iintegers ) THEN
      yzerrmsg = trcr_errorstr( izerror )
      CALL model_abort( my_cart_id, izerror, yzerrmsg, 'set_tempdist' )
    ENDIF
  ...
  zml(:,:,:) = 0.5_wp*( hhl(:,:,1:ke) + hhl(:,:,2:ke+1) )
  IF ((relaxfile.ne.rasofile) .OR. (nstart > 0)) THEN
    IF (my_cart_id == 0) THEN
       IF (relaxfile.ne.rasofile) print*,'ATTENTION!! relaxfile different from input file'
       IF (nstart > 0) print*,'read in reference profile'
    ENDIF
    qcproft(:,:,:) = 0.0_wp
    zmaxqv = HUGE(1.0_wp)

    CALL read_raso(rasofile, zml, hhl, tproft(:,:,:), zrhf, &
         uproft(:,:,:), vproft(:,:,:), &
         pst(:,:), t_st(:,:), zrhs, .true., .false., .false.)

!ROA: someone should check if this still makes sense...
    SELECT CASE (itype_fast_waves)
    CASE (1)
      IF (lztheta_ini) THEN
        ! theta_ini is OPTIONAL; IF present, this is taken instead of temperature, 
        ! and temperature is calculated from theta_ini      
        CALL calc_p_hydrostat_psts(ni=ie, nj=je, nk=ke, niter=40, &
             zml=zml, sqrtg=sqrtg_r_w(:,:,1:ke),hsurf=hsurf, psurf=pst(:,:), &
             t=tproft(:,:,:), relhum=zrhf, t0=t0, p0=p0 ,rho0=rho0, &
             piter=pproft(:,:,:), qv=qvproft(:,:,:), qc=qcproft(:,:,:), &
             zmaxqv=zmaxqv, r_d=r_d, rvd_m_o=rvd_m_o, &
             itype_fastwaves=itype_fast_waves, &
             theta=theta_ini )
      ELSE
    ...
    DO k=1,ke
       pprof(k)  = pproft(5,5,k)
       tprof(k)  = tproft(5,5,k)
       qvprof(k) = qvproft(5,5,k)
       uprof(k)  = uproft(5,5,k)
       vprof(k)  = vproft(5,5,k)
    ENDDO
  ELSE
    DO k=1,ke
       pprof(k)  = pp(5,5,k,nnew) + p0(5,5,k)
       tprof(k)  = t(5,5,k,nnew)
       qvprof(k) = zqv(5,5,k)
       uprof(k)  = u(5,5,k,nnew)
       vprof(k)  = v(5,5,k,nnew)
    ENDDO
 ...
 ENDIF
    ...
    IF (my_cart_id == 0) THEN
      WRITE (*,*)
      WRITE (*,*)  'Relaxation Profiles on full levels at point (1,1):'
      WRITE (*,'(A3,6A12)') 'k', 'zml', 'T', 'p', &
           'qv', 'u', 'v'
      DO k = ke,1,-1
        WRITE(*,'(I3,F12.1,F12.7,F12.2,3F12.7)'),     &
             k, zml(5,5,k), &
             tprof(k),  pprof(k),qvprof(k), uprof(k), vprof(k)
      ENDDO
      WRITE (*,*)
...
!     calculate height-dependent relaxationfct using errorfct

  ! Characteristics of the atmosphere
  ptr  = 50000.0_wp !erstmal so setzen, später ändern
  b=30000.0_wp


  DO k = 1,ke

     efct(k)=(ptr - pprof(k))/b
     
     call ERRORFCT(efct(k),efctout(k))
     
  ENDDO
  IF ( my_cart_id == 0 ) THEN
     OPEN(unit=34,file="efct_500_300.dat",status="replace",action="write")

     do k=1,ke

        write(34,'(2f10.7,i4,f12.2,2f12.7)')  efctout(k),(1.0_wp+efctout(k))/2.0_wp,k,pprof(k),tprof(k),zml(5,5,k)
     enddo
...
END SUBROUTINE open_relax_profile

!---------------------------------------------------------------------
SUBROUTINE relax_profile (ttensr,t_tot,qvtensr,qv_tot,&
     !qctensr,qc_tot,qitensr,qi_tot,&
     utensr,u_tot,vtensr,v_tot)
...
!============================================================================
  ...
  REAL(KIND=wp) ::             &
       t_tot(ie_tot,je_tot,ke),    &
       qv_tot(ie_tot,je_tot,ke),   &
       !qc_tot(ie_tot,je_tot,ke),  &
       !qi_tot(ie_tot,je_tot,ke),  &
       u_tot(ie_tot,je_tot,ke),    &
       v_tot(ie_tot,je_tot,ke)
...
REAL(KIND=wp) ::     &
       pmean(ke),        &  ! mean pressure
       tmean(ke),        &  ! mean temperature
       qvmean(ke),       &  ! mean temperature
       !qcmean(ke),       &  ! mean temperature
       !qimean(ke),       &  ! mean temperature
       umean(ke),        &  ! mean temperature
       vmean(ke),        &  ! mean temperature
       tau                  ! time scale


       INTEGER (KIND=iintegers):: ios,ind


  ! Set time constant for relaxation

  tau=24.0_wp*3600.0_wp


!----------------------------------------------------------------------------
! Begin Subroutine relax_profile
! Relax the simulated atmospheric profile consecutively towards and observed profile.
! The forcing is height dependent.
! Following the approach introduced in Raymond and Zeng (2005)  
!----------------------------------------------------------------------------
  ! initialize fields
  ...
  ! Total pressure and mean temperature
  DO k = 1, ke
     ...
     do j=1,je_tot
        do i=1,ie_tot
           ...
           tmean(k)=tmean(k)+t_tot(i,j,k)
           qvmean(k)=qvmean(k)+qv_tot(i,j,k)
           !qcmean(k)=qcmean(k)+qc_tot(i,j,k)
           !qimean(k)=qimean(k)+qi_tot(i,j,k)
           ind=ind+1

           umean(k)=umean(k)+u_tot(i,j,k)

           vmean(k)=vmean(k)+v_tot(i,j,k)
        enddo
     enddo
        
     tmean(k)=tmean(k)/real(ind,wp)
     qvmean(k)=qvmean(k)/real(ind,wp)
     !qcmean(k)=qcmean(k)/real(ind,wp)
     !qimean(k)=qimean(k)/real(ind,wp)
     umean(k)=umean(k)/real(ind,wp)
     vmean(k)=vmean(k)/real(ind,wp)
  ...
  DO k = 1,ke 
     ...
     ttensr(k)=-(1.0_wp/tau)*((1.0_wp+efctout(k))/2.0_wp)*(tmean(k)-tprof(k))
     qvtensr(k)=-(1.0_wp/tau)*((1.0_wp+efctout(k))/2.0_wp)*(qvmean(k)-qvprof(k))
!    ttensr(k)=-(1.0_wp/tau)*(tmean(k)-tprof(k))!height-independent
!    qvtensr(k)=-(1.0_wp/tau)*(qvmean(k)-qvprof(k))!height-independent
!    qctensr(k)=-(1.0_wp/tau)*((1.0_wp+efctout(k))/2.0_wp)*(qcmean(k)-qcprof(k))
!    qitensr(k)=-(1.0_wp/tau)*((1.0_wp+efctout(k))/2.0_wp)*(qimean(k)-qiprof(k))

     utensr(k)=-(1.0_wp/tau)*((1.0_wp+efctout(k))/2.0_wp)*(umean(k)-uprof(k))
     vtensr(k)=-(1.0_wp/tau)*((1.0_wp+efctout(k))/2.0_wp)*(vmean(k)-vprof(k))
!    utensr(k)=-(1.0_wp/tau)*(umean(k)-uprof(k))!height-independent
!    vtensr(k)=-(1.0_wp/tau)*(vmean(k)-vprof(k))!height-independent

  ENDDO

END SUBROUTINE relax_profile

         SUBROUTINE ERRORFCT(X,ERR)
!
!       =========================================
...
!        REAL(KIND=wp), parameter :: PI=3.141592653589793
        ...
        ELSE
           ...
           ERR=1.0_wp-C0*ER
           IF (X.LT.0.0_wp) ERR=-ERR
        ENDIF
        END SUBROUTINE ERRORFCT
...
!==============================================================================
  ...
  !**********************************************************************************
  !
  ! Read prescribed fluxes from a text file and distribute it to all processors. 
  ! Only processor 0 reads the file.
  !
  ! The text file is expected to have a certain format: 
  !   4 - TEST THIS - header lines, arbitrary number of data lines as follows:
  ! 
  ! #
  ! #
  ! #                           
  ! time [s], SHF [W m^{-2}], LHF [W m^{-2}]
  !  0.00000     -30.0     5.0
  !  14400.0      90.0   250.0
  !  23400.0     140.0   450.0
  !  27000.0     140.0   500.0
  ...
  ! ----------------------------------
  !
  ! fluxfile (CHAR):         Name of the text file (input)
  !
  !**********************************************************************************

 SUBROUTINE readfluxes(fluxfile)

    IMPLICIT NONE

    ! Input/Output parameters:
    CHARACTER(*), INTENT(in) :: fluxfile

    ! Local variables:
    INTEGER(KIND=iintegers) :: num_headerlines, freieunit, ios, i, &
         fehler, ierr, mpierror, index
    CHARACTER(len=20) :: zeile

    IF (my_cart_id == 0 .AND. ldebug_artif .AND. idbg_artif_level > 0) THEN
      WRITE (*,*)  'Subr. readfluxes() ...'
    END IF
    ...
    IF (my_cart_id == 0 .OR. num_compute == 1) THEN
      ...
      IF (freieunit == -1) THEN
        WRITE (*,*) 'ERROR: problem in readfluxes(): no free file unit available! Abort!'
        CALL model_abort (my_world_id, 10001, 'no free file unit available', &
             'readfluxes, getting free file unit')
        RETURN
      ENDIF

      OPEN(unit=freieunit, file=TRIM(fluxfile), status='old', iostat=ios)
      IF (ios /= 0) THEN
        WRITE (*,*) 'ERROR: problem in readfluxes(): no such flux-file! Abort!'
        CALL model_abort (my_world_id, 10002, 'no flux-file '//TRIM(fluxfile), &
             'readfluxes, openinig flux-file')
        RETURN
      END IF
      ...
      ! Determine number number of data lines:
      !  (NOTE: the above extra header line has already been read!)
      nfluxes_time = 0_iintegers
      DO
        READ(freieunit, *, iostat=ios)
        IF (ios < 0) EXIT
        nfluxes_time = nfluxes_time + 1_iintegers
      END DO
      ...
      ALLOCATE(fluxes_time(nfluxes_time), sensiflux_arr(nfluxes_time), latentflux_arr(nfluxes_time))
      ...
      DO i=1, nfluxes_time
        READ(freieunit,*)                            &
             fluxes_time(i), sensiflux_arr(i), latentflux_arr(i)
      END DO
      CLOSE(freieunit)
      CALL release_unit(freieunit)

      IF (abs(fluxes_time(1)) > 1e-6) THEN
        WRITE (*,*) 'ERROR: problem in readfluxes(): time does not start at zero'
        CALL model_abort (my_world_id, 10002, ' time does not start at zero in '//TRIM(fluxfile), &
             'read_flux, reading flux-file')
        RETURN
      END IF
    END IF
    ...
    IF (num_compute > 1) THEN
      ! Distribute the data to all PEs:
      CALL distribute_values(nfluxes_time,    1, 0, imp_integers, icomm_cart, mpierror)

      IF (my_cart_id /= 0) THEN
        ALLOCATE(fluxes_time(nfluxes_time), sensiflux_arr(nfluxes_time), latentflux_arr(nfluxes_time))
      END IF

      CALL distribute_values(fluxes_time,    nfluxes_time, 0, imp_reals, icomm_cart, mpierror)
      CALL distribute_values(sensiflux_arr,  nfluxes_time, 0, imp_reals, icomm_cart, mpierror)
      CALL distribute_values(latentflux_arr, nfluxes_time, 0, imp_reals, icomm_cart, mpierror)
    END IF

  END SUBROUTINE readfluxes
! ==============================================================================

 SUBROUTINE readlsf(lsffile)

    IMPLICIT NONE

    ! Input/Output parameters:
    CHARACTER(*), INTENT(in) :: lsffile
    CHARACTER(1) :: chmess1

    ! Local variables:
    INTEGER(KIND=iintegers) :: num_headerlines, freieunit, ios, i, k, ik, &
         fehler, ierr, mpierror, ntotal, index
    CHARACTER(len=20) :: zeile
    ...
    IF (my_cart_id == 0 .OR. num_compute == 1) THEN
      ...
      IF (freieunit == -1) THEN
        WRITE (*,*) 'ERROR: problem in readlsf(): no free file unit available! Abort!'
        CALL model_abort (my_world_id, 10001, 'no free file unit available', &
             'readlsf, getting free file unit')
        RETURN
      ENDIF

      OPEN(unit=freieunit, file=TRIM(lsffile), status='old', iostat=ios)
      IF (ios /= 0) THEN
        WRITE (*,*) 'ERROR: problem in readlsf(): no such lsf-file! Abort!'
        CALL model_abort (my_world_id, 10002, 'no lsf-file '//TRIM(lsffile), &
             'readlsf, opening lsf-file')
        RETURN
      END IF

      ! Determine number of lines (lines starting with #):
      num_headerlines = 0
      DO
        zeile(:) = ' '
        READ(freieunit, '(a)', iostat=ios) zeile
        ...
        ENDIF
      END DO
      nlsf_time=num_headerlines
      write(*,*) 'num_headerlines',num_headerlines
      ! anz is now the number of data lines!
      !=====================================

      REWIND(freieunit)

      ALLOCATE(lsf_time(nlsf_time),thetatlsf_mat(nlsf_time*ke),qvlsf_mat(nlsf_time*ke),&
      ugeo_mat(nlsf_time*ke),vgeo_mat(nlsf_time*ke),wsub_mat(nlsf_time*ke))


      DO I=1,nlsf_time
        chmess1 = "#"
        ios = 1 ! not zero
        !searich for the next line consisting of "# time", from there onwards the profiles will be read
        DO WHILE (.not.(chmess1 == "#" .and. ios ==0)) 
          read(freieunit,*,iostat=ios) chmess1,lsf_time(i)
          IF (ios < 0) THEN
            stop 'STOP: No time dependend data for end of run'
          END IF
        END DO
        WRITE(*,*) 'lsf_time = ',lsf_time(i)
        DO K=1,KE
          ik=(i-1)*ke+k
          READ(freieunit,*) &
            thetatlsf_mat(ik),qvlsf_mat(ik),ugeo_mat(ik),vgeo_mat(ik),wsub_mat(ik)
        END DO
        DO K=1,KE
          ik=(i-1)*ke+k
          WRITE(*,'(4e12.4)') &
      ...
      IF (abs(lsf_time(1)) > 1e-6) THEN
        WRITE (*,*) 'ERROR: problem in readlsf(): time does not start at zero'
        CALL model_abort (my_world_id, 10002, ' time does not start at zero in '//TRIM(lsffile), &
             'read_lsf, reading lsf-file')
        RETURN
      END IF
    END IF
    ...
    IF (num_compute > 1) THEN
      ! Distribute the data to all PEs:
      CALL distribute_values(nlsf_time,    1, 0, imp_integers, icomm_cart, mpierror)
      IF (my_cart_id /= 0) THEN
        ALLOCATE(lsf_time(nlsf_time),thetatlsf_mat(nlsf_time*ke),qvlsf_mat(nlsf_time*ke),&
        ugeo_mat(nlsf_time*ke),vgeo_mat(nlsf_time*ke),wsub_mat(nlsf_time*ke))
      END IF
      ntotal=nlsf_time*ke
      CALL distribute_values( lsf_time,     nlsf_time, 0, imp_reals, icomm_cart, mpierror)
      CALL distribute_values(thetatlsf_mat, nlsf_time*ke, 0, imp_reals, icomm_cart, mpierror)
      WRITE(*,*) 'ddd1'
      CALL distribute_values(qvlsf_mat,nlsf_time*ke, 0, imp_reals, icomm_cart, mpierror)
      WRITE(*,*) 'ddd2'
      CALL distribute_values( ugeo_mat,nlsf_time*ke, 0, imp_reals, icomm_cart, mpierror)
      WRITE(*,*) 'ddd3'
      CALL distribute_values( vgeo_mat,nlsf_time*ke, 0, imp_reals, icomm_cart, mpierror)
      WRITE(*,*) 'ddd4'
      CALL distribute_values( wsub_mat,nlsf_time*ke, 0, imp_reals, icomm_cart, mpierror)
      WRITE(*,*) 'ddd5'
    END IF

  END SUBROUTINE readlsf

SUBROUTINE lsf_profile(thetatensr,qvtensr,ugeotensr,vgeotensr,wsubtensr)

!============================================================================
! Execute the large scale forcings, SBoeing
! Note that potential temperature tendencies are prescribed...
  IMPLICIT NONE
  ...
  INTEGER(kind=iintegers) :: k , tindex_smallerk, tindex_largerk, nlsf_timek
  INTEGER(kind=iintegers) :: tindex_smaller, tindex_larger
  INTEGER(kind=iintegers), SAVE :: firstcall = 0
  REAL(kind=wp) :: tnow, dtp, dtm
  ...
  tindex_smaller=1
  tindex_larger=2
  DO WHILE(tnow>lsf_time(tindex_larger) .and. tindex_larger<nlsf_time)
    tindex_larger=tindex_larger+1
  END DO
  tindex_smaller=tindex_larger-1
  dtm = (tnow-lsf_time(tindex_smaller))/(lsf_time(tindex_larger)-lsf_time(tindex_smaller))
  dtp = (lsf_time(tindex_larger)-tnow)/(lsf_time(tindex_larger)-lsf_time(tindex_smaller))

  IF(tnow<lsf_time(nlsf_time)) THEN
    DO k = 1,ke 
      tindex_smallerk=(tindex_smaller-1)*ke+k
      tindex_largerk=(tindex_larger-1)*ke+k
      thetatensr(k)=thetatlsf_mat(tindex_smallerk)*dtp+thetatlsf_mat(tindex_largerk)*dtm
      qvtensr(k)=qvlsf_mat(tindex_smallerk)*dtp+qvlsf_mat(tindex_largerk)*dtm
      ugeotensr(k)=ugeo_mat(tindex_smallerk)*dtp+ugeo_mat(tindex_largerk)*dtm
      vgeotensr(k)=vgeo_mat(tindex_smallerk)*dtp+vgeo_mat(tindex_largerk)*dtm
      wsubtensr(k)=wsub_mat(tindex_smallerk)*dtp+wsub_mat(tindex_largerk)*dtm
    ENDDO
  ELSE
    DO k = 1,ke 
      nlsf_timek=(nlsf_time-1)*ke+k
      thetatensr(k)=thetatlsf_mat(nlsf_timek)
      qvtensr(k)=qvlsf_mat(nlsf_timek)
      ugeotensr(k)=ugeo_mat(nlsf_timek)
      vgeotensr(k)=vgeo_mat(nlsf_timek)
      wsubtensr(k)=wsub_mat(nlsf_timek)
    ENDDO
  ENDIF

END SUBROUTINE lsf_profile
! SBoeing, e
