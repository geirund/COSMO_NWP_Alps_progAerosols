  ...
  IF (lzcalc_mu) THEN
    ...
    IF (idebug > 500) THEN
      DO j = 1, jdim
        DO i = 1, idim
          WRITE (*,*)	&
!          WRITE (*,'(A,A,2I3,A,I2,A,F6.1,A,I2,A,I2,A,I2)')     &!HH commented this line according to DL suggestion
...
!local variables
INTEGER (KIND=iintegers) ::  i, j, k    
REAL    (KIND=wp)        ::  zpvs, zp, zqvs, zdtr
!------------------------------------------------------------------------------
...
!!$      nos = 8.E5_wp  ! now computed below!
      nog = 4.E6_wp
      p_r = (7.0_wp+mue_rain_c) / (4.0_wp+mue_rain_c)
      p_s = (2.0_wp*bms+1._wp)/(bms+1._wp)
      p_g = (2.0_wp*bmg+1._wp)/(bmg+1._wp)
...
!!$                                (ams*nos*gamma_fct(bms+1.))**(-p_s)
      z_s = mom_fak*ams**2 * gamma_fct(2.0_wp*bms+1.0_wp) *       &
                                (ams*gamma_fct(bms+1._wp))**(-p_s)
      z_g = mom_fak*amg**2 * nog*gamma_fct(2.0_wp*bmg+1.0_wp) *       &
                                (amg*nog*gamma_fct(bmg+1._wp))**(-p_g)
      IF (idebug > 5) THEN
