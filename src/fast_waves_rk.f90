...
!CDIR ON_ADB(v)
    ...
    DO k = 1, ke
      ...
      kp1 = MIN( ke, k+1 )
      km1 = MAX( 1 , k-1 )
! <fuo_hack: Hack to mimic pre v4.9 FW-solver which had a bug that multiplied slope
!       by 0.5 for lowest model level in computation of divergence. This is accomplished
!       using zfact factor.
      zfact = 1.0_wp
      IF ( k==1 .OR. k==ke ) zfact = 0.5_wp
! fuo_hack>
...
!       using zfact factor.
            zpi(i,j,k) =                                                     &
              ...
              + zfact*( zuhl(i-1,j,k+1)-zuhl(i-1,j,k) )*zdzdx(i-1,j,k)             &
              + zfact*( zuhl(i  ,j,k+1)-zuhl(i  ,j,k) )*zdzdx(i  ,j,k) )           &
              + zfydn(j)*(  v(i,j  ,k,nnew)                                  &
              + zfact*( zvhl(i,j  ,k+1)-zvhl(i,j  ,k) )*zdzdy(i,j  ,k) )           &
              + zfyds(j)*(- v(i,j-1,k,nnew)                                  &
              + zfact*( zvhl(i,j-1,k+1)-zvhl(i,j-1,k) )*zdzdy(i,j-1,k) )
! fuo_hack>
          ENDDO
        ENDDO
      ENDIF
...
!CDIR ON_ADB(ztpten)
          DO  i = istart, iend
            ...
            IF (lbud.and.irk==irk_order) THEN
              tt_dpdt(i,j,k) = tt_dpdt(i,j,k) + ztdiv(i,j,k)/dt
              tt_adv(i,j,k)  = tt_adv(i,j,k)  + dts * zct2(i,j,k) * zwavg_s/dt
            ENDIF
            !WL2011e
...
!CDIR ON_ADB(w)
          DO  i = istart, iend
            ...
            !WL2011b
            IF (lbud.and.irk==irk_order) THEN
              tt_dpdt(i,j,k) = tt_dpdt(i,j,k) + zwdiv/dt        
              tt_adv(i,j,k)  = tt_adv(i,j,k) + dts * zct2(i,j,k) * zwavg_s/dt
            ENDIF
            !WL2011e
          ENDDO
