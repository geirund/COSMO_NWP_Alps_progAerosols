...
!   PRINT *,'callair4 ', tscale
    ...
    zfsize = MIN( 4._wp, (  MAX( wtukrsa , wtukara , tipolmx )             &
...
!   -----------------------------------------
    nelevt     =   NINT(Header%StnHtAMSL,iintegers)
...
! read GPS buffer
    ...
    ntime    = nodbuf(ipos + 8)
    taof     = REAL ( nodbuf(ipos + 9) ,wp) /60._wp
...
!     hhday = imohh + INT (taof + 24._wp + epsy )
!     x2 = 24._wp
!     hhday = MOD( hhday, x2 )
!csc --> include term 24. also for fraction of hour (for obs time < 0)
!     miday = (24._wp + taof) - INT(24._wp+taof)
!     ttday = hhday + miday
...
! ---------------
  ...
  REAL (KIND = wp)         , PARAMETER  :: &
    cseason (12)  =  (/ 0.00_wp , 0.07_wp , 0.25_wp                &
                      , 0.50_wp , 0.75_wp , 0.93_wp                &
                      , 1.00_wp , 0.93_wp , 0.75_wp                &
                      , 0.50_wp , 0.25_wp , 0.07_wp /)
...
!-------------------------------------------------------------------------------
  ...
  IF     ((ttday <   8.0_wp) .OR.  (ttday >  18.0_wp)) THEN
    obs_bias_corr_iwv =  0.2_wp
  ELSEIF ((ttday >=  8.0_wp) .AND. (ttday <  14.0_wp)) THEN
    obs_bias_corr_iwv =  0.12_wp * ttday -0.72_wp
  ELSEIF ((ttday >= 14.0_wp) .AND. (ttday <= 18.0_wp)) THEN
    obs_bias_corr_iwv = -0.12_wp * ttday +2.4_wp
  ENDIF
...
!-------------------------------------------------------------------------------
  ...
  IF ((lrepevt) .AND. (my_cart_id == 0)) THEN
    nwarn (1) = MAX( nwarn(1) , NINT(  nwarn0(1) /                          &
                         SQRT( REAL (num_compute, wp) ) + 0.49_wp ) )
