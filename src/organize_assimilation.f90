...
! lobdens_d   = .FALSE.
  ...
  gnudgtv_d   = (/0.0000_wp, 0.0000_wp, 0.0006_wp, 0.0006_wp/)
  gnudggp_d   =   0.0000_wp
  ltipol_d    = .TRUE.
  ltipsu_d    = .TRUE. 
  tipolmx_d   = 1.0_wp
...
! vpblsu_d    = (/2.0_wp, 2.0_wp, 1.0_wp, 1.0_wp/)
  ...
  rhfrtv_d    = (/1.0_wp, 1.0_wp, 1.0_wp, 0.5_wp/)
  rhfgps_d    = 0.45_wp
  rhfpsdd_d   = 1.0_wp
  cnondiv_d   = 0.1_wp
  fnondiv_d   = 0.8_wp
...
!US   yerrmsg = 'invalid obs. area'
   CALL model_abort(my_world_id, 13211, yerrmsg, yroutine)
...
! check latent heat nudging parameters, impose limits
  IF (rlhn_search > ie_tot/2._wp) THEN
    rlhn_search = INT(MIN (REAL(rlhn_search,wp),    &
                           REAL(ie_tot,wp)/2.0_wp))
  ENDIF
  IF (rlhn_search > je_tot/2._wp) THEN
    rlhn_search = INT(MIN (REAL(rlhn_search,wp),    &
