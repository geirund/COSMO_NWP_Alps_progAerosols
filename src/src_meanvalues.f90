...
!------------------------------------------------------------------------------
  ...
  DO j = jstart,jend
    DO i = istart,iend
      ...
      IF (klv850>0 .and. klv850<ke) THEN 
        zwa850  = zwa850  + ABS ( w(i,j,klv850,nnow) )
      ELSE
        zwa850  = 0.      
      ENDIF
      IF (klv500>0 .and. klv500<ke) THEN       
        zwa500  = zwa500  + ABS ( w(i,j,klv500,nnow) )
      ELSE
        zwa500  = 0.      
      ENDIF
      IF (klv300>0 .and. klv300<ke) THEN            
...
!------------------------------------------------------------------------------
  ...
  IF (my_cart_id == 0) THEN
    WRITE (nuprmass,'(I8,11F10.3)')                                         &
        ntstep , zrealdiff, zdpsdtm*100.0_wp, zpsm*0.01_wp, (zdsem- dsem0)*0.001_wp, &
        (zmsem- msem0)*0.001_wp, (zkem- kem0), zvamax, zwamax,                 &
        zwa850*100.0_wp, zwa500*100.0_wp, zwa300*100.0_wp

    WRITE (nuprhumi,'(I8,11F10.3)')                                         &
        ntstep , zqcm*1.0E5_wp, zqim*1.0E5_wp, zqrm*1.0E5_wp, zqsm*1.0E5_wp, zqgm*1.0E5_wp,&
