...
!   nvpsbp     ,& ! bit pos. for report set passive due to next 4 flags "
    ...
    IF (kfmt <= 1) THEN
      ...
      IF (kfmt == 1) THEN
        ...
        ELSEIF (lev_new) THEN
          ...
          neventd (nelodr,icma,1) =   neventd(nelodr,icma,1)                   &
    ...
    IF ((l_mask(irep)) .AND. (kproc(irep) > 0)) THEN
      ...
      ELSEIF (MOD(kproc(irep), 64) >= 32) THEN
        ...
        IF (fdbk% h(irep)% r_state <= ST_ACTIVE)                               &
          neventr (nenoal,icmaa(irep),1) = neventr(nenoal,icmaa(irep),1) + 1
      ELSEIF (MOD(kproc(irep), 32) >= 16) THEN
        ...
        IF (fdbk% h(irep)% r_state <= ST_ACTIVE)                               &
          neventr (neloca,icmaa(irep),1) = neventr(neloca,icmaa(irep),1) + 1
  ...
  DO irep = 1 , fdbk% f% n_hdr
    IF ((l_mask(irep)) .AND. (kproc(irep) == 0)) THEN
      ...
      IF     (BTEST( fdbk% h(irep)% r_flags, FL_SUSP_LOCT  )) THEN
        neventr (nedbfl,icmaa(irep),1) = neventr(nedbfl,icmaa(irep),1) + 1
      ELSEIF (BTEST( fdbk% h(irep)% r_flags, FL_SURF       )) THEN
        neventr (nezdif,icmaa(irep),1) = neventr(nezdif,icmaa(irep),1) + 1
      ...
      ELSEIF (BTEST( fdbk% h(irep)% r_flags, FL_BLACKLIST  )) THEN
        neventr (neblak,icmaa(irep),1) = neventr(neblak,icmaa(irep),1) + 1
      ELSEIF (BTEST( fdbk% h(irep)% r_flags, FL_OBSTYPE    )) THEN
        neventr (neobct,icmaa(irep),1) = neventr(neobct,icmaa(irep),1) + 1
!     ELSEIF (.NOT. lreproc) THEN
!       IF     (BTEST( fdbk% h(irep)% r_flags, FL_REDUNDANT  )) THEN
        ELSEIF (BTEST( fdbk% h(irep)% r_flags, FL_REDUNDANT  )) THEN
          neventr (neredn,icmaa(irep),1) = neventr(neredn,icmaa(irep),1) + 1
        ELSEIF (BTEST( fdbk% h(irep)% r_flags, FL_FLIGHTTRACK)) THEN
          neventr (netrac,icmaa(irep),1) = neventr(netrac,icmaa(irep),1) + 1
        ELSEIF (BTEST( fdbk% h(irep)% r_flags, FL_THIN       )) THEN
          neventr (nethin,icmaa(irep),1) = neventr(nethin,icmaa(irep),1) + 1
!       ENDIF
      ENDIF
    ENDIF
  ENDDO
