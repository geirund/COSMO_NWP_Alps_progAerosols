...
! LS, b
    wso_relax,      & !  tendency on soil water
    s_so      ,     & ! saturation                                    (1)
! LS, e
...
! --------------------------
     ...
     nincrad,     &
     ! LS, b
     lrelax_soil, & ! switch for relaxation of soil moisture !LINDA 
     lbud           ! calculate budgets
     ! LS, e
...
! ---------------------------------------------------------------
  ...
  w_so_m  (ke_soil+1),       &  ! area mean water content          (m H2O)
  dwgdt_r (ke_soil),         &  ! tendency of water content (kg/(m**3 s))
  zdwgdtm (ke_soil),         &  ! mean tendency
  f_relax (ke_soil+1),       &  ! strength of relaxation
  tau_relax                     ! time constant for the relaxation 
                                ! of soil moisture                  (s)

  REAL (KIND=wp)  :: &

  ind                           ! index for calculations of area mean


  REAL  (KIND=wp)             ::       &

    w_so_tot(ie_tot,je_tot,ke_soil)  
! LS, e
          ...
          IF (llandmask(i,j)) THEN     ! land-points only
            IF (zep_s(i,j) < 0.0_wp  &    ! potential evaporation and
              ...
              ztrang(i,j,kso) =  zr_root*zbeta             & ! reduction
                                   ...
                                   * (1._wp - fr_wi(i,j))     & ! non-water
                                   * (1._wp - fr_snow(i,j))   & ! non-snow
    ...
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN      ! land-points only
        IF (m_styp(i,j) >= 3) THEN   ! neither ice nor rock as soil type
          ...
          zagb(i,j,ke_soil_hy) = 1._wp+ zalfa*zgam2m05*z1dgam1
    ...
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        ! interpolated liquid water content at interface to layer above
          ...
          zflmg(i,j,kso) =                              &
                   (1._wp-zalfa) * zflmg(i,j,kso) + & ! explicit flux component
      ...
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        IF ((ntstep >= 1) .OR. (nincrad == 1)) THEN
          IF (hincrad /= 0.0_wp) THEN
            zradstep = NINT ((hnextrad - hincrad) * 3600.0_wp / dt)
          ELSE
...
!       Influence of heatflux through snow on total forcing:
        ...
        IF (zwsnew(i,j).GT.eps_soil) THEN
          ...
          IF(zdwsndt(i,j)-zrs(i,j)-zrr(i,j).GT.0.0_wp) THEN
            ...
            zhm_snow(i,j,1) = zhm_snow(i,j,1) - (zdwsndt(i,j)-zrs(i,j)-zrr(i,j))*zdt/rho_i/2._wp-  &
              zrs(i,j)*zdt/zrho_snowf/2._wp- zrr(i,j)*zdt/rho_i/2._wp
          ...
          ELSE
            ...
            zhm_snow(i,j,1)  = zhm_snow(i,j,1) - zrs(i,j)*zdt/zrho_snowf/2._wp- zrr(i,j)*zdt/rho_i/2._wp
            ...
            IF(w_snow_mult(i,j,1,nx) .GT. 0._wp) THEN
              ...
              zhm_snow(i,j,1)  = zhm_snow(i,j,1) - (zdwsndt(i,j)-zrs(i,j)-zrr(i,j))*zdt/rho_snow_mult(i,j,1,nx)/2._wp
      ...
      IF (llandmask(i,j)) THEN  ! for landpoints only
        IF(zwsnew(i,j) .GT. eps_soil) THEN
          IF(zwsnow(i,j) .GT. eps_soil) THEN
            ...
            IF(dz_old(i,j,1).ne.0._wp.and.rho_snow_mult(i,j,1,nnow).ne.0._wp) THEN
        ...
        IF (llandmask(i,j)) THEN  ! for landpoints only
          IF(zwsnew(i,j) .GT. eps_soil) THEN
            IF(zwsnow(i,j) .GT. eps_soil) THEN
              ...
              IF(dz_old(i,j,ksn).ne.0._wp.and.rho_snow_mult(i,j,ksn,nnow).ne.0._wp) THEN
    ...
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN          ! land-points only
        ...
        IF(zwsnew(i,j) .GE. eps_soil) THEN
          zrocs(i,j) = wliq_snow(i,j,1,nx)/zdzh_snow(i,j,1)*chc_w*rho_w + &
            (w_snow_mult(i,j,1,nx)-wliq_snow(i,j,1,nx))/zdzh_snow(i,j,1)*chc_i*rho_i
          zswitch(i,j) = MAX(-zfor_snow_mult(i,j)/50._wp/zrocs(i,j)*zdt*ke_snow, &
                              zgsb(i,j)/50._wp/zrocs(i,j)*zdt*ke_snow)
    ...
    IF (llandmask(i,j)) THEN          ! land-points only
      ...
      IF (zwsnew(i,j) > eps_soil) THEN        ! points with snow cover only
        ...
        IF (ztsnown(i,j) > t0_melt .AND. t_so(i,j,1,nnew) < t0_melt ) THEN
          zdwsnm(i,j)   = zwsnew(i,j)*.5_wp*(ztsnown(i,j) - (t0_melt - eps_temp))/ &
                          (.5_wp* (zts(i,j) - (t0_melt - eps_temp)) - lh_f/chc_i)
  ...
  DO i = istarts, iends
    IF (llandmask(i,j)) THEN  ! for landpoints only
      IF(w_snow(i,j,nnew) .GT. eps_soil) THEN
        ...
        IF(dz_old(i,j,1).ne.0._wp.and.rho_snow_mult(i,j,1,nnew).ne.0._wp) THEN
    ...
    DO i = istarts, iends
      IF (llandmask(i,j)) THEN  ! for landpoints only
        IF(w_snow(i,j,nnew) .GT. eps_soil) THEN
          ...
          zdzm_snow(i,j,ksn     ) = zhm_snow(i,j,ksn) - zhm_snow(i,j,ksn-1) ! layer thickness betw. main levels
          IF(dz_old(i,j,ksn).ne.0._wp.and.rho_snow_mult(i,j,ksn,nnew).ne.0._wp) THEN
...
IF (lrelax_soil) THEN
   ...
   f_relax(8) = 1.0000_wp
   f_relax(9) = 1.0000_wp
   f_relax(10)= 1.0000_wp

!   tau_relax=3600.0_wp/zdt ! No interaction
   tau_relax=2.0_wp*24.0_wp*3600.0_wp/zdt !time constant for the relaxation of soil moisture

   ! calculation of the mean-values for the layer and
   ! calculation of the coefficients for the relaxation
   dwgdt_r(:)=0.0_wp
   w_so_tot(:,:,:)=0.0_wp

   DO kso = 1,ke_soil
      CALL gather_field(w_so(:,:,kso,nx),ie,je,w_so_tot(:,:,kso),ie_tot,je_tot,0,ierror)
   ENDDO
      ...
      DO kso = 1,ke_soil
         ...
         do j=1,je_tot
            do i=1,ie_tot
               ...
               w_so_m(kso)=w_so_m(kso)+w_so_tot(i,j,kso)
               ind=ind+1.0_wp

            END DO
         END DO

         w_so_m(kso)  = w_so_m(kso)/ind
         dwgdt_r(kso)=-(1.0_wp/tau_relax)*(w_so_m(kso)-w_so_r(kso))*rho_w*f_relax(kso)

      ENDDO
   ENDIF
   IF ( num_compute > 1 ) THEN
      CALL distribute_values (dwgdt_r,ke_soil,0,imp_reals,icomm_cart,ierror)
   ENDIF

   ! add coefficients for the relaxation to soil water tendencies
   DO kso = 1,ke_soil
      DO   j = jstarts, jends
         DO i = istarts, iends

            IF (llandmask(i,j)) THEN  ! for landpoints only
               zdwgdt(i,j,kso)=zdwgdt(i,j,kso)+dwgdt_r(kso)
            END IF  ! land-points only
            IF (lbud) THEN
               wso_relax(i,j,kso)=dwgdt_r(kso)
            ENDIF
         END DO
      END DO
   END DO        ! soil layers
  ...
  DO   j = jstartpar, jendpar
    DO i = istartpar, iendpar
      IF (llandmask(i,j)) THEN  ! for landpoints only
        ...
        s_so(i,j,kso)=w_so(i,j,kso,nnew)*100.0_wp/(zporv(i,j)*zdzhs(kso))
