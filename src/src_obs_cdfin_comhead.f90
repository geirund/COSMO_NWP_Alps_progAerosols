...
! -------------
  ...
  INTEGER (KIND=iintegers) ::  &  !   variable ID's in NetCDF file for:
    ...
    varid_ngg   , varid_msec   ,& ! minute, second  /  time
...
!   ----------------------------------------------------------------------
    IF (zaltob(irep) <= rmdich) THEN
      IF (     ((.NOT. lupair) .OR. (kobtyp == ntemp) .OR. (kobtyp == npilot)) &
         .AND. (mpassiv(irep) == 0))                                           &
        neventr (nenoal,icma,1) = neventr(nenoal,icma,1) + 1
      IF (.NOT. lupair) THEN
        mpassiv (irep) = 2
...
!     --------------------------------------------------------------------------
      IF ((nc_locq(irep) == 2) .OR. (min_dtl(irep) > 360)                      &
        ...
        IF (mpassiv(irep) == 0)                                                &
          neventr (nedbfl,icma,1) = neventr (nedbfl,icma,1) + 1
        mpassiv (irep) = 2
...
!   ---------------------------------
    IF (      (kobtyp == nairep)                                               &
      ...
      IF (mpassiv(irep) == 0)                                                  &
        neventr (neblak,icma,1) = neventr(neblak,icma,1) + 1
      mpassiv (irep) = 2
...
!   --------------------------------------------------------------------
    IF (      (roblat <= cma(icma)%exnlat+epsy)                                &
      ...
      IF (mpassiv(irep) == 0)                                                  &
        neventr (neobct,icma,1) = neventr(neobct,icma,1) + 1
      mpassiv (irep) = 2
