...
! SBoeing, b
  ...
  thetatens_lsf, & ! t-tendency without sound-wave terms           ( K/s )
  qvtens_lsf       ! qv-tendency                                   ( 1/s )
! SBoeing, e
...
!WL2011e
  tt_adv,         & !
  tt_relax,       & ! t-tendency from relaxation                   ( K/s )
  qvt_relax,      & ! qv-tendency from relaxation                  ( 1/s )
  ut_relax,       & ! u-tendency from relaxation                   ( m/s**2 )
  vt_relax,       & ! v-tendency from relaxation                   ( m/s**2 )
!SBoeing, b
  tt_lsf,       & ! t-tendency from large scale forcing          ( K/s )
  qvt_lsf,      & ! qv-tendency from large scale forcing         ( 1/s )
  qcit_lsf,      & ! qc-tendency from large scale forcing         ( 1/s )
  ut_lsf,       & ! u-tendency from large scale forcing          ( m/s**2 )
  vt_lsf          ! v-tendency from large scale forcing          ( m/s**2 )
!Sboeing, e
! end of data_fields
...
!SBoeing, b
  llsf              !large-scale forcing terms
!Sboeing, e
...
USE src_artifdata,      ONLY: relax_profile,lsf_profile
...
USE parallel_utilities,       ONLY :  &
  ...
  distribute_values,      & ! subr. distributing a total model field to all nodes
...
USE stoch_physics,            ONLY : pertstoph
...
!    tges,                         & ! total time
!    ntreal,                       & !real timestep
!    modu_relax,                   & !modulus for relaxation

  CHARACTER (LEN=12)               ::           &
    yakdatout     ! date for relaxation (ydate_ini+ntstep/dt) in the form 
                  ! ddmmyyyyhh (day, month, year, hour)

! SBoeing, B: local variables for subsidence
   REAL  (KIND=wp)             ::       &
     subs_t, subs_qv, subs_qc
! SBoeing, E
...
!------------------------------------------------------------------------------
  ...
  ENDIF
#endif
  IF (lbud .OR. lrelax .OR. llsf) THEN
    ! get required tracers (all qx at nnew)
    CALL trcr_get(izerror, 'QV', ptr_tlev = nnow, ptr = qv_now, ptr_tens=qvtens)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'QC', ptr_tlev = nnow, ptr = qc_now, ptr_tens=qctens)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'QI', ptr_tlev = nnow, ptr = qi_now, ptr_tens=qitens)
    IF (izerror /= 0 .AND. izerror /= T_ERR_NOTFOUND) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
...
!------------------------------------------------------------------------------
  ...
  IF ( ldiabf_lh ) THEN
    ...
    DO k = 1, ke
      DO j = jstart, jend 
        DO i = istart, iend
          !WL2011b this is the first time (each time step) that
          !tendencies are added to tt_mic, hence no adding
          IF (lbud) THEN
            tt_mic(i,j,k)=ttens(i,j,k)
          ENDIF
          !WL2011e
          ttens(i,j,k) = ttens(i,j,k) + zdtr * tinc_lh(i,j,k)
          !WL2011b
          IF (lbud) THEN
            tt_mic(i,j,k)=ttens(i,j,k) - tt_mic(i,j,k)
          ENDIF
          !WL2011e
        ENDDO
      ENDDO
  ...
  IF ( lcori .AND. (.NOT. l_Coriolis_every_RK_substep) .AND.  &
     .NOT. lsppt) THEN
...
! ***************************************************************
!
!  RELAXATION TOWARDS PROFILE
!
! Relax domain-mean value of variables towards reference profile.
! This is done for T, QV, U and V. Additionally, QC and QI can be
! relaxed.
!
! Schlemmer et al. 2011, JAS
!
! ***************************************************************
! 
! the following lines can be used, if the relaxation-tendencies
! are updated not every timestep

! relaxtime=0
! ntreal=real(ntstep,wp)
! tges=ntreal*dt
! modu_relax=mod(tges,300.0_wp) !relaxation

! update tendencies due to relaxation
  ...
  IF ( lrelax) THEN
    ...
    ELSE                            ! we are running on multiple PE's
      DO k=1,ke
        CALL gather_field(t(:,:,k,nnow),ie,je,t_tot(:,:,k),ie_tot,je_tot,0,izerror)
        CALL gather_field(qv_now(:,:,k),ie,je,qv_tot(:,:,k),ie_tot,je_tot,0,izerror)
!        CALL gather_field(qc_now(:,:,k),ie,je,qc_tot(:,:,k),ie_tot,je_tot,0,izerror)
!        CALL gather_field(qi_now(:,:,k),ie,je,qi_tot(:,:,k),ie_tot,je_tot,0,izerror)
        CALL gather_field(u(:,:,k,nnow),ie,je,u_tot(:,:,k),ie_tot,je_tot,0,izerror)
        CALL gather_field(v(:,:,k,nnow),ie,je,v_tot(:,:,k),ie_tot,je_tot,0,izerror)
      ENDDO
    ENDIF
    ...
    IF (my_cart_id == 0) THEN
      CALL relax_profile(ttens_tot,t_tot,qvtens_tot,qv_tot,&
!                            qctens_tot,qc_tot,qitens_tot,qi_tot,&
                            utens_tot,u_tot,vtens_tot,v_tot)
    ENDIF

    IF ( num_compute > 1 ) THEN
      CALL distribute_values (ttens_tot,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (qvtens_tot,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (utens_tot,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (vtens_tot,ke,0,imp_reals,icomm_cart,izerror)
    ENDIF
 
    DO  k = 1 , ke
      DO    j = jstart, jend 
        DO  i = istart, iend
          ttens (i,j,k) = ttens (i,j,k) + ttens_tot(k)
          qvtens(i,j,k) = qvtens(i,j,k) + qvtens_tot(k)
          IF (lbud) THEN
            tt_relax(i,j,k) = ttens_tot(k)
            qvt_relax(i,j,k)= qvtens_tot(k)
            ut_relax(i,j,k) = utens_tot(k)
            vt_relax(i,j,k) = vtens_tot(k)
          ENDIF
        ENDDO
      ENDDO
      DO j = jstartu, jendu
        DO i = istartu, iendu
          utens(i,j,k) = utens(i,j,k) + utens_tot(k)
        ENDDO
      ENDDO
     
      DO j = jstartv, jendv
        DO i = istartv, iendv
          vtens(i,j,k) = vtens(i,j,k) + vtens_tot(k)
        ENDDO
      ENDDO
    ENDDO
...
! ***************************************************************
!
! INCLUDE LARGE SCALE FORCINGS
!
! Apply lsf across entire domain.
! This is done for T, QT, U and V. Hydrometeors   
! are not forced.
!
! Implemented in order to be able to run GCSS cases
!
! ***************************************************************
! 
! Inspired by Linda's implementation of relaxation, as well as my implementation 
! of the time dependent surface fluxes

! update tendencies due to LSF
! IMPORTANT: IN ORDER TO RUN GCSS CASES ACCORDING TO SETUPi
! POTENTIAL TEMPERATURE TENDENCIES RATHER THAN TEMPERATURE TENDENCIES ARE 
! PRESCRIBED 

  IF ( llsf) THEN
    thetatens_lsf(:)  = 0.0_wp
    qvtens_lsf(:) = 0.0_wp
    ugeo_lsf(:)  = 0.0_wp
    vgeo_lsf(:)  = 0.0_wp
    wsub_lsf(:)  = 0.0_wp
    ...
    IF (my_cart_id == 0) THEN
      CALL lsf_profile(thetatens_lsf,qvtens_lsf,ugeo_lsf,vgeo_lsf,wsub_lsf)
    ENDIF

    IF ( num_compute > 1 ) THEN
      CALL distribute_values (thetatens_lsf,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (qvtens_lsf,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (ugeo_lsf,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (vgeo_lsf,ke,0,imp_reals,icomm_cart,izerror)
      CALL distribute_values (wsub_lsf,ke,0,imp_reals,icomm_cart,izerror)
    ENDIF
    ...
    DO j=jstart, jend
      DO i=istart, iend
        IF ((wsub_lsf(k+1)+wsub_lsf(k))<0.0_wp) THEN   !assume same subsidence effect as next level!
          subs_t    = 0.5_wp*(wsub_lsf(k+1)+wsub_lsf(k)) * ((t(i,j,k,nnow) - t(i,j,k+1,nnow))/(hhl(i,j,k)-hhl(i,j,k+1))+g/cp_d)
          subs_qv   = 0.5_wp*(wsub_lsf(k+1)+wsub_lsf(k)) * (qv_now(i,j,k) - qv_now(i,j,k+1))/(hhl(i,j,k)-hhl(i,j,k+1))
          subs_qc   = 0.5_wp*(wsub_lsf(k+1)+wsub_lsf(k)) * (qc_now(i,j,k) - qc_now(i,j,k+1))/(hhl(i,j,k)-hhl(i,j,k+1))
        ELSE
          subs_t    = 0.5_wp*(wsub_lsf(k+2)+wsub_lsf(k+1)) * ((t(i,j,k+1,nnow) - t(i,j,k+2,nnow))/(hhl(i,j,k+1)-hhl(i,j,k+2))+g/cp_d)
          subs_qv   = 0.5_wp*(wsub_lsf(k+2)+wsub_lsf(k+1)) * (qv_now(i,j,k+1) - qv_now(i,j,k+2))/(hhl(i,j,k+1)-hhl(i,j,k+2))
          subs_qc   = 0.5_wp*(wsub_lsf(k+2)+wsub_lsf(k+1)) * (qc_now(i,j,k+1) - qc_now(i,j,k+2))/(hhl(i,j,k+1)-hhl(i,j,k+2))
        ENDIF
        exner(i,j,k) = EXP(-rovcp*LOG((p0(i,j,k)+pp(i,j,k,nnow))*rp00))
        ! CONVERT THETA-TENDENCY TO T-TENDENCY
        ttens (i,j,k) = ttens (i,j,k) + thetatens_lsf(k)*exner(i,j,k) - subs_t
        qvtens(i,j,k) = qvtens(i,j,k) + qvtens_lsf(k) - subs_qv
        qctens(i,j,k) = qctens(i,j,k) - subs_qc
        IF (lbud) THEN
          tt_lsf(i,j,k) = thetatens_lsf(k)*exner(i,j,k) - subs_t
          qvt_lsf(i,j,k)= qvtens_lsf(k) - subs_qv
          qcit_lsf(i,j,k)= - subs_qc
        ENDIF
      ENDDO
    ENDDO
    ...
    DO k=2,ke-1
      DO j=jstart, jend
        DO i=istart, iend
          IF ((wsub_lsf(k-1)+wsub_lsf(k))<0.0_wp) THEN   !downwind scheme for subsidence
            subs_t    = 0.5_wp*(wsub_lsf(k-1)+wsub_lsf(k)) * ((t(i,j,k-1,nnow) - t(i,j,k,nnow))/(hhl(i,j,k-1)-hhl(i,j,k))+g/cp_d)
            subs_qv   = 0.5_wp*(wsub_lsf(k-1)+wsub_lsf(k)) * (qv_now(i,j,k-1) - qv_now (i,j,k))/(hhl(i,j,k-1)-hhl(i,j,k))
            subs_qc   = 0.5_wp*(wsub_lsf(k-1)+wsub_lsf(k)) * (qc_now(i,j,k-1) - qc_now (i,j,k))/(hhl(i,j,k-1)-hhl(i,j,k))
          ELSE
            subs_t    = 0.5_wp*(wsub_lsf(k+1)+wsub_lsf(k)) * ((t(i,j,k,nnow) - t(i,j,k+1,nnow))/(hhl(i,j,k)-hhl(i,j,k+1))+g/cp_d)
            subs_qv   = 0.5_wp*(wsub_lsf(k+1)+wsub_lsf(k)) * (qv_now(i,j,k) - qv_now(i,j,k+1))/(hhl(i,j,k)-hhl(i,j,k+1))
            subs_qc   = 0.5_wp*(wsub_lsf(k+1)+wsub_lsf(k)) * (qc_now(i,j,k) - qc_now(i,j,k+1))/(hhl(i,j,k)-hhl(i,j,k+1))
    ...
    DO j=jstart, jend
      DO i=istart, iend
        IF ((wsub_lsf(k-1))<0.0_wp) THEN   !downwind scheme for subsidence
          subs_t    = 0.5_wp*(wsub_lsf(k-1)) * ((t(i,j,k-1,nnow) - t(i,j,k,nnow))/(hhl(i,j,k-1)-hhl(i,j,k))+g/cp_d)
          subs_qv   = 0.5_wp*(wsub_lsf(k-1)) * (qv_now(i,j,k-1) - qv_now (i,j,k))/(hhl(i,j,k-1)-hhl(i,j,k))
          subs_qc   = 0.5_wp*(wsub_lsf(k-1)) * (qc_now(i,j,k-1) - qc_now (i,j,k))/(hhl(i,j,k-1)-hhl(i,j,k))
        ELSE
          subs_t = 0.0_wp
          subs_qv = 0.0_wp
          subs_qc = 0.0_wp
     ...
     IF(lcori) THEN 
       IF(lbud) THEN
         DO  k = 1 , ke
           DO    j = jstart, jend 
             DO  i = istart, iend
                ut_lsf(i,j,k) = -vgeo_lsf(k)*fc(i,j)
                vt_lsf(i,j,k) =  ugeo_lsf(k)*fc(i,j)
             ENDDO
           ENDDO
         ENDDO
       ENDIF
       DO  k = 1 , ke
         DO j = jstartu, jendu
           DO i = istartu, iendu
             utens(i,j,k) = utens(i,j,k) - vgeo_lsf(k)*fc(i,j)
           ENDDO
        ENDDO
        DO j = jstartv, jendv
          DO i = istartv, iendv
            vtens(i,j,k) = vtens(i,j,k) + ugeo_lsf(k)*fc(i,j)
          ENDDO
        ENDDO
      ENDDO
    ENDIF
...
#ifdef TEND
! Variables for tendency-sum output (dmaurer)
  ttend_tmp1 = ttens                                   ! T tendency before turbulence scheme
  utend_tmp1 = utens                                   ! U tendency before turbulence scheme
  vtend_tmp1 = vtens                                   ! V tendency before turbulence scheme
#endif
...
#ifdef TEND
! Variables for tendency-sum output (dmaurer)
  ttend_tmp2 = ttens                              ! T tendency after turbulence scheme
  utend_tmp2 = utens                              ! U tendency after turbulence scheme
  vtend_tmp2 = vtens                              ! V tendency after turbulence scheme
  ttend_tur = ttend_tur + ttend_tmp2 - ttend_tmp1 ! Sum T tendency after turbulence scheme
  utend_tur = utend_tur + utend_tmp2 - utend_tmp1 ! Sum U tendency after turbulence scheme
  vtend_tur = vtend_tur + vtend_tmp2 - vtend_tmp1 ! Sum V tendency after turbulence scheme

  ttend_tot = ttend_tot + ttens      ! Sum total T tendency
  utend_tot = utend_tot + utens      ! Sum total U tendency
  vtend_tot = vtend_tot + vtens      ! Sum total V tendency
#endif
  ...
  runge_kutta_loop: DO irk = 1, irk_order
    ...
    IF ( y_vert_adv_dyn == "impl2" ) THEN
      ...
      IF ( irk_order == 3 .AND. irunge_kutta == 2 ) THEN
        ...
        CALL complete_tendencies_uvwtpp( nadv, gamma_rk*dt, irk, alpha_rk, beta_rk )
        !WL2011e
      ELSE
        !WL2011b
        !CALL complete_tendencies_uvwtpp( nadv, gamma_rk*dt )
        CALL complete_tendencies_uvwtpp( nadv, gamma_rk*dt, irk )
        !WL2011e
      END IF
    ...
    ELSE IF ( itype_fast_waves == 1 ) THEN
      !WL2011b
      !store temperature tendency from slow-mode to fast-mode array
      IF (lbud .AND. irk == irk_order) THEN
        !tt_dpdt(:,:,:) = tadvt(:,:,:)
        !Initialize fast-mode tendency with zero
        tt_dpdt(:,:,:) = 0.0_wp
      END IF
      !WL2011e
    ...
    !WL2011b
    !IF (lbud .AND. irk == irk_order ) THEN
    !   !depending on itheta_adv different things happen in the fast modes
    !     tt_dpdt(:,:,:) = (t(:,:,:,nnew)-t(:,:,:,nnow))/dt - tt_dpdt(:,:,:)
    !ENDIF
    !WL2011e
    ...
    ENDIF
    !WL2011b
    IF (lbud .AND. irk == irk_order .AND. ldiabf_lh) THEN
      ! remove latent heating estimate
         tt_mic(:,:,:) = tt_mic(:,:,:) - tinc_lh(:,:,:) * zdtr
    ENDIF
    !WL2011e
    IF (ltime) THEN
...
!CDIR COLLAPSE
  ...
  !WL2011b
  !tendencies from positive definite advection
  IF (lbud) THEN 
    qvt_adv(:,:,:)   = ( qv(:,:,:) - qv_now(:,:,:) ) / dt
    qcit_adv(:,:,:)  = ( qc(:,:,:) - qc_now(:,:,:) ) / dt
    IF (ASSOCIATED(qi))  qcit_adv(:,:,:)  = qcit_adv(:,:,:) + (qi(:,:,:) - qi_now(:,:,:)) / dt
    qrsgt_adv(:,:,:) = 0.0_wp
    IF (ASSOCIATED(qr))  qrsgt_adv(:,:,:) = qrsgt_adv(:,:,:)+ (qr(:,:,:) - qr_now(:,:,:)) / dt
    IF (ASSOCIATED(qs))  qrsgt_adv(:,:,:) = qrsgt_adv(:,:,:)+ (qs(:,:,:) - qs_now(:,:,:)) / dt
    IF (ASSOCIATED(qg))  qrsgt_adv(:,:,:) = qrsgt_adv(:,:,:)+ (qg(:,:,:) - qg_now(:,:,:)) / dt
  END IF
  !WL2011e
        ...
        DO k = 1, ke
          DO j = 1, je
            DO i = 1, ie
              ...
              vseda(i,j,k,isp) = (-1._wp)*vseda(i,j,k,isp)
            ENDDO
          ENDDO
        ENDDO
  ...
  ! (WL;2009)b
  IF(lbud) THEN
    qvt_turb(:,:,:)  = (qv(:,:,:)-qv_now(:,:,:))/dt + qvtens(:,:,:) - qvt_turb(:,:,:)
    qcit_turb(:,:,:) = (qc(:,:,:)-qc_now(:,:,:))/dt + qctens(:,:,:) - qcit_turb(:,:,:)
    IF (ASSOCIATED(qi)) qcit_turb(:,:,:) = qcit_turb(:,:,:) + (qi(:,:,:)-qi_now(:,:,:))/dt + qitens(:,:,:)
  END IF
  !(WL;2009)e
  ...
  !(WL;2009)b
  IF (lbud) THEN
    qvt_turb(:,:,:)  = (qv(:,:,:)-qv_now(:,:,:))/dt - qvt_turb(:,:,:)
    qcit_turb(:,:,:) = (qc(:,:,:)-qc_now(:,:,:))/dt - qcit_turb(:,:,:)
    IF (ASSOCIATED(qi)) qcit_turb(:,:,:) = qcit_turb(:,:,:) + (qi(:,:,:)-qi_now(:,:,:))/dt 
  END IF
  !(WL;2009)e
  ...
  !----------------------------------------------------------------------------
!WL2011b
  !Calculate horizontal diffusion tendencies
  IF (lbud) THEN
    qvt_hd(:,:,:)  = (qv(:,:,:)-qv_now(:,:,:))/dt
    qcit_hd(:,:,:) = (qc(:,:,:)-qc_now(:,:,:))/dt
    IF (ASSOCIATED(qi)) qcit_hd(:,:,:) = qcit_hd(:,:,:) + (qi(:,:,:)-qi_now(:,:,:))/dt
    tt_hd(:,:,:)   = (t(:,:,:,nnew)-t(:,:,:,nnow))/dt
  END IF
!WL2011e
...
!WL2011b
  IF (lbud) THEN
    qvt_hd(:,:,:)  = (qv(:,:,:)-qv_now(:,:,:))/dt - qvt_hd(:,:,:)
    qcit_hd(:,:,:) = (qc(:,:,:)-qc_now(:,:,:))/dt - qcit_hd(:,:,:)
    IF (ASSOCIATED(qi)) qcit_hd(:,:,:) = qcit_hd(:,:,:) + (qi(:,:,:)-qi_now(:,:,:))/dt
    tt_hd(:,:,:)   =  (t(:,:,:,nnew)-t(:,:,:,nnow))/dt - tt_hd(:,:,:)
  END IF
!WL2011e
...
!   ENDIF
   ...
   !WL2011b
    IF (lbud .AND. .NOT.ldiabf_lh) THEN
      ! no tendency has been written to tt_mic so far
      tt_mic(:,:,:) = (t(:,:,:,nnew) - t(:,:,:,nnow))/dt
    ENDIF
    IF (lbud .AND. ldiabf_lh) THEN
      tt_mic(:,:,:) = (t(:,:,:,nnew) - t(:,:,:,nnow))/dt - tt_mic(:,:,:)
    ENDIF
    IF (lbud) THEN
      qvt_mic(:,:,:)  = (qv(:,:,:) - qv_now(:,:,:))/dt
      qcit_mic(:,:,:)  = (qc(:,:,:) - qc_now(:,:,:))/dt
      IF (ASSOCIATED(qi)) qcit_mic(:,:,:)  = qcit_mic(:,:,:) + (qi(:,:,:) - qi_now(:,:,:))/dt
    ENDIF
    !WL2011e
    ...
    IF (lbud) THEN
      tt_mic(:,:,:)= (t(:,:,:,nnew) - t(:,:,:,nnow))/dt -  tt_mic(:,:,:)
      qvt_mic(:,:,:)= (qv(:,:,:) - qv_now(:,:,:))/dt - qvt_mic(:,:,:)
      qcit_mic(:,:,:)= (qc(:,:,:) - qc_now(:,:,:))/dt - qcit_mic(:,:,:)
      IF (ASSOCIATED(qi)) qcit_mic(:,:,:)= qcit_mic(:,:,:) + (qi(:,:,:) - qi_now(:,:,:))/dt
    ENDIF
    !WL2011e
...
! Initialize, whether debug output shall be done
  ...
  IF ( cfl_max > 0.95_wp * cfl_limit_1D ) THEN
    ...
    epsray0 = SIGN(1._wp,dt) * MIN( epsray0, 1.0_wp / (2.0_wp * ABS(dt) ) )
