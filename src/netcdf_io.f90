...
!   stepDate = list_yakdat(1)
  ...
  CALL open_file (nuedat, yname, ymode_write, 'ncdf',  &
                  icomm_asynio, me_asynio, -1,lasync_io, llockfiles, 2, &
                  errmsg, izerror)
  IF (izerror /= 0) THEN
    ierror = 3322
    yerrmsg = errmsg
...
! Local scalars:
  INTEGER (KIND=iintegers)  :: i1,i2,i3, iid, jid, n
      ...
      ELSE
        ...
        ELSEIF ( vardim3_p3 == nhori) THEN
          ...
          IF (ierror /= NF90_NOERR) THEN
            ...
            RETURN
          ENDIF
! LS,  b for the output of s_so and wso_relax the following lines need to be added
! to render possible the output of soil variables for 3d fields
        ELSEIF ( vardim3_p3 == ke_soil+1 ) THEN
          ierror = nf90_def_var(ncid, TRIM(var(i1,i2,i3)%name), NF90_FLOAT,  &
                          (/ idims_id_out(iid), idims_id_out(jid),         &
                             idims_id_out(8), idims_id_out(5) /), var_id(n))
          IF (ierror /= NF90_NOERR) THEN
