...
!OCH <<
  ...
  USE wolken_driver,     ONLY: cloud_type => wolke_typ,          &
       ...
       &                       ltabdminwgg, int1, int2, int3, int4, int5, &
       &                       int6, int7, int8, int9, int10, &
       &                       int11, int12, int13, int14, int15, &
       &                       ccn_num1, ccn_num2, ccn_num3, &
!OCH>>
!       &                       cmp_nuc_qi_d,        &
...
!       &                       cmp_dep_qs_d, cmp_sub_qi_d, cmp_self_qi_d,&
       &                       cmp_self_qni_d, &!cmp_self_qns_d,           &
!       &                       cmp_self_qng_d, cmp_self_qnc_d, cmp_sub_qs_d,           &
       &                       cmp_rime_qi_qc_d, cmp_rime_qs_qc_d,       &
       &                       cmp_rime_qi_qr_d, cmp_rime_qs_qr_d,       &
       &                       cmp_melt_qi_qr_d, cmp_melt_qs_qr_d,       &
       &                       cmp_rime_snow_d,cmp_rime_ice_d,           &
!       &                       cmp_rime_qg_qc_d, cmp_rime_ice_d,         &
!       &                       cmp_rime_snow_d, cmp_rime_graup_d,        &
...
!OCH <<
  ...
  USE data_gscp, ONLY : sumIN_t0,sumIN1_t0, sumIN2_t0,sumIN3_t0,sumIN4_t0,sumIN5_t0,sumIN6_t0,&
                        ...
                        sumIN14_t0,sumIN15_t0,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15, &
                        sumCCN_t0,sumCCN1_t0, sumCCN2_t0, sumCCN3_t0,wccn1, wccn2, wccn3
...
! Number of habits and coefficients for ice cloud radiative feedback based on r_eff:
  ...
  TYPE t_reff
    REAL  (KIND=wp), POINTER :: r_effc (:,:,:) => NULL()      ! effective radius of cloud droplets solar/thermal [m]
    REAL  (KIND=wp), POINTER :: r_effi_so (:,:,:,:) => NULL() ! effective radius of ice particles solar   [m]
    REAL  (KIND=wp), POINTER :: r_effi_th (:,:,:,:) => NULL() ! effective radius of ice particles thermal [m]
    REAL  (KIND=wp), POINTER :: odepthw_so (:,:,:) => NULL()  ! optical depth of cloud droplets
    REAL  (KIND=wp), POINTER :: odepthi_so (:,:,:) => NULL()  ! optical depth of ice particles
    REAL  (KIND=wp), POINTER :: odepthw_th (:,:,:) => NULL()  ! optical depth of cloud droplets
    REAL  (KIND=wp), POINTER :: odepthi_th (:,:,:) => NULL()  ! optical depth of ice particles
    ! constant coefficient for calculation of r_effc:
    REAL  (KIND=wp) :: c_reffc
    ! constant coefficients for calculation of r_effci_so after def. of Key et al. (2002) for two
    ! particle habits:
    REAL  (KIND=wp) :: c_reffi_so(nhabits_ice,nparams_per_habit_reff_so)
    ! constant coefficients for calculation of r_effci_so after def. of Fu et al. (1998) for two
    ! particle habits (each needs two coeffs):
    REAL  (KIND=wp) :: c_reffi_th(nhabits_ice,nparams_per_habit_reff_th)
 END TYPE t_reff

  TYPE(t_reff), SAVE      :: refft
...
! For Seifert/Beheng scheme:
! Set QNICE in a way that it leads to an average mass corresponding to a diameter of 100 microns:
! Input: QI in kg/m^3; Output: QNICE in 1/m^3
...
!!!!! dann auch die ssw, ssi ueberall nachberechnen.
  ...
  ! LM modules
  USE data_runcontrol , ONLY : lINprog, lCCNprog, tperturb, CCN_1_N, CCN_2_N, CCN_3_N
  USE data_gscp, ONLY: sumIN_t0
  USE parallel_utilities, ONLY: gather_field, distribute_values
  USE data_modelconfig, ONLY: ie_tot, je_tot, ke
  USE data_fields, ONLY: debug1 , debug2, debug3, debug4, debug5
  ...
  REAL(KIND=wp), PARAMETER :: qnc_const = 1200.0e06_wp    ! Assumed constant QNC (1/m^3) in case of nuc_c_typ = 0 
  REAL(KIND=wp), PARAMETER :: Nice0 = 500000.0_wp          ! Assumed constant QNI (1/m^3) in case of lni_const_imm=.TRUE.
  ...
  LOGICAL, PARAMETER :: debug_maxval = .false.!APTEST .true.  ! Output von Max./Min.-Werten der QX auf stdout
! UB_20090814>>
...
! Voher war das an den Output der Max./Min.-Werte der QX gekoppelt.
  LOGICAL, PARAMETER :: debug_maxrain = .false. !APTEST .true.
...
! field for inst. microphys. conv. rates     (kg/kg*s)
  ...
  REAL(KIND=wp), PARAMETER :: wcb_min=0.0001, scb_min=0.0e0, scb_max=-0.0e0
...
!----------------
  ...
  REAL (KIND=wp), POINTER :: &
    in_t1 (:,:,:) => NULL(),       & !IN_T1 at nx
    in_t2 (:,:,:) => NULL(),       & !IN_T2 at nx
    in_t3 (:,:,:) => NULL(),       & !IN_T3 at nx
...
!------------------------------------------------------------------------------
  ...
  IF (lINprog) then
    CALL trcr_get(izerror, 'IN_T1', ptr_tlev = nx, ptr = in_t1)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T2', ptr_tlev = nx, ptr = in_t2)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T3', ptr_tlev = nx, ptr = in_t3)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T4', ptr_tlev = nx, ptr = in_t4)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T5', ptr_tlev = nx, ptr = in_t5)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T6', ptr_tlev = nx, ptr = in_t6)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T7', ptr_tlev = nx, ptr = in_t7)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T8', ptr_tlev = nx, ptr = in_t8)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T9', ptr_tlev = nx, ptr = in_t9)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T10', ptr_tlev = nx, ptr = in_t10)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T11', ptr_tlev = nx, ptr = in_t11)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T12', ptr_tlev = nx, ptr = in_t12)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T13', ptr_tlev = nx, ptr = in_t13)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T14', ptr_tlev = nx, ptr = in_t14)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'IN_T15', ptr_tlev = nx, ptr = in_t15)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF

!Implement weight calculation here
    if (num_compute >1) then
      ALLOCATE(IN_dom(ie_tot,je_tot,ke))
      do k=1,ke
        dz_dom(k)=hhl(1,1,k) - hhl(1,1,k+1)
        CALL gather_field(in_t1(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w1=sum(IN_dom)
      endif
      CALL distribute_values (w1,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t2(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w2=sum(IN_dom)
      endif
      CALL distribute_values (w2,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t3(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w3=sum(IN_dom)
      endif
      CALL distribute_values (w3,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t4(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w4=sum(IN_dom)
      endif
      CALL distribute_values (w4,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t5(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w5=sum(IN_dom)
      endif
      CALL distribute_values (w5,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t6(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w6=sum(IN_dom)
      endif
      CALL distribute_values (w6,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t7(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w7=sum(IN_dom)
      endif
      CALL distribute_values (w7,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t8(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w8=sum(IN_dom)
      endif
      CALL distribute_values (w8,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t9(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w9=sum(IN_dom)
      endif
      CALL distribute_values (w9,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t10(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w10=sum(IN_dom)
      endif
      CALL distribute_values (w10,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t11(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w11=sum(IN_dom)
      endif
      CALL distribute_values (w11,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t12(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w12=sum(IN_dom)
      endif
      CALL distribute_values (w12,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t13(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w13=sum(IN_dom)
      endif
      CALL distribute_values (w13,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t14(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w14=sum(IN_dom)
      endif
      CALL distribute_values (w14,1,10,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(in_t15(:,:,k)*dz_dom(k),ie,je,IN_dom(:,:,k),ie_tot,je_tot,10,izerror)
      enddo
      if (my_cart_id == 10) then
      w15=sum(IN_dom)
      endif
      CALL distribute_values (w15,1,10,imp_reals,icomm_world,izerror)
      DEALLOCATE(IN_dom)
    endif
    insum=w1+w2+w3+w4+w5+w6+w7+w8+w9+w10+w11+w12+w13+w14+w15
!debug4(:,:,1)=w1
!debug4(:,:,2)=w2
!debug4(:,:,3)=w3
!debug4(:,:,4)=w4
...
!debug4(:,:,8)=sumIN4_t0
    if (insum == sumIN_t0) then
      ...
      if (w1 < 1.01*sumIN1_t0) then
        w1=abs(w1-sumIN1_t0)!/sumIN1_t0
      else
        w1=0.0_wp
      endif
      if (w2 < 1.01*sumIN2_t0) then
        w2=abs(w2-sumIN2_t0)!/sumIN2_t0
      else
        w2=0.0_wp
      endif
      if (w3 < 1.01*sumIN3_t0) then
        w3=abs(w3-sumIN3_t0)!/sumIN3_t0
      else
        w3=0.0_wp
      endif
      if (w4 < 1.01*sumIN4_t0) then
        w4=abs(w4-sumIN4_t0)!/sumIN4_t0
      else
        w4=0.0_wp
      endif
      if (w5 < 1.01*sumIN5_t0) then
        w5=abs(w5-sumIN5_t0)!/sumIN5_t0
      else
        w5=0.0_wp
      endif
      if (w6 < 1.01*sumIN6_t0) then
        w6=abs(w6-sumIN6_t0)!/sumIN6_t0
      else
        w6=0.0_wp
      endif
      if (w7 < 1.01*sumIN7_t0) then
        w7=abs(w7-sumIN7_t0)!/sumIN7_t0
      else
        w7=0.0_wp
      endif
      if (w8 < 1.01*sumIN8_t0) then
        w8=abs(w8-sumIN8_t0)!/sumIN8_t0
      else
        w8=0.0_wp
      endif
      if (w9 < 1.01*sumIN9_t0) then
        w9=abs(w9-sumIN9_t0)!/sumIN9_t0
      else
        w9=0.0_wp
      endif
      if (w10 < 1.01*sumIN10_t0) then
        w10=abs(w10-sumIN10_t0)!/sumIN10_t0
      else
        w10=0.0_wp
      endif
      if (w11 < 1.01*sumIN11_t0) then
        w11=abs(w11-sumIN11_t0)!/sumIN11_t0
      else
        w11=0.0_wp
      endif
      if (w12 < 1.01*sumIN12_t0) then
        w12=abs(w12-sumIN12_t0)!/sumIN12_t0
      else
        w12=0.0_wp
      endif
      if (w13 < 1.01*sumIN13_t0) then
        w13=abs(w13-sumIN13_t0)!/sumIN13_t0
      else
        w13=0.0_wp
      endif
      if (w14 < 1.01*sumIN14_t0) then
        w14=abs(w14-sumIN14_t0)!/sumIN14_t0
      else
        w14=0.0_wp
      endif
      if (w15 < 1.01*sumIN15_t0) then
        w15=abs(w15-sumIN15_t0)!/sumIN15_t0
      else
        w15=0.0_wp
      endif
      weight_wIN = w1+w2+w3+w4+w5+w6+w7+w8+w9+w10+w11+w12+w13+w14+w15
      !if ((weight_wIN - 1.0) > 0.00001_wp .AND. weight_wIN > 0.0_wp) then
      if (weight_wIN > 0.0_wp) then
        weight_wIN=1.0/weight_wIN
        w1=weight_wIN*w1
        w2=weight_wIN*w2
        w3=weight_wIN*w3
        w4=weight_wIN*w4
...
debug3(:,:,10)=w1+w2+w3+w4+w5+w6+w7+w8+w9+w10+w11+w12+w13+w14+w15
!debug3(:,:,2)=w2
!debug3(:,:,3)=w3
!debug3(:,:,4)=w4
!debug3(:,:,5)=1000.0
    if (ntstep > tperturb*3600/dt .AND. (ntstep-2 < tperturb*3600/dt .OR. ntstep-2 <0)) then
        PRINT *, "I call SUBROUTINE add_IN_perturbation at ntstep=",ntstep
        CALL add_IN_perturbation(ke,je,ie,in_t1, in_t2, in_t3, in_t4, in_t5, in_t6, in_t7, in_t8, in_t9, &
                                 in_t10, in_t11, in_t12, in_t13, in_t14, in_t15)
    endif
  ENDIF !lINprog

  !GEcheck
  !do k=1,ke
  !   debug5(:,:,k)=ie_tot*je_tot*dz_dom(k)*qnc(:,:,k)
  !end do
  !debug3(1,1,1)=sum(debug5)
  !debug3(1,1,3)=debug3(1,1,1)-debug3(1,1,2) !qnc end of 2M minus qnc after read in by tracer
  !GE


  IF (lCCNprog) THEN
    CALL trcr_get(izerror, 'CCN_1', ptr_tlev = nx, ptr = ccn1)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'CCN_2', ptr_tlev = nx, ptr = ccn2)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
    CALL trcr_get(izerror, 'CCN_3', ptr_tlev = nx, ptr = ccn3)
    IF (izerror /= 0) THEN
      yzerrmsg = trcr_errorstr(izerror)
      CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
    ENDIF
!AP_CCNSFC>>
!  ccn1(:,:,ke-2:ke)=CCN_1_N
!  ccn2(:,:,ke-2:ke)=CCN_2_N
!AP_CCNSFC<<
    !Implement weight calculations here
    if (num_compute >1) then
      ALLOCATE(CCN_dom(ie_tot,je_tot,ke))
      do k=1,ke
        dz_dom(k)=hhl(1,1,k) - hhl(1,1,k+1)
        CALL gather_field(ccn1(:,:,k)*dz_dom(k),ie,je,CCN_dom(:,:,k),ie_tot,je_tot,11,izerror)
      enddo
      if (my_cart_id == 11) then
      wccn1=sum(CCN_dom)
      endif
      CALL distribute_values (wccn1,1,11,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(ccn2(:,:,k)*dz_dom(k),ie,je,CCN_dom(:,:,k),ie_tot,je_tot,11,izerror)
      enddo
      if (my_cart_id == 11) then
      wccn2=sum(CCN_dom)
      endif
      CALL distribute_values (wccn2,1,11,imp_reals,icomm_world,izerror)
      do k=1,ke
        CALL gather_field(ccn3(:,:,k)*dz_dom(k),ie,je,CCN_dom(:,:,k),ie_tot,je_tot,11,izerror)
      enddo
      if (my_cart_id == 11) then
      wccn3=sum(CCN_dom)
      endif
      CALL distribute_values (wccn3,1,11,imp_reals,icomm_world,izerror)
      DEALLOCATE(CCN_dom)
    endif
    ccnsum=wccn1+wccn2+wccn3
    if (ccnsum >= 1.01*sumCCN_t0) then  !make sure that via evaporation I never add >1% of total
      wccn1=0.0_wp
      wccn2=0.0_wp
      wccn3=0.0_wp
    else
      if (wccn1 < 1.01*sumCCN1_t0) then
        wccn1=abs(wccn1-sumCCN1_t0)!/sumCCN1_t0
      else
        wccn1=0.0_wp
      endif
      if (wccn2 < 1.01*sumCCN2_t0) then
        wccn2=abs(wccn2-sumCCN2_t0)!/sumCCN2_t0
      else
        wccn2=0.0_wp
      endif
      if (wccn3 < 1.01*sumCCN3_t0) then
        wccn3=abs(wccn3-sumCCN3_t0)!/sumCCN3_t0
      else
        wccn3=0.0_wp
      endif
      weight_wCCN=wccn1+wccn2+wccn3
      if (weight_wCCN > 0.0_wp) then
        weight_wCCN=1.0/weight_wCCN
...
! UB_20090227<<
  ...
  if (lCCNprog) then
    ccn1(:,:,:)=ccn1(:,:,:)+wccn1*(ncloud_tmpi(:,:,:) - (rho(:,:,:)*qnc(:,:,:)))
    ccn2(:,:,:)=ccn2(:,:,:)+wccn2*(ncloud_tmpi(:,:,:) - (rho(:,:,:)*qnc(:,:,:))) 
    ccn3(:,:,:)=ccn3(:,:,:)+wccn3*(ncloud_tmpi(:,:,:) - (rho(:,:,:)*qnc(:,:,:)))   
  endif
  ...
  if (lCCNprog) then 
    ccn1(:,:,:)=ccn1(:,:,:)+wccn1*(ncloud_tmpi(:,:,:) - (rho(:,:,:)*qnr(:,:,:)))   
    ccn2(:,:,:)=ccn2(:,:,:)+wccn2*(ncloud_tmpi(:,:,:) - (rho(:,:,:)*qnr(:,:,:)))
    ccn3(:,:,:)=ccn3(:,:,:)+wccn3*(ncloud_tmpi(:,:,:) - (rho(:,:,:)*qnr(:,:,:)))
  endif
...
!CDIR    NODEP                                      
      DO i=0,loc_ix
        ...
        IF (lINprog) then
          !no unit conversion needed as tracers are already in #/m3
          int1(i,j,k)    = DBLE(in_t1(ii,jj,kk))
          int2(i,j,k)    = DBLE(in_t2(ii,jj,kk))
          int3(i,j,k)    = DBLE(in_t3(ii,jj,kk))
          int4(i,j,k)    = DBLE(in_t4(ii,jj,kk))
          int5(i,j,k)    = DBLE(in_t5(ii,jj,kk))
          int6(i,j,k)    = DBLE(in_t6(ii,jj,kk))
          int7(i,j,k)    = DBLE(in_t7(ii,jj,kk))
          int8(i,j,k)    = DBLE(in_t8(ii,jj,kk))
          int9(i,j,k)    = DBLE(in_t9(ii,jj,kk))
          int10(i,j,k)    = DBLE(in_t10(ii,jj,kk))
          int11(i,j,k)    = DBLE(in_t11(ii,jj,kk))
          int12(i,j,k)    = DBLE(in_t12(ii,jj,kk))
          int13(i,j,k)    = DBLE(in_t13(ii,jj,kk))
          int14(i,j,k)    = DBLE(in_t14(ii,jj,kk))
          int15(i,j,k)    = DBLE(in_t15(ii,jj,kk))
        ENDIF

        IF (lCCNprog) THEN
          ccn_num1(i,j,k)     = DBLE(ccn1(ii,jj,kk))
          ccn_num2(i,j,k)     = DBLE(ccn2(ii,jj,kk))
          ccn_num3(i,j,k)     = DBLE(ccn3(ii,jj,kk))
        ENDIF
        if (lni_const_imm) then
          if (q_ice(i,j,k) .gt. 0.0_wp) then
            n_ice(i,j,k) = Nice0
          endif
        endif
        if (lnc_const) then
          if (q_cloud(i,j,k) .gt. 0.0_wp) then
            n_cloud(i,j,k)=qnc_const
          endif
        endif
...
! UB_20111222<<
        ...
        IF (lINprog) then
          in_t1(ii,jj,kk) = int1(i,j,k)
          in_t2(ii,jj,kk) = int2(i,j,k)
          in_t3(ii,jj,kk) = int3(i,j,k)
          in_t4(ii,jj,kk) = int4(i,j,k)
          in_t5(ii,jj,kk) = int5(i,j,k)
          in_t6(ii,jj,kk) = int6(i,j,k)
          in_t7(ii,jj,kk) = int7(i,j,k)
          in_t8(ii,jj,kk) = int8(i,j,k)
          in_t9(ii,jj,kk) = int9(i,j,k)
          in_t10(ii,jj,kk) = int10(i,j,k)
          in_t11(ii,jj,kk) = int11(i,j,k)
          in_t12(ii,jj,kk) = int12(i,j,k)
          in_t13(ii,jj,kk) = int13(i,j,k)
          in_t14(ii,jj,kk) = int14(i,j,k)
          in_t15(ii,jj,kk) = int15(i,j,k)
        ENDIF
        IF (lCCNprog) then
          ccn1(ii,jj,kk) = ccn_num1(i,j,k)
          ccn2(ii,jj,kk) = ccn_num2(i,j,k)
          ccn3(ii,jj,kk) = ccn_num3(i,j,k)
!AP_CCNSFC>>
!if (kk >= ke-2) then
!  ccn1(ii,jj,kk)=CCN_1_N
!  ccn2(ii,jj,kk)=CCN_2_N
!  ccn3(ii,jj,kk)=CCN_3_N
!endif
!AP_CCNSFC
        ENDIF 
!OCH>>
...
!GE: not needed
!      IF (lCCNprog) then
!        satad_dummy(:,:) = qcdummy(:,:) -qc(:,:,k)  !then evap_qc>0
!        evap_qc(:,:)=0.0_wp
!        WHERE (qcdummy(:,:) > qc(:,:,k)) evap_qc(:,:) = (-1)*satad_dummy(:,:)
!        DO i=1,ie
!        DO j=1,je
!        xc=Min(Max(qc(i,j,k)/(qnc(i,j,k)+eps2),cloud%x_min),cloud%x_max)
!        evap_n=Max(Min(evap_qc(i,j)/xc,0.0_wp),(-1)*qc(i,j,k)/xc)
!        ccn1(i,j,k)=ccn1(i,j,k)-wccn1*evap_n
!        ccn2(i,j,k)=ccn2(i,j,k)-wccn2*evap_n
!        ccn3(i,j,k)=ccn3(i,j,k)-wccn3*evap_n
!        END DO
!        END DO
!      endif
...
!            endif
            ...
            if (lINprog) then
              if (in_t1(ii,jj,kk).lt.0) then
                WRITE (*,*) ' SEIFERT: IN_T1 < 0, STOPPED AFTER CLOUDS AND RECOPYING TO LM-VARIABLES'
                CALL model_abort (my_world_id, 999, 'negative values in IN_T1',TRIM(yzroutine))
              endif
              if (in_t2(ii,jj,kk).lt.0) then
                WRITE (*,*) ' SEIFERT: IN_T2 < 0, STOPPED AFTER CLOUDS AND RECOPYING TO LM-VARIABLES'
                CALL model_abort (my_world_id, 999, 'negative values in IN_T2',TRIM(yzroutine))
              endif
              if (in_t3(ii,jj,kk).lt.0) then
                WRITE (*,*) ' SEIFERT: IN_T3 < 0, STOPPED AFTER CLOUDS AND RECOPYING TO LM-VARIABLES'
                CALL model_abort (my_world_id, 999, 'negative values in IN_T3',TRIM(yzroutine))
              endif
            endif
          ENDDO
        ENDDO
    ...
    nt_sedi = 2 ! CEILING(dt/dt_sedi)      ! 1
    dt_sedi = zdt/nt_sedi
...
CONTAINS
  ...
  SUBROUTINE add_IN_perturbation(k,j,i,int1, int2, int3, int4, int5, int6, int7, int8, int9, &
                                 int10, int11, int12, int13, int14, int15)
  USE data_gscp, ONLY : sumIN_t0, sumIN1_t0,sumIN2_t0,sumIN3_t0,sumIN4_t0,sumIN5_t0,sumIN6_t0,&
                        sumIN7_t0,sumIN8_t0,sumIN9_t0,sumIN10_t0,sumIN11_t0,sumIN12_t0,sumIN13_t0,&
                        sumIN14_t0,sumIN15_t0,w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w15, &
                        sumCCN_t0,sumCCN1_t0, sumCCN2_t0, sumCCN3_t0,wccn1, wccn2, wccn3
  USE data_runcontrol, ONLY : Na_IN_pert, Tmax_IN, Tdiff_IN
  USE data_modelconfig, ONLY: ie_tot, je_tot, ke
  USE data_fields,     ONLY: hhl !height of model half levels                   (m )

  IMPLICIT NONE

  INTEGER(kind=iintegers), INTENT(IN)          :: i, j, k
  REAL (kind=wp), INTENT(INOUT)                :: int1(i,j,k), int2(i,j,k), int3(i,j,k), int4(i,j,k),int5(i,j,k), &
                                                  int6(i,j,k), int7(i,j,k), int8(i,j,k), int9(i,j,k),int10(i,j,k),&
                                                  int11(i,j,k), int12(i,j,k), int13(i,j,k), int14(i,j,k), int15(i,j,k)

  REAL (kind=wp) :: Na(i,j,k), in_temp1(i,j,k), in_temp2(i,j,k), in_temp3(i,j,k), in_temp4(i,j,k), in_temp5(i,j,k), &
                    in_temp6(i,j,k), in_temp7(i,j,k), in_temp8(i,j,k), in_temp9(i,j,k),in_temp10(i,j,k), in_temp11(i,j,k), &
                    in_temp12(i,j,k), in_temp13(i,j,k), in_temp14(i,j,k), in_temp15(i,j,k),INsum(i,j,k), Temp_bin, dz_dom

  REAL (kind=wp) :: sumIN1,sumIN2,sumIN3,sumIN4,sumIN5,sumIN6,sumIN7,sumIN8,sumIN9,sumIN10,sumIN11,sumIN12,&
                    sumIN13,sumIN14,sumIN15,sumIN

  INTEGER (kind=iintegers) ::  kk

  Na(:,:,:)= Na_IN_pert
  Temp_bin=Tmax_IN
  INsum=0.0_wp

  in_temp1(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp)
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp1
  in_temp2(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp2
  in_temp3(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp3
  in_temp4(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp4
  in_temp5(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp5
  in_temp6(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp6
  in_temp7(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp7
  in_temp8(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp8
  in_temp9(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp9
  in_temp10(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp10
  in_temp11(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp11
  in_temp12(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp12
  in_temp13(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp13
  in_temp14(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  Temp_bin=Temp_bin-Tdiff_IN
  INsum=INsum+in_temp14
  in_temp15(:,:,:)=1e3_wp*Na(:,:,:)**1.25_wp*exp(0.46_wp*(273.16_wp-Temp_bin)-11.6_wp) - INsum
  ...
  int8=int8+in_temp8
  int9=int9+in_temp9
  int10=int10+in_temp10
  int11=int11+in_temp11
  int12=int12+in_temp12
  int13=int13+in_temp13
  int14=int14+in_temp14
  int15=int15+in_temp15
...
do kk=1,k
  dz_dom=hhl(1,1,kk) - hhl(1,1,kk+1)
  sumIN1=sumIN1+ie_tot*je_tot*dz_dom*in_temp1(1,1,1)
  sumIN2=sumIN2+ie_tot*je_tot*dz_dom*in_temp2(1,1,1)
  sumIN3=sumIN3+ie_tot*je_tot*dz_dom*in_temp3(1,1,1)
  sumIN4=sumIN4+ie_tot*je_tot*dz_dom*in_temp4(1,1,1)
  sumIN5=sumIN5+ie_tot*je_tot*dz_dom*in_temp5(1,1,1)
  sumIN6=sumIN6+ie_tot*je_tot*dz_dom*in_temp6(1,1,1)
  sumIN7=sumIN7+ie_tot*je_tot*dz_dom*in_temp7(1,1,1)
  sumIN8=sumIN8+ie_tot*je_tot*dz_dom*in_temp8(1,1,1)
  sumIN9=sumIN9+ie_tot*je_tot*dz_dom*in_temp9(1,1,1)
  sumIN10=sumIN10+ie_tot*je_tot*dz_dom*in_temp10(1,1,1)
  sumIN11=sumIN11+ie_tot*je_tot*dz_dom*in_temp11(1,1,1)
  sumIN12=sumIN12+ie_tot*je_tot*dz_dom*in_temp12(1,1,1)
  sumIN13=sumIN13+ie_tot*je_tot*dz_dom*in_temp13(1,1,1)
  sumIN14=sumIN14+ie_tot*je_tot*dz_dom*in_temp14(1,1,1)
  sumIN15=sumIN15+ie_tot*je_tot*dz_dom*in_temp15(1,1,1)
end do
  sumIN=sumIN1+sumIN2+sumIN3+sumIN4+sumIN5+sumIN6+sumIN7+sumIN8+&
           sumIN9+sumIN10+sumIN11+sumIN12+sumIN13+sumIN14+sumIN15
  sumIN_t0=sumIN_t0+sumIN
  sumIN1_t0=sumIN1_t0+sumIN1
  sumIN2_t0=sumIN2_t0+sumIN2
  sumIN3_t0=sumIN3_t0+sumIN3
  sumIN4_t0=sumIN4_t0+sumIN4
  sumIN5_t0=sumIN5_t0+sumIN5
  sumIN6_t0=sumIN6_t0+sumIN6
  sumIN7_t0=sumIN7_t0+sumIN7
  sumIN8_t0=sumIN8_t0+sumIN8
  sumIN9_t0=sumIN9_t0+sumIN9
  sumIN10_t0=sumIN10_t0+sumIN10
  sumIN11_t0=sumIN11_t0+sumIN11
  sumIN12_t0=sumIN12_t0+sumIN12
  sumIN13_t0=sumIN13_t0+sumIN13
  sumIN14_t0=sumIN14_t0+sumIN14
  sumIN15_t0=sumIN15_t0+sumIN15


  END SUBROUTINE add_IN_perturbation
...
!      qnc(i,j,k) = MIN(qnc(i,j,k), qc(i,j,k)/cloud%x_min)
      ...
      if (lCCNprog) then
        ccn1(i,j,k)=ccn1(i,j,k)+wccn1*(ncloud_tmpi(i,j,k) - qnc(i,j,k))
        ccn2(i,j,k)=ccn2(i,j,k)+wccn2*(ncloud_tmpi(i,j,k) - qnc(i,j,k))
        ccn3(i,j,k)=ccn3(i,j,k)+wccn3*(ncloud_tmpi(i,j,k) - qnc(i,k,j))
      endif
...
SUBROUTINE calc_reff_rad_2mom (ntlev, qc_thresh, qi_thresh)

  IMPLICIT NONE

  INTEGER, INTENT(in)           :: ntlev
  ...
  INTEGER (KIND=iintegers) :: izerror
  CHARACTER (LEN=80)       :: yzerrmsg
  CHARACTER (LEN=25)       :: yzroutine = 'calc_reff_rad_2mom'
  LOGICAL, PARAMETER       :: debug = .FALSE.
  ...
  IF (izerror /= 0) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNCLOUD', ptr_tlev = ntlev, ptr = qnc)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  ...
  IF (izerror /= 0) THEN
    ...
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  CALL trcr_get(izerror, 'QNICE', ptr_tlev = ntlev, ptr = qni)
  IF (izerror /= 0) THEN
    yzerrmsg = trcr_errorstr(izerror)
    CALL model_abort(my_cart_id, izerror, yzerrmsg, yzroutine)
  ENDIF
  ...
  IF (itype_gscp >= 100) THEN
    CALL prefactors_reff_2mom(ldebug=debug, c_reffc=refft%c_reffc, &
         c_reffi_so=refft%c_reffi_so, c_reffi_th=refft%c_reffi_th)
    CALL diag_reffc_2mom(its=1, ite=ie, jts=1, jte=je, kts=1, kte=ke, &
         hhl=hhl, qc=qc(:,:,:), qnc=qnc(:,:,:), qc_thresh=qc_thresh, &
         c_reffc=refft%c_reffc, r_effc=refft%r_effc)
    CALL diag_reffi_2mom(its=1, ite=ie, jts=1, jte=je, kts=1, kte=ke, &
         qi=qi(:,:,:), qni=qni(:,:,:), qi_thresh=qi_thresh, &
         c_reffi_so=refft%c_reffi_so, c_reffi_th=refft%c_reffi_th, &
         r_effi_so=refft%r_effi_so, r_effi_th=refft%r_effi_th)
  END IF
...
!===========================================================================================
!
! Number of ice nuclei as function of temperature as assumed in the COSMO 1-moment schemes
!
!===========================================================================================
!===========================================================================================
...
REAL (KIND=wp) ELEMENTAL FUNCTION ice_nuclei_number(temp, t0_melt)
  ...
  ice_nuclei_number = 1.0E2_wp * EXP(0.2_wp * (t0_melt -  temp )) !GE change ice formation T ->here something can be substracted
...
DOUBLE PRECISION FUNCTION reff_gammfac_1mom(bgeo,nue,mue,f) 
  IMPLICIT NONE
  
  DOUBLE PRECISION :: bgeo, mue, nue, f
...
SUBROUTINE prefactors_reff_1mom_icemono (pi, nue_c, mue_c, ageo_i, bgeo_i, rho_w, rho_ice, ldebug, &
     c_reffc, c_reffi_so, c_reffi_th)

  IMPLICIT NONE

  REAL (kind=wp), INTENT(in)  :: pi, nue_c, mue_c, ageo_i, bgeo_i, rho_w, rho_ice
  LOGICAL, INTENT(in)             :: ldebug
  REAL (kind=wp), INTENT(out) :: c_reffc, c_reffi_so(2,2), c_reffi_th(2,4)

  REAL (kind=wp), SAVE :: nue_c_l=-99.99, mue_c_l=-99.99, ageo_i_l=-99.99, bgeo_i_l=-99.99
  LOGICAL,            SAVE :: firstcall = .TRUE.
  REAL (kind=wp), SAVE :: c_reffc_s, c_reffi_so_s(2,2), c_reffi_th_s(2,4)

  ! .. Assumption: N_c(D) = N0 * D^nue_c * exp(-lam*D^mue_c)
  !                   x_c = pi/6 * rhow * D^3
  !                N_i(D) = delta(D_0),  D_0 = qi/ni(T)
  !                   x_i = ageo_i * D^bgeo_i

  IF (firstcall .OR. (nue_c /= nue_c_l .OR.  mue_c /= mue_c_l .OR. ageo_i /= ageo_i_l .OR. bgeo_i /= bgeo_i_l)) THEN
    ...
    c_reffc_s          = 0.5 * (6.0 / (pi*rho_w))**(1.0/3.0) * &
    ...
    ! .. r_eff for ice thermal, def. of Fu et al. (1998), hex plates, monodisperse PSD, needs two coefficients:
    ...
    ! .. r_eff for ice thermal, def. of Fu et al. (1998), hex columns, monodisperse PSD, needs two coefficients:
...
SUBROUTINE prefactors_reff_1mom_icepoly (pi, nue_c, mue_c, ageo_i, bgeo_i, nue_i, mue_i, rho_w, rho_ice, ldebug, &
     c_reffc, c_reffi_so, c_reffi_th)

  IMPLICIT NONE

  REAL (kind=wp), INTENT(in)  :: pi, nue_c, mue_c, ageo_i, bgeo_i, nue_i, mue_i, rho_w, rho_ice
  LOGICAL, INTENT(in)             :: ldebug
  REAL (kind=wp), INTENT(out) :: c_reffc, c_reffi_so(2,2), c_reffi_th(2,4)


  REAL (kind=wp), SAVE  :: nue_c_l, mue_c_l, ageo_i_l, bgeo_i_l, nue_i_l, mue_i_l
  LOGICAL,            SAVE  :: firstcall = .TRUE.
  REAL (kind=wp), SAVE  :: c_reffc_s, c_reffi_so_s(2,2), c_reffi_th_s(2,4)

  ! .. Assumption: N_c(D) = N0 * D^nue_c * exp(-lam*D^mue_c)
  !                   x_c = pi/6 * rhow * D^3
  !                N_i(D) = N0 * D^nue_i * exp(-lam*D^mue_i)
  !                   x_i = ageo_i * D^bgeo_i

  IF (firstcall .OR. ( nue_c  /= nue_c_l .OR.  mue_c  /= mue_c_l     .OR. &
                       ageo_i /= ageo_i_l .OR. bgeo_i /= bgeo_i_l ) .OR. &
                       nue_i  /= nue_i_l .OR.  mue_i  /= mue_i_l    ) THEN
 
    IF (ldebug) THEN
      WRITE (*,*) '  pp_utilities:  computing prefactors_reff_1mom_icepoly()'
    END IF
    ...
    c_reffi_so_s(2,2)  = ( 0.5*(bgeo_i-1.0)/bgeo_i )                                       ! exponent
    ! .. r_eff for ice thermal, def. of Fu et al. (1998), hex plates, polydisperse PSD, needs two coefficients:
    c_reffi_th_s(1,1)  = ageo_i**(1.0/bgeo_i) / &                                      ! prefactor
    ...
    c_reffi_th_s(1,3)  = 9.0*ageo_i**(-2.0/bgeo_i)*rho_ice / 32.0 / &                  ! prefactor
         reff_gammfac_1mom( bgeo=bgeo_i, nue=nue_i, mue=mue_i, f=(bgeo_i-2.0)/bgeo_i )
    c_reffi_th_s(1,4)  = ( (2.0-bgeo_i)/bgeo_i )                                       ! exponent
    ! .. r_eff for ice thermal, def. of Fu et al. (1998), hex columns, polydisperse PSD, needs two coefficients:
    c_reffi_th_s(2,1)  = SQRT( 3.0*SQRT(3.0)*rho_ice / ( 8.0*ageo_i**(1.0/bgeo_i) ) ) / &   ! prefactor
         reff_gammfac_1mom( bgeo=bgeo_i, nue=nue_i, mue=mue_i,  f=0.5*(bgeo_i-1.0)/bgeo_i )
    c_reffi_th_s(2,2)  = ( 0.5*(1.0-bgeo_i)/bgeo_i )                                        ! exponent
    c_reffi_th_s(2,3)  = SQRT(3.0)*ageo_i**(1.0/bgeo_i)/4.0 / &                             ! prefactor
...
END SUBROUTINE prefactors_reff_1mom_icepoly
...
SUBROUTINE diag_reffc_1mom(its, ite, jts, jte, kts, kte, &
     hhl, qc, cloud_num, qc_thresh, c_reffc, r_effc)

  IMPLICIT NONE

  INTEGER (kind=iintegers), INTENT(in)  :: its, ite, jts, jte, kts, kte
  REAL (kind=wp), INTENT(in)        :: hhl(its:ite,jts:jte,kts:kte+1), qc(its:ite,jts:jte,kts:kte)
  REAL (kind=wp), INTENT(in)        :: cloud_num, qc_thresh, c_reffc
  REAL (kind=wp), INTENT(out)       :: r_effc(its:ite,jts:jte,kts:kte)

  REAL(KIND=wp),  PARAMETER :: eps2 = 1e-20
  REAL (kind=wp), PARAMETER :: bgeo_c = 3.0_wp
  REAL (kind=wp), PARAMETER :: z0_ncn   = 2000.0  ! up to this height constant CN-concentration
  REAL (kind=wp), PARAMETER :: z1oe_ncn = 2000.0  ! above, exp. decreasing with 1/e-height z1oe_ncn
  ...
  r_effc= 10.e-6_wp ! preset reasonable value
  DO k = kts, kte
!CDIR COLLAPSE
    DO j = jts, jte
      DO i = its, ite
        ...
        IF (qc(i,j,k) > qc_thresh) THEN
          ! cloud droplet number asssumed as an exponentially decreasing function of height.
          ...
          qnc = cloud_num * MIN(EXP((z0_ncn-zml_k)/z1oe_ncn), 1.0d0)
          ! average cloud droplet mass:
          zxc = MIN(MAX( qc(i,j,k)/(qnc+eps2), zxc_min), zxc_max )
          ! effective radius
          r_effc(i,j,k) = c_reffc *  zxc**(1.0/bgeo_c)
        ENDIF
      ENDDO
    ENDDO
  ENDDO
...
SUBROUTINE diag_reffi_1mom(its, ite, jts, jte, kts, kte, &
     t, rhol, qi, bgeo_i, qi_thresh, c_reffi_so, c_reffi_th, r_effi_so, r_effi_th)
  ...
  REAL(KIND=wp),  PARAMETER :: eps2 = 1e-20
  REAL (kind=wp), PARAMETER :: xi_min = 1.0e-12_wp  ! min. allowed mean ice particle mass
  REAL (kind=wp), PARAMETER :: xi_max = 1.0e-9_wp   ! max. allowed mean ice particle mass

  INTEGER (kind=iintegers) :: i, j, k
  REAL (kind=wp)       :: zxi, qni, temp


  r_effi_so  = 20.e-6_wp ! preset reasonable value
  r_effi_th  = 20.e-6_wp ! preset reasonable value
  DO k = kts, kte
!CDIR COLLAPSE
    DO j = jts, jte
      DO i = its, ite
...
DOUBLE PRECISION FUNCTION reffi_gammfac(p,f) 
  ...
  ! .. Assumption: N(D) = N0 * D^nu * exp(-lam*D^mu)
  !                  D  = ageo * x^bgeo
...
SUBROUTINE prefactors_reff_2mom(ldebug, c_reffc, c_reffi_so, c_reffi_th)

  USE wolken_konstanten, ONLY: cloud, ice, rho_ice
  ...
  CHARACTER(len=1000), SAVE :: cloud_s, ice_s
  CHARACTER(len=1000)       :: cloud_a, ice_a
  ...
  IF (firstcall .OR. TRIM(cloud_a) /= TRIM(cloud_s) .OR. TRIM(ice_a) /= TRIM(ice_s) ) THEN
    ...
    c_reffc_s = 0.5 * cloud%a_geo *  fracmoment_gamma(cloud,3.*cloud%b_geo)/fracmoment_gamma(cloud, 2.0*cloud%b_geo )
    ! .. r_eff for ice solar, def. of Key et al. (2002), hex plates:
    c_reffi_so_s(1,1)   = 2.0*SQRT(3.0)/(3.0*ice%a_geo**2*rho_ice)         * reffi_gammfac(ice, 1.0-2.0*ice%b_geo   )
    c_reffi_so_s(1,2)   = (1.0-2.0*ice%b_geo)
    ! .. r_eff for ice solar, def. of Key et al. (2002), hex columns:
    c_reffi_so_s(2,1)   = SQRT( 27.0*SQRT(3.0)/(128.0*ice%a_geo*rho_ice) ) * reffi_gammfac(ice, 0.5*(1.0-ice%b_geo) )
    c_reffi_so_s(2,2)   = (0.5*(1.0-ice%b_geo))
    ! .. r_eff for ice thermal, def. of Fu et al. (1998), hex plates, needs two coefficients:
    c_reffi_th_s(1,1) = 1.0 / ice%a_geo                                    / reffi_gammfac(ice, ice%b_geo           )
    c_reffi_th_s(1,2) = (-ice%b_geo)
    c_reffi_th_s(1,3) = 9.0*ice%a_geo**2*rho_ice / 32.0                    / reffi_gammfac(ice, 1.0-2.0*ice%b_geo   )
    c_reffi_th_s(1,4) = (2.0*ice%b_geo-1.0)
    ! .. r_eff for ice thermal, def. of Fu et al. (1998), hex columns, needs two coefficients:
    c_reffi_th_s(2,1) = SQRT( 3.0*SQRT(3.0)*ice%a_geo*rho_ice / 8.0 )      / reffi_gammfac(ice, 0.5*(1.0-ice%b_geo) )
    c_reffi_th_s(2,2) = (0.5*(ice%b_geo-1.0))
    c_reffi_th_s(2,3) = SQRT(3.0)/(4.0*ice%a_geo)                          / reffi_gammfac(ice, ice%b_geo           )
    c_reffi_th_s(2,4) = (-ice%b_geo)
...
END SUBROUTINE prefactors_reff_2mom
...
SUBROUTINE diag_reffc_2mom(its, ite, jts, jte, kts, kte, &
     hhl, qc, qnc, qc_thresh, c_reffc, r_effc)
  ...
  INTEGER (kind=iintegers), INTENT(in)  :: its, ite, jts, jte, kts, kte
  REAL (kind=wp), INTENT(in)        :: hhl(its:ite,jts:jte,kts:kte+1), qc(its:ite,jts:jte,kts:kte), qnc(its:ite,jts:jte,kts:kte)
  REAL (kind=wp), INTENT(in)        :: c_reffc, qc_thresh
  REAL (kind=wp), INTENT(out)       :: r_effc(its:ite,jts:jte,kts:kte)
    ...
    DO j = jts, jte
      DO i = its, ite
        ...
        IF (qc(i,j,k) > qc_thresh) THEN
          ...
          ! effective radius
          r_effc(i,j,k) = c_reffc *  zxc**cloud%b_geo
        ENDIF
      ENDDO
    ENDDO
  ENDDO
...
SUBROUTINE diag_reffi_2mom(its, ite, jts, jte, kts, kte, &
     qi, qni, qi_thresh, c_reffi_so, c_reffi_th, r_effi_so, r_effi_th)

  USE wolken_konstanten, ONLY: ice

  IMPLICIT NONE

  INTEGER (kind=iintegers), INTENT(in)  :: its, ite, jts, jte, kts, kte
  REAL (kind=wp), INTENT(in)        :: qi(its:ite,jts:jte,kts:kte), qni(its:ite,jts:jte,kts:kte)
  REAL (kind=wp), INTENT(in)        :: qi_thresh, c_reffi_so(2,2), c_reffi_th(2,4)
  REAL (kind=wp), INTENT(out)       :: r_effi_so(its:ite,jts:jte,kts:kte,2)
  REAL (kind=wp), INTENT(out)       :: r_effi_th(its:ite,jts:jte,kts:kte,2)
    ...
    DO j = jts, jte
      DO i = its, ite
        ...
        IF (qi(i,j,k) > qi_thresh) THEN
          ...
          zxi = MIN(MAX(qi(i,j,k)/(qni(i,j,k)+eps2),ice%x_min),ice%x_max)
...
END SUBROUTINE diag_reffi_2mom
