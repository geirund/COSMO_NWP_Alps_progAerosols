...
!  REAL replaced by REAL of kind wp.
...
! -------------
  ...
  REAL (KIND=wp)           , PARAMETER  :: &
    fbogus  =   0.0_wp   ! if (fbogus /= 0) then bogus data are produced by
...
! If obs. pressure is missing, get model pressure
  ...
  IF (kvar == nvar( 7)) THEN
     ...
     IF (nvarib == nmdi) THEN
        ...
        rzzz   =  ( nvarib * rfact (nvar(7)) - nalmcn*1._wp)
...
!                ------------------------------------------------
  ...
  kflgwr  =  ktotal + INT(kordgr/4._wp *(1._wp+epsy) +0.75_wp )
  koc6b   =  kordgr - INT(kordgr/4._wp *(1._wp+epsy) -0.25_wp )*4
...
!     check if obs./code type to be printed and location inside print region
      IF (     (noctrq == aofbuf(nobtp))                                       &
        ...
        roblat   =  (aofbuf(nlatit) - nltmcn) * 0.01_wp
        roblon   =  (aofbuf(nlongt) - nlgmcn) * 0.01_wp
...
!   --------------------
    roblat     =  REAL ( aofbuf(nlatit) - nltmcn, wp ) * 0.01_wp
    roblon     =  REAL ( aofbuf(nlongt) - nlgmcn, wp ) * 0.01_wp
...
!     IF (mpassiv == 0)                                                        &
    ...
    nodbuf (ipos+ 9) = NINT( zsurf   *1000._wp )
    nodbuf (ipos+10) = NINT( zio_tot *1000._wp )
    nodbuf (ipos+11) = NINT( zjo_tot *1000._wp )
    nodbuf (ipos+12) = NINT( roblon  *1000._wp )
    nodbuf (ipos+13) = NINT( roblat  *1000._wp )
...
!-------------------------------------------------------------------------------
  ...
  nflag   = nodbuf(kpos + 5)
  ntime   = nodbuf(kpos + 6)
  taof    = REAL ( nodbuf(kpos + 7), wp ) /60._wp
  nelevt  = nodbuf(kpos + 8)
  zsurf   = REAL ( nodbuf(kpos + 9), wp ) /1000._wp
  zio_tot = REAL ( nodbuf(kpos +10), wp ) /1000._wp
  zjo_tot = REAL ( nodbuf(kpos +11), wp ) /1000._wp
  roblon  = REAL ( nodbuf(kpos +12), wp ) /1000._wp
  roblat  = REAL ( nodbuf(kpos +13), wp ) /1000._wp
...
!   -------------------
    ...
    ELSE
      omlhed (nmlob,nhalt ) = REAL ( nelevt, wp )
    ENDIF
...
!   --------------------
    ...
    ELSE
      osghed (nsgob,nhalt ) = REAL ( nelevt, wp )
    ENDIF
...
! --------------------------------------------
  ...
  IF ((zstalt > rmdich) .AND. (nobtyp == ntemp .OR. nobtyp == npilot)) THEN
     IF ((rzzz > zstalt-MAX( rprlim*12,40.0_wp )) .OR. (isurfc == 1)) THEN
...
!   mandla  =  ibits (nlevin,nlidbp( 6),nibits (nlidoc))
    IF (     (NINT(rppp-70000._wp) /= 0)                                   &
        .AND.(NINT(rppp-50000._wp) /= 0)                                   &
        .AND.(NINT(rppp-40000._wp) /= 0)                                   &
        .AND.(NINT(rppp-30000._wp) /= 0)                                   &
        .AND.(NINT(rppp-25000._wp) /= 0)                                   &
        .AND.(NINT(rppp-20000._wp) /= 0)                                   &
        .AND.(NINT(rppp-15000._wp) /= 0)                                   &
        .AND.(NINT(rppp-10000._wp) /= 0)                                   &
        .AND.(NINT(rppp- 7000._wp) /= 0)                                   &
        .AND.(NINT(rppp- 5000._wp) /= 0)                                   &
        .AND.(NINT(rppp- 3000._wp) /= 0)                                   &
        .AND.(NINT(rppp- 2500._wp) /= 0)                                   &
        .AND.(NINT(rppp- 2000._wp) /= 0)                                   &
        .AND.(NINT(rppp- 1000._wp) /= 0)) THEN
       IF (lactrep)                                                            &
...
! -----------
  ...
  IF (      (rttt < t0_melt-90._wp) .OR. (rttt > t0_melt+60._wp)       &
      .OR. ((rttt > t0_melt+20) .AND. (rppp < 70000._wp))                  &
      .OR. ((rttt > t0_melt+ 5) .AND. (rppp < 50000._wp))                  &
      .OR. ((rttt > t0_melt- 5) .AND. (rppp < 40000._wp))) THEN
...
! IF ((mpassiv > 0) .OR. (nobtyp == npilot)) THEN
  ...
  IF ((isurfc == 1) .AND. (lsurfob)) THEN
     ...
     IF ((.NOT. lno) .OR. (lverif)) THEN
       ...
       IF (lbogus) rttt = MAX( 100._wp, rttt - 1._wp*fbogus )
     ...
     ENDIF
  ENDIF
  IF ((lbogus).AND.(rppp >= 66999._wp).AND.(rppp <= 86000._wp))        &
     rttt = MAX( 100._wp, rttt + 1._wp*fbogus )
...
! ----------------------------------------
  ...
  IF (     (rtdt < t0_melt-150._wp) .OR. (rtdt > t0_melt+40._wp)       &
      .OR. ((rtdt < t0_melt-90._wp) .AND. (isurfc == 1) .AND. (lsurfob))) THEN
    IF ((mpassiv == 0) .AND. (lactrep))                                        &
...
!    ==> eobs-corr / eswo = eobs / eseo
  ...
  roberr                        =   rherr1 / 100._wp
  IF (rheo  <  rrhlim)  roberr  =   rherr2 / 100._wp
  IF (rttt  <  rttlim)  roberr  =   rherr3 / 100._wp
  IF (isurfc == 1)      roberr  = 4*rherr1 / 100._wp
  ...
  IF ((isurfc == 1).AND. lsurfob ) THEN
     ...
     IF ((.NOT. lno) .OR. (lverif)) THEN
       IF (lno) roberr = rmdi
       IF (lno) nvarfw = insert( nvarfw, 1, nvfbps(4) )
       IF (lbogus) rheo = MAX( 0.01_wp , MIN( c1, rheo +.1_wp*fbogus ) )
  ...
  ENDIF
  IF ((lbogus) .AND. (rppp >= 66999._wp) .AND. (rppp <= 86000._wp))    &
     rheo = MAX( 0.01_wp , MIN( c1, rheo +0.1_wp*fbogus ) )
  IF (isurfc == 1) roberr = rmdi
...
! =========
  ...
  zfff     =    REAL ( nvarib, wp )
  IF  (      (zfff < -epsy) .OR. (zfff > 150.0_wp)                         &
       .OR. ((zfff > 90.0_wp) .AND. (rppp >= 70000._wp))) THEN
    IF ((mpassiv == 0) .AND. (lactrep))                                        &
...
! Set switch for surface/non-surface wind
  ...
  IF ((isurfc == 1) .AND. (lsurfob)) THEN
     ...
     IF ((.NOT. lno) .OR. (lverif)) THEN
       ...
       IF (lno) nvarfw = insert( nvarfw, 1, nvfbps(4) )
       IF (lbogus) THEN
          ruuu = ruuu - 1._wp*fbogus
          rvvv = rvvv + 1._wp*fbogus
       ENDIF
  ...
  ENDIF
  IF (lbogus) THEN
     IF ((rppp >= 55999._wp) .AND. (rppp <= 86000._wp)) THEN
        ruuu = ruuu + 1._wp*fbogus
        rvvv = rvvv + 1._wp*fbogus
     ELSEIF ((rppp >= 26000._wp).AND.(rppp <= 54000._wp)) THEN
        ruuu = ruuu + 1._wp*fbogus
     ELSEIF (rppp <= 24000._wp) THEN
        ruuu = ruuu - 1._wp*fbogus
        rvvv = rvvv - 1._wp*fbogus
     ENDIF
  ENDIF
...
!      precipitation amount exceeds check limit
       ...
       IF (nacout+ilen <= nmxoln) THEN
         ...
         outbuf(nacout+3) = INT(rr*100._wp)
         outbuf(nacout+4) = rtr
         DO icl = 1 , istrej
...
! write visibility to ODR
      osgbdy (nsgob , nbsvis) = REAL ( MIN( iexv , 99999 ), wp )
      ncwgfw  =  insert ( ncwgfw , nvisfl , nvvfbp )
    ENDIF

! pre-set flag for fog
    lfog  =  (nvisfl < 2) .AND. (REAL( iexv, wp ) < vfoglim) .AND. (iexv > 0)
  ENDIF
    ...
    IF ((npwwfl < 2) .AND. (lfog) .AND. (npww/10 /= 4)) THEN
      lfog  =        (npww >= 50) .OR. (npww == 28) .OR. (npww == 17)          &
               .AND. (     (      ((npww <= 71) .OR. (npww >= 76))             &
                            ...
                            .AND. (npww <= 97))                                &
                      .OR. (osgbdy(nsgob,nbsvis) < 100.1_wp))
      ilen = 6 + istrej
      IF (nacout+ilen <= nmxoln) THEN
        outbuf(nacout+1) = ilen
...
!       ncclqf  =  MAX( nflag , nflag1 , npwwfl ) + 2
    ...
    IF ((ncclfl <  2) .OR. ((lverif) .AND. (ncclfl /= imdi))) THEN
      roberr  =  1.0_wp
      IF (ncclqf == 1)  roberr = 2.0_wp
      IF (ncclqf == 2)  roberr = 3.0_wp
      IF (ncclqf == 3)  roberr = 4.0_wp
...
! write low cloud cover to ODR, and include low cloud flag in combined flag word
      osgbdy (nsgob , nbscl ) = REAL ( nccl, wp )
...
! write maximum 10' mean wind speed to ODR
      osgbdy (nsgob , nbsfme) = REAL ( iexv, wp )
      nflag   =  nflag / 2
...
! write maximum gust wind speed to ODR
      osgbdy (nsgob , nbsfgu) = REAL ( iexv, wp )
      nflag   =  nflag / 2
...
! write global radiation to ODR
      osgbdy (nsgob , nbsrad) = iexv * 10000.0_wp
      nflag   =  nflag / 2
...
!         -station altitude below 3700m and reports 500mb level
        ...
        IF(npcode ==  4 .AND. ABS(zstalt- 500._wp) > rh800) lpract=.TRUE.
        IF(npcode ==  5 .AND. ABS(zstalt-1000._wp) > rh800) lpract=.TRUE.
        IF(npcode ==  6 .AND. ABS(zstalt-2000._wp) > rh800) lpract=.TRUE.
        IF(npcode ==  7 .AND. ABS(zstalt-3000._wp) > rh800) lpract=.TRUE.
        IF(npcode ==  8 .AND. ABS(zstalt-4000._wp) > rh800) lpract=.TRUE.
...
! additional observation error due to extrapolation
        zadder = 0.04_wp * ABS( zstalt - rzzz )
...
! -------------------------------------------------------------------------
  ...
  IF ((nobtyp == nairep) .OR. (nobtyp == nsatob)) THEN
    mpassiv  =  0
    IF (nobtyp == nairep) THEN
      IF ((rzzz > rmdich) .AND. (rzzz < zsurf+50._wp))  mpassiv = 2
      IF ((rzzz < rmdich) .AND. (rppp > psurf(iob,job) -600._wp))          &
          mpassiv = 2
    ELSEIF (nobtyp == nsatob) THEN
      IF ((rzzz > rmdich) .AND. (rzzz < zsurf+200._wp))  mpassiv = 2
      IF ((rzzz < rmdich) .AND. (rppp > psurf(iob,job) -2000._wp))         &
          mpassiv = 2
    ENDIF
  ...
  IF (nobtyp == nsynop) THEN
    ...
    IF (nvarib /= nmdi) THEN
      IF ((nflag < 2) .AND. (ABS(nvarib-500) < 400)) THEN
        ldatpsv    = .TRUE.
        osgbdy (nsgob,nbspst) = REAL ( (nvarib - 500) * 10, wp )
      ELSEIF (lactrep) THEN
...
! endif
  ...
  zfff     =    REAL  ( nvarib, wp )
  IF  (      (zfff < -epsy)                                                    &
       .OR. ((zfff >  90.0_wp) .AND. (     (nobtyp == nsynop)              &
                                          .OR. (nobtyp == ndribu)))            &
       .OR. ((zfff > 150.0_wp) .AND. (     (nobtyp == nairep)              &
                                          .OR. (nobtyp == nsatob))) ) THEN
    IF ((mpassiv == 0) .AND. (lactrep))                                        & 
      neventd (nefneg,ncdtpp,nobtpp) = neventd(nefneg,ncdtpp,nobtpp) + 1
                                                                       EXIT Wind
...
! --------------------------------------------------
  ...
  IF ((rtdt <  t0_melt-150._wp) .AND. (mpassiv == 0) .AND. (lactrep))      &
     neventd (neqlow,ncdtpp,nobtpp) = neventd (neqlow,ncdtpp,nobtpp) + 1
  IF  (rtdt <  t0_melt-150._wp)                                 EXIT tem_hum
  IF ((rppp <  pqmin )         .AND. (mpassiv == 0) .AND. (lactrep))           &
     neventd (neq300,ncdtpp,nobtpp) = neventd (neq300,ncdtpp,nobtpp) + 1
...
!     ==> eobs-corr / eswo = eobs / eseo
  ...
  IF (rheo >= rhtsat) THEN
     IF ((rheo <  1.0_wp-epsy) .AND. (mpassiv == 0) .AND. (lactrep)) THEN
       IF (rttt <  t0_melt) neventd (neqsam,ncdtpp,nobtpp) =                   &
                       neventd (neqsam,ncdtpp,nobtpp) + 1
       IF (rttt >= t0_melt) neventd (neqsap,ncdtpp,nobtpp) =                   &
                       neventd (neqsap,ncdtpp,nobtpp) + 1
...
! ===============
  ...
  IF ((rttt2 < t0_melt-90._wp) .OR. (rttt2 > t0_melt+60._wp)) THEN
    IF ((mpassiv == 0) .AND. (lactrep))                                        &
      neventd (netext,ncdtpp,nobtpp) = neventd(netext,ncdtpp,nobtpp) + 1
                                                                 EXIT tem_hum_2m
  ENDIF
...
! check station model height difference
  ...
  IF    (     (.NOT. lseaonly .AND. zstalt >  altopsu(3))                      &
         .OR. (      lseaonly .AND. landsy )                                   &
         .OR. (fisdtt >      doromx(3)                    ) ) lno = .TRUE.
!        .OR. (fisdtt > MAX( doromx(3), zstalt/5._wp) ) ) lno = .TRUE.
  IF ((lno) .AND. (mpassiv == 0) .AND. (lactrep))                              &
     neventd (netalt,ncdtpp,nobtpp) = neventd(netalt,ncdtpp,nobtpp) + 1
  IF ((.NOT. lno) .OR. (lverif)) THEN
! note: extrapolation of T2m to the model surface level is done in 'sing_local'
     roberr = 1._wp + 0.003_wp *ABS(fisd)
...
! ---------------------------------
  ...
  IF ((rtdt2 < t0_melt-90._wp) .OR. (rtdt2 > t0_melt+40._wp)) THEN
    IF ((mpassiv == 0) .AND. (lactrep))                                        &
      neventd (neqlow,ncdtpp,nobtpp) = neventd(neqlow,ncdtpp,nobtpp) + 1
                                                                 EXIT tem_hum_2m
  ENDIF
...
!    ==> eobs-corr / eswo = eobs / eseo
  ...
  IF ((lno) .AND. (mpassiv == 0) .AND. (lactrep))                              &
     neventd (neqalt,ncdtpp,nobtpp) = neventd(neqalt,ncdtpp,nobtpp) + 1
  IF ((.NOT. lno) .OR. (lverif)) THEN
     roberr    =  rherr1 / 100._wp  +  0.0004_wp *ABS(fisd)
     IF ((lno) .OR. (mpassiv > 0))  roberr = rmdi
     IF  (lno)  nvarfw  =  insert( nvarfw, 1, nvfbps(4) )
     osgbdy (nsgob,nbsrh )  =  rheo
     osgbdy (nsgob,nbsqer)  =  roberr
...
rmod (ztti,ziv) = ztti - ziv *REAL(INT( ztti/ziv + epsy ),wp) + epsy
...
!  Preset thresholds for 4-d colocation
  ...
  IF (nobtyp == nairep) THEN
    ...
    rhzlmt = rhzlair
    rvtlmt = rvtlair * 100._wp
    rdplmt = rvtlair * 100._wp
    rprlmt = rprlim  * 100._wp
  ELSE
    rtmlmt = rtmlim
    rhzlmt = rhzlim
    rvtlmt = rvtlim
    rdplmt = rdplim * 100._wp
    rprlmt = rprlim * 100._wp
  ENDIF
  rhzlmt = rhzlmt **2
...
!               and disseminated from one and the same radiosonde ascent.)
  ...
  DO WHILE (      (   omlbdy(nodrrej,krej,nbtp)                                &
     ...
     laddq = .FALSE.
     IF (lrepla) THEN
       lmand =     (rmod( omlbdy(nodrrej,krej,nbtp),10000.0_wp ) < 2*epsy) &
              .OR. ( ABS( omlbdy(nodrrej,krej,nbtp)- 5000.0_wp ) < epsy)   &
              .OR. ( ABS( omlbdy(nodrrej,krej,nbtp)-15000.0_wp ) < epsy)   &
              .OR. ( ABS( omlbdy(nodrrej,krej,nbtp)-25000.0_wp ) < epsy)   &
              .OR. ( ABS( omlbdy(nodrrej,krej,nbtp)-85000.0_wp ) < epsy)   &
              .OR. ( ABS( omlbdy(nodrrej,krej,nbtp)-92500.0_wp ) < epsy)
...
!   If level of rejected report is mandatory, replace pressure and height
    ...
    lmand  =     (rmod( omlbdy(nodrrej,kclo,nbtp),10000.0_wp ) < 2*epsy)   &
