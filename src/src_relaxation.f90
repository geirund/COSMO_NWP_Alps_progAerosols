...
!  Added _wp to literal constants
...
!WL2011b
    tt_mic        , & ! fields for budget analysis
    qvt_mic       , &
    qrsgt_mic     , &
    qcit_mic      , &
    tt_rlx        , & 
...
!WL2011b
    lbud,         & ! if .TRUE., budgets are calculated
    itype_gscp      !
!WL2011e
...
!------------------------------------------------------------------------------
  ...
  IF (lspubc .EQV. .TRUE.) THEN
    ...
    ! height vcoord%vert_coord (refering to z=0) is used, i.e. it is assumed that the
    ...
    DO k = 1, ke
      zlev = 0.5_wp*(vcoord%vert_coord(k)+vcoord%vert_coord(k+1))
      IF( zlev >= rdheight ) THEN
        rdcoef(k) = 1.0_wp - COS( pi*(zlev-rdheight)/(vcoord%vert_coord(1)-rdheight) )
...
!-----------------
  REAL (KIND=wp),     POINTER :: &
    ...
    qi_new     (:,:,:)  => NULL(),   & ! QI at tlev=nnew
    qi_now     (:,:,:)  => NULL(),   & ! QI at tlev=nnow
    qr_new     (:,:,:)  => NULL(),   & ! QR at tlev=nnew
    qr_now     (:,:,:)  => NULL(),   & ! QR at tlev=nnow
    qs_new     (:,:,:)  => NULL(),   & ! QS at tlev=nnew
    qs_now     (:,:,:)  => NULL(),   & ! QS at tlev=nnow
    qg_new     (:,:,:)  => NULL(),   & ! QG at tlev=nnew
    qg_now     (:,:,:)  => NULL()      ! QG at tlev=nnow
...
! Reduction coefficients for lateral boundary relaxation if lateral radiation condition is used
  ...
  IF (lradlbc) THEN
    zrelfac_u = MIN(1._wp,2._wp*relax_fac)
    zrelfac_s = MIN(1._wp,relax_fac)
    zrelfac_p = 0._wp
  ELSE
    zrelfac_u = 1._wp
    zrelfac_s = 1._wp
...
ELSE
  DO j = jstartpar, jendpar
    DO i = istartpar, iendpar
      zmys(i,j) = zrelfac_s*rmy(i,j,1) / (1.0_wp + rmy(i,j,1))
      zmyp(i,j) = zrelfac_p*rmy(i,j,1) / (1.0_wp + rmy(i,j,1))
      zmyu(i,j) = zrelfac_u*rmy(i,j,2) / (1.0_wp + rmy(i,j,2))
      zmyv(i,j) = zrelfac_u*rmy(i,j,3) / (1.0_wp + rmy(i,j,3))
    ENDDO
  ENDDO
ENDIF
#ifdef NECSX
...
IF (lbud) THEN
  IF (ntstep < nstop) THEN
    tt_rlx (:,:,:)    = (t(:,:,:,nnew) - t(:,:,:,nnow))/dt
    qvt_rlx (:,:,:)   = (qv_new(:,:,:) - qv_now(:,:,:))/dt
    qcit_rlx (:,:,:)  = (qc_new(:,:,:) - qc_now(:,:,:))/dt 
    IF(ASSOCIATED(qi_now)) qcit_rlx (:,:,:)  = qcit_rlx (:,:,:) + (qi_new(:,:,:) - qi_now(:,:,:))/dt
    qrsgt_rlx (:,:,:) = 0.0_wp 
    IF(ASSOCIATED(qr_now)) qrsgt_rlx (:,:,:) = qrsgt_rlx(:,:,:) + (qr_new(:,:,:) - qr_now(:,:,:))/dt
    IF(ASSOCIATED(qs_now)) qrsgt_rlx (:,:,:) = qrsgt_rlx(:,:,:) + (qs_new(:,:,:) - qs_now(:,:,:))/dt
    IF(ASSOCIATED(qg_now)) qrsgt_rlx (:,:,:) = qrsgt_rlx(:,:,:) + (qg_new(:,:,:) - qg_now(:,:,:))/dt
  ELSE
    ! no relaxation is done at last timestep
    tt_rlx (:,:,:)    = 0.0_wp
    qvt_rlx (:,:,:)   = 0.0_wp
    qcit_rlx (:,:,:)  = 0.0_wp
    qrsgt_rlx (:,:,:) = 0.0_wp
        ...
        DO i = istartpar, iendpar
          ...
          ELSE
            ...
            IF ( lexpl_lbc ) THEN
              zmyq(i,j,k) = rmyq(i,j)
            ELSE
              zmyq(i,j,k) = rmyq(i,j) / (1.0_wp + rmyq(i,j))
            ENDIF
          ENDIF
        ENDDO
      ENDDO
    ...
    IF ( izlbcqi /= T_LBC_FILE ) THEN
      ...
      ! diameter given in the below SR set_qni_from_qi_sb_scalar().
        ...
        DO k = 1, ke
          DO j = jstartpar, jendpar
            DO i = istartpar, iendpar
              ztrbd(i,j,k,idt_qc) = set_qni_from_qi_sb_scalar(ztrbd(i,j,k,idt_qi))
            ENDDO
          ENDDO
        ENDDO
#endif
...
!WL2011b
  IF (lbud) THEN
    tt_rlx (:,:,:)    = (t(:,:,:,nnew) - t(:,:,:,nnow))/dt - tt_rlx (:,:,:)
    qvt_rlx (:,:,:)   = (qv_new(:,:,:) - qv_now(:,:,:))/dt - qvt_rlx (:,:,:)
    qcit_rlx (:,:,:)  = (qc_new(:,:,:) - qc_now(:,:,:))/dt  - qcit_rlx (:,:,:)
    IF(ASSOCIATED(qi_now))  qcit_rlx (:,:,:)  =  qcit_rlx (:,:,:) +  (qi_new(:,:,:) - qi_now(:,:,:))/dt 
    qrsgt_rlx (:,:,:) = - qrsgt_rlx(:,:,:)
    IF(ASSOCIATED(qr_now))  qrsgt_rlx (:,:,:) =  qrsgt_rlx(:,:,:) +  (qr_new(:,:,:) - qr_now(:,:,:))/dt
    IF(ASSOCIATED(qs_now))  qrsgt_rlx (:,:,:) =  qrsgt_rlx(:,:,:) +  (qs_new(:,:,:) - qs_now(:,:,:))/dt
...
!------------------------------------------------------------------------------
!WL2011b
  !store current fields to latent heating array before satad
  IF (lbud) THEN
  ...
  DO iztrcr = 1, trcr_get_ntrcr()
    ...
    IF ( izsp_adv_lf(iztrcr) == 1_iintegers .AND. izadv(iztrcr) == T_ADV_ON ) THEN
      ...
      DO  k = 1, ke
        DO j = jstartpar, jendpar
          DO i = istartpar, iendpar
            zdd = epsass*( ztrcr_new(i,j,k) - 2._wp*ztrcr_now(i,j,k)      &
                         + ztrcr_old(i,j,k) )
