...
!------------------------------------------------------------------------------
  ...
  zforrange2 = 0._wp                     ! DL added this line
...
SUBROUTINE open_file (nudat, datname, ymode, yformat, icomm, my_id, npes,   &
                      lasync, llock_file, idbglv, yerrmsg, ierror)
...
LOGICAL                  , INTENT(IN)    ::  &
  ...
  llock_file ! indicates whether to use lock files or not
...
CHARACTER (LEN=300)                 :: &
  datname_new                            ! copy of datname for renaming
...
CASE ('grb1')
  ...
  ELSE
    ...
    IF (my_id == 0) THEN
      datname_new = datname(1:LEN_TRIM(datname))
      IF (llock_file .AND. (ymode(1:1).EQ.'w') ) THEN
        ! rename to lockfile
        datname_new = datname_new(1:LEN_TRIM(datname_new)) // '.lck'
      END IF
      CALL copen(nudatc,datname_new(1:LEN_TRIM(datname_new)),ymode,ierrc)
      nudat = nudatc
      IF (ierrc /= 0) THEN
        yerrmsg = 'Error opening '//datname
        ! this error message has to be broadcasted to all other PEs
...
! <fuo_restart
! note: we don't want model to stop if a restart file is already present. this
!       has caused problem in operations when a run has been restarted manually
!       and the operators did not clean away old restart files.
...
SUBROUTINE close_file (nudat, yformat, icomm, my_id, npes, lasync,    &
                       llock_file, idbglv, yerrmsg, ierror)
      ...
      CASE ('COSMO')
        ...
        IF     ( (zpv(1) < 500.0_wp) .AND. (pvz_in(1) < 500.0_wp) ) THEN
          ...
          ELSE
            ...
            DO k = 1, kedim+1
              IF (ABS(zpv(6+k)-pvz_in(6+k)) > 1.0E-3_wp) THEN
                ierror = 24
              ENDIF
            ENDDO
          ENDIF
        ELSEIF ( (zpv(1) > 500.0_wp) .AND. (pvz_in(1) > 500.0_wp) ) THEN
          ...
          ELSE
            ! just check the vertical coordinate parameters
            DO k = 1, kedim+1
              IF (ABS(zpv(4+k)-pvz_in(4+k)) > 1.0E-3_wp) THEN
                ierror = 24
              ENDIF
            ENDDO
          ENDIF
        ELSEIF ( (zpv(1) < 500.0_wp) .AND. (pvz_in(1) > 500.0_wp) ) THEN
          ! just check the vertical coordinate parameters
          DO k = 1, kedim+1
            IF (ABS(zpv(6+k)-pvz_in(4+k)) > 1.0E-3_wp) THEN
              ierror = 24
            ENDIF
          ENDDO
        ELSEIF ( (zpv(1) > 500.0_wp) .AND. (pvz_in(1) < 500.0_wp) ) THEN
          ! just check the vertical coordinate parameters
          DO k = 1, kedim+1
            IF (ABS(zpv(4+k)-pvz_in(6+k)) > 1.0E-3_wp) THEN
              ierror = 24
            ENDIF
          ENDDO
        ENDIF
...
!------------------------------------------------------------------------------
    ...
    IF (ydatreptyp(1:10) == 'rotated_ll') THEN
      ...
      IF (ABS(zpollat_grib - zsouthp_lat) > 1.0E-5_wp) ierror = 6
      IF (ABS(zpollon_grib - zsouthp_lon) > 1.0E-5_wp) ierror = 7
    ENDIF
  ...
  DO i = 0, npe-1
    IF (irecverror(i)/= 0) THEN
      ...
      IF (myid == i) THEN
        IF (ierror < 10) THEN
          WRITE(*,*)
          WRITE(*,*) 'SUBROUTINE check_lm_grid:'
          WRITE(*,*) 'ierror: ', ierror
          WRITE(*,*)
        ...
        ENDIF
          ...
          WRITE (*,*)
          WRITE (*,*) 'Temporary bug fix: resetting ierror=0'
          WRITE (*,*)
          ierror = 0
