...
!==============================================================================
  ...
  DO  j = jstart, jend
    ...
    ELSE
      DO i = istart, iend
        zmyfac(i) = 1.0_wp - rmy(i,j,1)/( 1.0_wp + rmy(i,j,1) )
      ENDDO
    END IF
...
!******************************************************************************
  ...
  DO k = 1, kdim
    DO i = isc, iec
      IF( pt(i,k) - ctmelt.GT.0._wp) THEN     
          zc3    = b2w
          zc4    = b4w
        ELSE                            
          zc3    = b2i
  ...
  DO k = kdim-1, 2, -1     ! Vertical loop over layers
    ...
    DO i = isc, iec
      IF( loflag(i) ) THEN        ! cloud base not found yet
        ...
        IF(zbuo.GT.0._wp) mlab(i,k) = 1 ! sub-cloud indicator for positve buoyancy
...
!      
    DO i = isc, iec
      IF( loflag(i) .AND. zqu(i,k).NE.zqold(i) ) THEN
        ...
        zbuo =   ztu  (i,k)*( 1.0_wp + rvd_m_o*zqu  (i,k) )  &
               - ztenh(i,k)*( 1.0_wp + rvd_m_o*zqenh(i,k) ) + 0.5_wp
        IF(zbuo.GT.0._wp) THEN ! Test for buoyancy
  ...
  DO k = kdim-1, 2, -1
    ...
    DO i = isc, iec
      IF(loflag(i)) THEN
        ...
        zlu  (i,k) =  zmfulk*( 1._wp/MAX(cmfcmin,zmfu(i,k)) )
        zqu(i,k) =  zmfuqk*( 1._wp/MAX(cmfcmin,zmfu(i,k)) )
        ztu (i,k) = (zmfusk*( 1._wp/MAX(cmfcmin,zmfu(i,k)) ) -      &
                                  pfih(i,k))/cp_d  
    ...
    DO i = isc, iec
      IF( loflag(i) .AND.  zqu(i,k).NE.zqold(i) ) THEN
        zlu (i,k) = zlu(i,k) + zqold(i) - zqu(i,k)
        zbuo      =  ztu(i,k)*(1._wp+rvd_m_o*zqu(i,k))  &
                    - ztenh(i,k)*(1._wp+rvd_m_o*zqenh(i,k))
    ...
    DO i = isc, iec
      IF (locum(i)) THEN ! for convective grid points only
        ...
        pdtke_con(i,k) = MAX( 0.0_wp, g*r_d/pph(i,k) *    &
                         ( zmfus(i,k)/cp_d + &
                           ztenh(i,k)*zmfuq(i,k)*rvd_m_o/(1.0_wp-rvd_m_o*zqenh(i,k)) ) )
        ! RA
      END IF
    END DO
  END DO
...
!------------------------------------------------------------------------------
    ...
    DO i = isc, iec
      ...
      IF(plflag(i)) THEN        ! only, if ascent still continues
        ...
        zqsat = zqsat*zcor
        zcond1   = (pqv(i)-zqsat)/(1._wp+zc5*zqsat*zcor/(pt(i)-zc4)**2)
