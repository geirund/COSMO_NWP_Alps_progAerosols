...
USE data_runcontrol , ONLY :   &
    ...
    itype_pert,   & ! for adding random perturbations (0: none, 1: at init, 2: at every steps)
    rperturb,     & ! coefficient for adding relative perturbations
!OCH >>
    lcmprates,    & ! .TRUE.: allocate cmp rates and calculate
!OCH <<
!WL2011b 
    lbud,         & ! .TRUE.: allocate budget terms and calculate them
    lbud_avg,     & ! .TRUE.: average tendencies are computed in lmorg.f90
    lbud_h_theta, & ! .TRUE.: calc heat budget for pot. temperature
    itheta_adv,   & !
    irunge_kutta, &
!WL2011e 
    lrelax,       & ! if lgen=.TRUE.and lperi=.TRUE.: relaxation towards 
                    ! profile(.TRUE.) or without (.FALSE.)
!SBoeing, b
    llsf,         & ! large scale forcint
!SBoeing, e
    lrelax_soil,  & ! if lgen=.TRUE.and lperi=.TRUE. and lsoil=.true. and 
                    ! lsoil_multlay=.true.: relaxation towards profile in
                    ! soil (.TRUE.) or without (.FALSE.)
    ...
    lnc_const,    & !constant NCLOUD in src_twomom_sb.f90
    lni_const_imm       !constnat NICE in src_twomom_sb.f90
...
! -180.0 <= pollon, polgam, startlon_tot <= 180.0
  IF ( (-180.0_wp > pollon) .OR. (pollon > 180.0_wp) ) THEN
    PRINT *,' *** WRONG VALUE OF VARIABLE pollon: ',pollon,'  ***'
    ierrstat = 1002
  ENDIF
  IF ( (-180.0_wp > polgam) .OR. (polgam > 180.0_wp) ) THEN
...
! -90.0 <= pollat, startlat_tot <= 90.0
  IF ((-90.0_wp > pollat) .OR. (pollat > 90.0_wp)) THEN
...
! Variables for default values
  INTEGER (KIND=iintegers)   ::       &
    ...
    itype_pert_d      ! determines pertubation type
  ...
  REAL (KIND=wp)             ::       &
    ...
    rperturb_d      ! coefficient for relative perturbation
...
!OCH<<
    !WL2011b
    lbud_d,       &! if lbud_h_d=.TRUE.: allocate budget terms and calculate
    lbud_avg_d,   &! if lbud_avg_d=.True., average tendencies are computed in lmorg.f90
    lbud_h_theta_d, & ! if lbud_h_theta_d=.TRUE.: pot. temp budget terms are calculated
    !WL2011e
    lrelax_d,     & ! if lgen=.TRUE.and lperi=.TRUE.: relaxation towards
                    ! profile(.TRUE.) or without (.FALSE.)
!Sboeing, b
    llsf_d,       & ! lsf flag
!Sboeing, e
    lrelax_soil_d,& ! if lgen=.TRUE.and lperi=.TRUE.: relaxation towards
                    ! profile in soil (.TRUE.) or without (.FALSE.)
    lnc_const_d,  & !constant NCLOUD in src_twomom_sb.f90
    lni_const_imm_d     !constnat NICE in src_twomom_sb.f90
...
! Define the namelist group
  NAMELIST /runctl/ nstop, hstart, hstop, dt,                               &
                    ...
                    l_cosmo_art, l_pollen, itype_pert, rperturb, lsppt,     &
                    lrelax, llsf,lrelax_soil,       &
                    !WL2011b
                    lbud, lbud_h_theta, lbud_avg, lcmprates, lnc_const, lni_const_imm
                    !WL2011e
...
! LS, e
  ...
  !WL2011b
  ! initialize budget switches: .FALSE.
  lbud_d          = .FALSE.
  lbud_avg_d      = .FALSE.
  lbud_h_theta_d  = .FALSE.
  !WL2011e
  !OCH >>
  lcmprates_d     = .FALSE.
  !OCH <<
...
! LS, e  
  ...
  lbud          = lbud_d
  lbud_avg      = lbud_avg_d
  lbud_h_theta  = lbud_h_theta_d
  !WL2011e
  !OCH
  lcmprates      = lcmprates_d
  !OCH
  lnc_const      = lnc_const_d
  lni_const_imm      = lni_const_imm_d
!------------------------------------------------------------------------------
!- Section 3: Input of the namelist values
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
  ...
  IF ( hincmxu /= hincmxu_d) THEN
    IF ( ABS(NINT(hincmxu) - hincmxu) > 1.0E-5_wp) THEN
...
!US   IF ( (hlastmxu <= hstart) .AND. (hstart < hlastmxu + hincmxu) ) THEN
  ...
  IF ( hincmxt /= hincmxt_d) THEN
    IF ( ABS(NINT(hincmxt) - hincmxt) > 1.0E-5_wp) THEN
      ! then it is not a full hour, only allow 0.25 and 0.5
...
!US   IF ( (hlastmxt <= hstart) .AND. (hstart < hlastmxt + hincmxt) ) THEN
  ...
  ! check perturbation
  IF (itype_pert < 0_iintegers .OR. itype_pert > 2_iintegers) THEN
    PRINT *,'WARNING  ***  Wrong value for itype_pert = ', itype_pert, ' ***'
    PRINT *,'         ***  must be >= 0 and <= 2   ***' 
    ierrstat = 645342
  ENDIF
  IF (itype_pert /= 0 .AND. rperturb >= 0.0_wp .AND. rperturb < 1.0_wp*rprecision) THEN
    PRINT *,'WARNING  ***  Invalid value for rperturb = ', rperturb, ' ***'
    PRINT *,'         ***  must be >= ',1.0_wp*rprecision,'  ***' 
    PRINT *,'         ***  Increasing to this minimum value ****'
    rperturb = 1.0_wp*rprecision
  ENDIF
  IF (itype_pert /= 0 .AND. rperturb < 0.0_wp .AND. rperturb > -1.0_wp) THEN
    PRINT *,'WARNING  ***  Invalid value for rperturb = ', rperturb, ' ***'
    PRINT *,'         ***  must be <= ',-1.0_wp,'  ***' 
    PRINT *,'         ***  Increasing to this maximum value ****'
    rperturb = -1.0_wp
  ENDIF
...
!WL2011b
  IF (lbud) THEN
    IF ((itheta_adv==1.OR.itheta_adv==2).AND.(.NOT.lbud_h_theta)) THEN
      PRINT *,' WARNING  *** lbud_h_theta is set false, but theta ***'
      PRINT *,'          *** dynamics is used ***'
      !PRINT *,'WARNING  ***  lbud_h_theta is set .true. ***'
      !lbud_h_theta=.True.
    ENDIF 
    IF (irunge_kutta==2) THEN
      PRINT *,'WARNING  ***  lbud computed tt_adv correctly only in case of irunge_kutta=1  ***'
    END IF
  ENDIF
!WL2011e
...
! LS, b
  ...
  IF (.NOT. lartif_data) THEN
    IF (lrelax) THEN
      PRINT *,' ERROR    *** lrelax = .TRUE. only if lartif_data = .TRUE. *** '
      ierrstat = 1002
    ENDIF
    IF (llsf) THEN
      PRINT *,' ERROR    *** llsf = .TRUE. only if lartif_data = .TRUE. *** '
      ierrstat = 1002
    ENDIF
  ENDIF
  ! Check for relaxation
!  IF (.NOT. lsoil) THEN
!    IF (lrelax_soil) THEN
!      PRINT *,' ERROR    *** lrelax_soil = .TRUE. only if lsoil = .TRUE. *** '
!      ierrstat = 1002
!    ENDIF
!  ENDIF
...
IF (nproc > 1) THEN
  ...
  IF (my_world_id == 0) THEN
    ...
    logbuf  (40) = lrelax
    logbuf  (41) = llsf
    logbuf  (42) = lrelax_soil
    !WL2011b
    ! namelist entry for heat budget terms
    logbuf  (43) = lbud
    logbuf  (44) = lbud_avg
    logbuf  (45) = lbud_h_theta
    !WL2011e
    logbuf  (46) = lsppt
    !OCH
    logbuf  (47) = lcmprates
    !OCH
    logbuf  (48) = lnc_const
    logbuf  (49) = lni_const_imm

    charbuf ( 1) = ydate_ini
    charbuf ( 2) = ydate_end
    charbuf ( 3) = ydate_bd
  ENDIF
  CALL distribute_values (intbuf, 18, 0, imp_integers,  icomm_world, ierr)
  CALL distribute_values (realbuf,10, 0, imp_reals,     icomm_world, ierr)
  CALL distribute_values (logbuf, 49, 0, imp_logical,   icomm_world, ierr)
  ...
  IF (my_world_id /= 0) THEN
    ...
    lrelax       = logbuf  (40)
    llsf         = logbuf  (41)
    lrelax_soil  = logbuf  (42)
    !WL2011b
    ! namelist entry for heat budget terms
    lbud         = logbuf (43)
    lbud_avg     = logbuf (44)
    lbud_h_theta = logbuf (45)
    !WL2011e
    lsppt        = logbuf  (46)
    !OCH
    lcmprates    = logbuf  (47)
...
IF (my_world_id == 0) THEN
  ...
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                                          'lrelax',lrelax, lrelax_d   ,' L '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                                          'llsf',llsf, llsf_d   ,' L '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                           'lrelax_soil',lrelax_soil, lrelax_soil_d   ,' L '
  !OCH
!  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
!                        'lcmprates'    , lcmprates    , lcmprates_d  , ' L '
  ...
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                        'lbud'         , lbud         , lbud_d       , ' L '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                        'lbud_avg'     , lbud_avg     , lbud_avg_d   , ' L '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                        'lbud_h_theta' , lbud_h_theta ,lbud_h_theta_d, ' L '
  !WL2011e
  WRITE (nuspecif, '(A2)')  '  '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                        'lnc_const'    , lnc_const    , lnc_const_d  , ' L '
  WRITE (nuspecif, '(T8,A,T21,L12  ,T40,L12  ,T59,A3)')                      &
                        'lni_const_imm', lni_const_imm , lni_const_imm_d  , ' L '
...
! Variables for default values
  REAL (KIND=wp)             ::       &
    ...
    c_smag_d,     & ! Smagorinsky constant
    c_pr_d,       & ! Turbulent Prandtl number
...
!------------------------------------------------------------------------------
  ...
  ! c_smag  
  IF (c_smag < 0.0_wp) THEN
    ! check for possible values
    PRINT *, ' ERROR  *** c_smag must be > 0!  actual value = ', c_smag  , ' *** '
    ierrstat = 1002
  ELSE
    ! check for meaningful values
    IF ( (c_smag  > 0.30_wp) .OR. (c_smag < 0.10_wp) ) THEN
      PRINT *, ' WARNING  *** c_smag = ', c_smag, ' is not a meaningful value!  '
      PRINT *, '          *** Best interval is [ 0.1 , 0.3 ] *** '
    ENDIF
  ENDIF
  ...
  IF (c_pr <= 0.0_wp) THEN
    ! check for possible values
    PRINT *, ' ERROR  *** c_pr must be > 0!  actual value = ', c_pr  , ' *** '
    ierrstat = 1002
  ELSE
    ! check for meaningful values
    IF ( (c_pr  > 1.10_wp) .OR. (c_pr < 0.20_wp) ) THEN
      PRINT *, ' WARNING  *** c_pr = ', c_pr, ' is not a meaningful value!  '
      PRINT *, '          *** Best interval is [ 0.2 , 1.1 ] *** '
    ENDIF
  ENDIF
...
IF (nproc > 1) THEN
  ...
  CALL distribute_values (realbuf, 39, 0, imp_reals,    icomm_world, ierr)
...
! ---------------------------------------------------------------------
  ...
  IF (lmetr .EQV. .FALSE.) THEN
    ...
    IF ( lcori .EQV. .TRUE. ) THEN ! Set fc to value for 45deg north
      fc (:,:) = z2om * sin(pi*startlat_tot/180._wp)
    ENDIF

    ! Modification due to deep atmosphere, Ronny Petrik
    IF (lcori_deep) THEN 
      ! Set fccos to a value for 45deg north
      fccos (:,:) = z2om * cos(pi*startlat_tot/180._wp)
    ENDIF
    ...
    IF ( lexpl_lbc ) THEN
      ...
      DO j = 1 , je
        ...
        DO i = 1 , ie
          ...
          IF (zdrh <= 0.0_wp) THEN
            rmy(i,j,1) = 1.0_wp / ztaur
          ELSE IF ( zdrh >= rlwidth ) THEN
            rmy(i,j,1) = 0.0_wp
          ELSE
            ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmy(i,j,1) = 1.0_wp - COS( pi * (rlwidth-zdrh) / rlwidth )
! JF:             rmy(i,j,1) = rmy(i,j,1) / ( 2.0_wp*ztaur )
            ! f_relaxation = EXP( -6.0 * x/L )
            rmy(i,j,1) = EXP( -zexpfac * zdrh / rlwidth ) / ztaur
            ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmy(i,j,1) = ( zdrh / rlwidth - 1.0_wp )**2 / ztaur
            ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmy(i,j,1) = - ( zdrh / rlwidth - 1.0_wp )**3 / ztaur
          ENDIF

          IF (zdrh <= 0.0_wp) THEN
            rmyq(i,j) = 1.0_wp / ztaur
          ELSE IF ( zdrh >= zqrlwid ) THEN
            rmyq(i,j) = 0.0_wp
          ELSE
            ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmyq(i,j) = 1.0_wp - COS( pi * (zqrlwid-zdrh) / zqrlwid )
! JF:             rmyq(i,j) = rmyq(i,j)  / ( 2.0_wp*ztaur )
            ! f_relaxation = EXP( -6.0 * x/L )
            rmyq(i,j) = EXP( -zexpfac * zdrh / zqrlwid ) / ztaur
            ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmyq(i,j) = ( zdrh / zqrlwid - 1.0_wp )**2 / ztaur
            ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmyq(i,j) = - ( zdrh / zqrlwid - 1.0_wp )**3 / ztaur
          ENDIF
          ...
          IF (zdru <= 0.0_wp) THEN
            rmy(i,j,2) = 1.0_wp / ztaur
          ELSE IF ( zdru >= rlwidth ) THEN
            rmy(i,j,2) = 0.0_wp
          ELSE
            ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmy(i,j,2) = 1.0_wp - COS( pi * (rlwidth-zdru) / rlwidth )
! JF:             rmy(i,j,2) = rmy(i,j,2) / ( 2.0_wp*ztaur )
            ! f_relaxation = EXP( -6.0 * x/L )
            rmy(i,j,2) = EXP( -zexpfac * zdru / rlwidth ) / ztaur
            ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmy(i,j,2) = ( zdru / rlwidth - 1.0_wp )**2 / ztaur
            ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmy(i,j,2) = - ( zdru / rlwidth - 1.0_wp )**3 / ztaur
          ENDIF
          ...
          IF (zdrv <= 0.0_wp) THEN
            rmy(i,j,3) = 1.0_wp / ztaur
          ELSE IF ( zdrv >= rlwidth ) THEN
            rmy(i,j,3) = 0.0_wp
          ELSE
            ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmy(i,j,3) = 1.0_wp - COS( pi * (rlwidth-zdrv) / rlwidth )
! JF:             rmy(i,j,3) = rmy(i,j,3) / ( 2.0_wp*ztaur )
            ! f_relaxation = EXP( -6.0 * x/L )
            rmy(i,j,3) = EXP( -zexpfac * zdrv / rlwidth ) / ztaur
            ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmy(i,j,3) = ( zdrv / rlwidth - 1.0_wp )**2 / ztaur
            ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmy(i,j,3) = - ( zdrv / rlwidth - 1.0_wp )**3 / ztaur
          ENDIF
      ...
      IF( l2dim ) THEN  ! Recalculate the Davis Parameter for 2-D Version
        ...
        IF ( .NOT.lperi_x ) THEN ! only for non-periodicity in x-direction
          ...
          DO i = 1 , ie
            ...
            zdrh  =  zifac * zdrh
            IF (zdrh <= 0.0_wp) THEN
              rmy(i,j2dim,1) = 1.0_wp / ztaur
            ELSE IF ( zdrh >= rlwidth ) THEN
              rmy(i,j2dim,1) = 0.0_wp
            ELSE
              ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmy(i,j2dim,1) = 1.0_wp - COS( pi * (rlwidth-zdrh) / rlwidth )
! JF:             rmy(i,j2dim,1) = rmy(i,j2dim,1) / ( 2.0_wp*ztaur )
              ! f_relaxation = EXP( -6.0 * x/L )
              rmy(i,j2dim,1) = EXP( -zexpfac * zdrh / rlwidth ) / ztaur
              ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmy(i,j2dim,1) = ( zdrh / rlwidth - 1.0_wp )**2 / ztaur
              ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmy(i,j2dim,1) = - ( zdrh / rlwidth - 1.0_wp )**3 / ztaur
            ENDIF
            IF (zdrh <= 0.0_wp) THEN
              rmyq(i,j2dim) = 1.0_wp / ztaur
            ELSE IF ( zdrh >= zqrlwid ) THEN
              rmyq(i,j2dim) = 0.0_wp
            ELSE
            ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmyq(i,j2dim) = 1.0_wp - COS( pi * (zqrlwid-zdrh) / zqrlwid )
! JF:             rmyq(i,j2dim) = rmyq(i,j2dim) / ( 2.0_wp*ztaur )
            ! f_relaxation = EXP( -6.0 * x/L )
              rmyq(i,j2dim) = EXP( -zexpfac * zdrh / zqrlwid ) / ztaur
            ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmyq(i,j2dim) = ( zdrh / zqrlwid - 1.0_wp )**2 / ztaur
            ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmyq(i,j2dim) = - ( zdrh / zqrlwid - 1.0_wp )**3 / ztaur
            ENDIF

            zdru =  MIN ( i_td - istartu_tot + 0.75_wp , iendu_tot - i_td + 0.75_wp )
            zdru  =  zifac * zdru
            IF (zdru <= 0.0_wp) THEN
              rmy(i,j2dim,2) = 1.0_wp / ztaur
            ELSE IF ( zdru >= rlwidth ) THEN
              rmy(i,j2dim,2) = 0.0_wp
            ELSE
            ! f_relaxation = 0.5 * ( 1.0 - COS( pi * ( 1 - x/L ) ) )
! JF:             rmy(i,j2dim,2) = 1.0_wp - COS( pi * (rlwidth-zdru) / rlwidth )
! JF:             rmy(i,j2dim,2) = rmy(i,j2dim,2) / ( 2.0_wp*ztaur )
            ! f_relaxation = EXP( -6.0 * x/L )
              rmy(i,j2dim,2) = EXP( -zexpfac * zdru / rlwidth ) / ztaur
            ! f_relaxation = ( x/L - 1.0 )**2
! JF:             rmy(i,j2dim,2) = ( zdru / rlwidth - 1.0_wp )**2 / ztaur
            ! f_relaxation = - ( x/L - 1.0 )**3
! JF:             rmy(i,j2dim,2) = - ( zdru / rlwidth - 1.0_wp )**3 / ztaur
            ENDIF
    ...
    ELSE
      ...
      IF (llm) THEN
        ...
        ztanfac = 0.125_wp
      ELSE
        zdtddtr = ABS(dt) / 300.0_wp * 0.5_wp / MAX (dlon,dlat)
        ztanfac = 0.5_wp
      ENDIF
      ...
      DO j = 1 , je
        ...
        DO i = 1 , ie
          ...
          IF ( lperi_x ) THEN
            zdrh = MIN ( j_td - jstart_tot + 0.25_wp , jend_tot - j_td + 0.25_wp )
          ELSE IF ( lperi_y ) THEN
            zdrh = MIN ( i_td - istart_tot + 0.25_wp , iend_tot - i_td + 0.25_wp )
          ELSE
            zdrh  =  MIN ( i_td - istart_tot + 0.25_wp , iend_tot - i_td + 0.25_wp , &
                           j_td - jstart_tot + 0.25_wp , jend_tot - j_td + 0.25_wp )
          END IF

          IF (zdrh <= 0.0_wp) THEN
            rmy (i,j,1) = 1.0_wp * zdtddtr
          ELSE
            rmy (i,j,1) = ( 1.0_wp - TANH (ztanfac*zdrh) )               &
                                       / TANH (ztanfac*zdrh)   *   zdtddtr
          ENDIF

          ! rmy(:,:,2) is defined for the u- gridpoints
          IF ( lperi_x ) THEN
            zdru =  MIN ( j_td - jstartu_tot + 0.25_wp , jendu_tot - j_td + 0.25_wp )
          ELSE IF ( lperi_y ) THEN
            zdru =  MIN ( i_td - istartu_tot + 0.75_wp , iendu_tot - i_td + 0.75_wp )
          ELSE
            zdru =  MIN ( i_td - istartu_tot + 0.75_wp , iendu_tot - i_td + 0.75_wp ,&
                          j_td - jstartu_tot + 0.25_wp , jendu_tot - j_td + 0.25_wp )
          END IF

          IF (zdru <= 0.0_wp) THEN
            rmy (i,j,2) = 1.0_wp * zdtddtr
          ELSE
            rmy (i,j,2) = ( 1.0_wp - TANH (ztanfac*zdru) )               &
                                       / TANH (ztanfac*zdru)   *   zdtddtr
          ENDIF

          ! rmy(:,:,3) is defined for the v- gridpoints
          IF ( lperi_x ) THEN
            zdrv =  MIN ( j_td - jstartv_tot + 0.75_wp , jendv_tot - j_td + 0.75_wp )
          ELSE IF ( lperi_y ) THEN
            zdrv =  MIN ( i_td - istartv_tot + 0.25_wp , iendv_tot - i_td + 0.25_wp )
          ELSE
            zdrv =  MIN ( i_td - istartv_tot + 0.25_wp , iendv_tot - i_td + 0.25_wp ,&
                          j_td - jstartv_tot + 0.75_wp , jendv_tot - j_td + 0.75_wp )
          END IF

          IF (zdrv <= 0.0_wp) THEN
            rmy (i,j,3) = 1.0_wp * zdtddtr
          ELSE
            rmy (i,j,3) = ( 1.0_wp - TANH (ztanfac*zdrv) )               &
      ...
      IF( l2dim ) THEN  ! Recalculate the Davis Parameter for 2-D Version
        ...
        IF ( .NOT.lperi_x ) THEN ! only for non-periodicity in x-direction
          ...
          DO i = 1 , ie
            ...
            zdrh  =  MIN ( i_td - istart_tot + 0.25_wp , iend_tot - i_td + 0.25_wp)
            IF (zdrh <= 0.0_wp) THEN
              rmy (i,j2dim,1) = 1.0_wp * zdtddtr
            ELSE
              rmy (i,j2dim,1) = ( 1.0_wp - TANH (ztanfac*zdrh) )           &
                   / TANH (ztanfac*zdrh)   *   zdtddtr
            ENDIF
            zdru =  MIN ( i_td - istartu_tot + 0.75_wp , iendu_tot - i_td + 0.75_wp )
            IF (zdru <= 0.0_wp) THEN
              rmy (i,j2dim,2) = 1.0_wp * zdtddtr
            ELSE
              rmy (i,j2dim,2) = ( 1.0_wp - TANH (ztanfac*zdru) )           &
                   / TANH (ztanfac*zdru)   *   zdtddtr
            ENDIF
